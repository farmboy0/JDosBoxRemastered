...
public class Dosbox {
    public static boolean allPrivileges = true;
    ...
    private static final LoopHandler Normal_Loop = () -> {
        /*Bits*/
        int ret;
        while (true) {
            if (Pic.PIC_RunQueue()) {
...
//                               System.out.println(Callback.CallBack_Handlers[ret].getName());
                        /*Bitu*/
                        int blah = Callback.CallBack_Handlers[ret].call();
                        if (blah != 0)
                            return blah;
            ...
            if (ticksNew > ticksLast) {
                ...
                if (CPU.CPU_CycleAutoAdjust && !CPU.CPU_SkipCycleAutoAdjust) {
                    if (ticksScheduled >= 250 || ticksDone >= 250 || ticksAdded > 15 && ticksScheduled >= 5) {
                        ...
                        /*Bit32s*/
                        int ratio = (int) (ticksScheduled * (CPU.CPU_CyclePercUsed * 90 * 1024 / 100 / 100)
                            / ticksDone);
                        /*Bit32s*/
                        int new_cmax = CPU.CPU_CycleMax;
                        /*Bit64s*/
                        /*Bit64s*/
                        long cproc = CPU.CPU_CycleMax * ticksScheduled;
                        if (cproc > 0) {
                            /* ignore the cycles added due to the io delay code in order
                               to have smoother auto cycle adjustments */
                            double ratioremoved = (double) CPU.CPU_IODelayRemoved / (double) cproc;
                            if (ratioremoved < 1.0) {
                                ratio = (/*Bit32s*/int) (ratio * (1 - ratioremoved));
                                /* Don't allow very high ratio which can cause us to lock as we don't scale down
                                 * for very low ratios. High ratio might result because of timing resolution */
                                if (ticksScheduled >= 250 && ticksDone < 10 && ratio > 20480)
                                    ratio = 20480;
                                /*Bit64s*/
                                long cmax_scaled = (/*Bit64s*/long) CPU.CPU_CycleMax * (/*Bit64s*/long) ratio;
                                if (ratio <= 1024)
                                    new_cmax = (/*Bit32s*/int) (cmax_scaled / 1024);
                                else
                                    new_cmax = (/*Bit32s*/int) (1 + (CPU.CPU_CycleMax >> 1) + cmax_scaled / 2048);
                            }
                        }
                        ...
                        if (ratio > 10) {
                            ...
                            if (ratio > 120 || ticksDone < 700) {
                                ...
                                if (CPU.CPU_CycleLimit > 0) {
                                    if (CPU.CPU_CycleMax > CPU.CPU_CycleLimit)
                ...
                ticksAdded = 0;
                MainBase.Delay(1);
                ticksDone -= MainBase.GetTicks() - ticksNew;
                if (ticksDone < 0)
                    ticksDone = 0;
            }
        }
    ...
    private static final Mapper.MAPPER_Handler DOSBOX_UnlockSpeed = pressed -> {
        if (pressed) {
    ...
    private static final Section.SectionFunction DOSBOX_RealInit = sec -> {
    ...
    private static void DOSBOX_SetLoop(LoopHandler handler) {
        loop = handler;
    }

    public static void DOSBOX_SetNormalLoop() {
        loop = Normal_Loop;
    }
    ...
    public static void Init(Config cfg) {
        ...
        if (Render.RENDER_USE_ADVANCED_SCALERS > 2)
            scalers = new String[] { "none", "normal2x", "normal3x", "advmame2x", "advmame3x", "advinterp2x",
                "advinterp3x", "hq2x", "hq3x", "2xsai", "super2xsai", "supereagle", "tv2x", "tv3x", "rgb2x", "rgb3x",
                "scan2x", "scan3x" };
        else if (Render.RENDER_USE_ADVANCED_SCALERS > 0)
            scalers = new String[] { "none", "normal2x", "normal3x", "tv2x", "tv3x", "rgb2x", "rgb3x", "scan2x",
                "scan3x" };
        else
        ...
        if (allPrivileges) {
            ...
            Pint.Set_help(
                "The minimum number of ops the block must contain in order to be compiled.  In general 2 is a good value.");
        }
        ...
        String[] ems_settings = { "true", "emsboard", "emm386", "false" };
        ...
        if (Config.C_NE2000) {
            ...
            String[] ne2000_settings = { "false", "user", "pcap", "pcaphost" };
