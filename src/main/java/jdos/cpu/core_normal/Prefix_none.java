...
public class Prefix_none extends StringOp {
    ...
    public static OP[] ops = new OP[0x400];
    ...
    static {
        ...
        ops[0x8e] = () -> {
            ...
            switch (which) {
                ...
                case 0x05: /* MOV GS,Ew */
                case 0x04: /* MOV FS,Ew */
                    if (CPU.CPU_SetSegGeneral_index(which, val))
                        return RUNEXCEPTION();
                    break;
                default:
                    return ILLEGAL_OPCODE;
        ...
        ops[0xf6] = () -> {
            ...
            switch (which) {
                ...
                case 0x06: /* DIV Eb */
                    if (rm >= 0xc0) {
                        if (!DIVB(Modrm.GetEArb[rm].get()))
                            return RUNEXCEPTION();
                    } else {
                        int eaa = getEaa(rm);
                        if (!DIVB(Memory.mem_readb(eaa)))
                            return RUNEXCEPTION();
                    }
                    break;
                case 0x07: /* IDIV Eb */
                    if (rm >= 0xc0) {
                        if (!IDIVB(Modrm.GetEArb[rm].get()))
                            return RUNEXCEPTION();
                    } else {
                        int eaa = getEaa(rm);
                        if (!IDIVB(Memory.mem_readb(eaa)))
                            return RUNEXCEPTION();
                    }
                    break;
            }
        ...
        ops[0xf7] = () -> {
            ...
            switch (which) {
                ...
                case 0x06: /* DIV Ew */
                    if (rm >= 0xc0) {
                        if (!DIVW(Modrm.GetEArw[rm].word()))
                            return RUNEXCEPTION();
                    } else {
                        int eaa = getEaa(rm);
                        if (!DIVW(Memory.mem_readw(eaa)))
                            return RUNEXCEPTION();
                    }
                    break;
                case 0x07: /* IDIV Ew */
                    if (rm >= 0xc0) {
                        if (!IDIVW(Modrm.GetEArw[rm].word()))
                            return RUNEXCEPTION();
                    } else {
                        int eaa = getEaa(rm);
                        if (!IDIVW(Memory.mem_readw(eaa)))
                            return RUNEXCEPTION();
                    }
                    break;
            }
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_CPU, LogSeverities.LOG_ERROR,
                            "CPU:GRP5:Illegal Call " + Integer.toString(which, 16));
                    return ILLEGAL_OPCODE;
            }
            return HANDLED;
        };
