    ...
    static {
        ...
        ops[0x2f7] = () -> {
            ...
            switch (which) {
                ...
                case 0x06: /* DIV Ed */
                    if (rm >= 0xc0) {
                        if (!DIVD(Modrm.GetEArd[rm].dword))
                            return RUNEXCEPTION();
                    } else {
                        int eaa = getEaa(rm);
                        if (!DIVD(Memory.mem_readd(eaa)))
                            return RUNEXCEPTION();
                    }
                    break;
                case 0x07: /* IDIV Ed */
                    if (rm >= 0xc0) {
                        if (!IDIVD(Modrm.GetEArd[rm].dword))
                            return RUNEXCEPTION();
                    } else {
                        int eaa = getEaa(rm);
                        if (!IDIVD(Memory.mem_readd(eaa)))
                            return RUNEXCEPTION();
                    }
                    break;
            }
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_CPU, LogSeverities.LOG_ERROR,
                            "CPU:66:GRP5:Illegal call " + Integer.toString(which, 16));
                    return ILLEGAL_OPCODE;
            }
            return HANDLED;
        };
