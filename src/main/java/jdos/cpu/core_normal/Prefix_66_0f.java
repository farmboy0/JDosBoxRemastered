    ...
    static {
                ...
                case 0x05: {
                    ...
                    switch (which) {
                        case 0x02:
                            ...
                            if (CPU.CPU_LLDT(loadval))
                                return RUNEXCEPTION();
                            break;
                        case 0x03:
                            if (CPU.cpu.cpl != 0)
                                return EXCEPTION(CPU.EXCEPTION_GP);
                            if (CPU.CPU_LTR(loadval))
                                return RUNEXCEPTION();
                            break;
                        case 0x04:
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_CPU, LogSeverities.LOG_ERROR,
                            "GRP6:Illegal call " + Integer.toString(which, 16));
                    return ILLEGAL_OPCODE;
            }
            return HANDLED;
        };
            ...
            if (rm < 0xc0) { //First ones all use EA
                ...
                switch (which) {
                    ...
                    case 0x04: /* SMSW */
                        ...
                        break;
                    case 0x06: /* LMSW */
                        if (CPU.CPU_LMSW(Modrm.GetEArd[rm].dword))
                            return RUNEXCEPTION();
                        break;
                    default:
                        if (Log.level <= LogSeverities.LOG_ERROR)
                            Log.log(LogTypes.LOG_CPU, LogSeverities.LOG_ERROR,
                                "Illegal group 7 RM subfunction " + which);
                        return ILLEGAL_OPCODE;
                }
        ...
        ops[0x3a3] = () -> {
            ...
            if (rm >= 0xc0) {
                ...
                /*PhysPt*/
                int eaa = getEaa(rm);
                eaa += (Modrm.Getrd[rm].dword >> 5) * 4; // intentional signed shift
                /*Bit32u*/
...
//                System.out.println(System.currentTimeMillis()-start);
        ...
        ops[0x3c7] = () -> {
            ...
            switch (rm & 0x38) {
                case 8: // CMPXCHG8B
                {
                    long value1 = (CPU_Regs.reg_edx.dword & 0xffffffffL) << 32
                        | CPU_Regs.reg_eax.dword & 0xffffffffL;
                    int eaa = getEaa(rm);
                    long value2 = Memory.mem_readd(eaa) & 0xffffffffL
                        | (Memory.mem_readd(eaa + 4) & 0xffffffffL) << 32;
                    Flags.FillFlags();
                    if (value1 == value2) {
