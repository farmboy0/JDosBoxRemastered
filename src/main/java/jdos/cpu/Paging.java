...
public class Paging extends Module_base {
    ...
    public static final int TLB_SIZE;
    public static final int BANK_SHIFT = 28;
    public static final int BANK_MASK = 0xffff; // always the same as TLB_SIZE-1?
    public static final int TLB_BANKS;

    public static int LINK_START = (1024 + 64) / 4; //Start right after the HMA

    public static final int INVALID_ADDRESS = 0xFFFFFFFF;
    ...
    public static /*Bitu*/ int cr3;
    public static /*Bitu*/ int cr2;
    ...
    public static final /*HostPt*/ int[] read = new int[TLB_SIZE];
    public static final /*HostPt*/ int[] write = new int[TLB_SIZE];
    public static final PageHandler[] readhandler = new PageHandler[TLB_SIZE];
    public static final PageHandler[] writehandler = new PageHandler[TLB_SIZE];
    public static final /*Bit32u*/ int[] phys_page = new int[TLB_SIZE];
    ...
    public static final Links ur_links = new Links();
    public static final Links krw_links = new Links();
    public static final Links kr_links = new Links();
    public static final /*Bit32u*/ long[] firstmb = new long[LINK_START];
    ...
    public static int getDirectIndex(int address) {
        ...
        if (tlb_addr == INVALID_ADDRESS)
            return -1;
        tlb_addr = get_tlb_write(address);
        if ((tlb_addr == INVALID_ADDRESS) || ((get_tlb_writehandler(address).flags & PFLAG_HASCODE) != 0))
            return -1;
        return tlb_addr + address;
    }
    ...
    public static /*Bit16u*/int mem_readw_inline(/*PhysPt*/int address) {
        if ((address & 0xfff) < 0xfff) {
            ...
            int tlb_addr = get_tlb_read(address);
            if (tlb_addr != INVALID_ADDRESS)
                return RAM.readw(tlb_addr + address);
            else
    ...
    public static /*Bit32u*/int mem_readd_inline(/*PhysPt*/int address) {
        ...
        if ((address & 0xfff) < 0xffd) {
            ...
            int tlb_addr = get_tlb_read(address);
            if (tlb_addr != INVALID_ADDRESS)
                return RAM.readd(tlb_addr + address);
            else
                return get_tlb_readhandler(address).readd(address);
        } else
    ...
    public static void mem_writeb_inline(/*PhysPt*/int address, /*Bit8u*/int val) {
        ...
        if (tlb_addr != INVALID_ADDRESS)
            RAM.writeb(tlb_addr + address, val);
        else
    ...
    public static void mem_writew_inline(/*PhysPt*/int address, /*Bit16u*/int val) {
        if ((address & 0xfff) < 0xfff) {
            ...
            int tlb_addr = get_tlb_write(address);
            if (tlb_addr != INVALID_ADDRESS)
                RAM.writew(tlb_addr + address, val);
            else
    ...
    public static void mem_writed_inline(/*PhysPt*/int address, /*Bit32u*/int val) {
        ...
        if ((address & 0xfff) < 0xffd) {
            ...
            int tlb_addr = get_tlb_write(address);
            if (tlb_addr != INVALID_ADDRESS)
                RAM.writed(tlb_addr + address, val);
            else
                get_tlb_writehandler(address).writed(address, val);
        } else
    ...
    private static Pf_queue pf_queue = new Pf_queue();
    ...
    private static CPU.CPU_Decoder PageFaultCore = () -> {
        ...
        if (pentry.block.p != 0 && entry.cs == CPU_Regs.reg_csVal.dword && entry.eip == CPU_Regs.reg_eip) {
            ...
            if (Callback.inHandler == 0) {
                ...
                CPU.cpudecoder = Paging.old_cpudecoder;
                Paging.old_cpudecoder = null;
    ...
    final static private String[] mtr = { "KR ", "KRW", "UR ", "URW", "PFL" };
...
// There's something strange about KR with WP=1 though
    private final static /*Bit8u*/ int[] translate_array = { ACCESS_KR, // 00 00
        ACCESS_KR, // 00 01
        ACCESS_KR, // 00 10
        ACCESS_KR, // 00 11
        ACCESS_KR, // 01 00
    ...
    final static private String[] lnm = { "RW ", "RE ", "EE " }; // debug stuff

    // bit0-1 ACCESS_ type
    // bit2   1=user mode
    // bit3   WP on

    private final static /*Bit8u*/ int[] xlat_mapping = {
        //  KR        KRW       UR        URW
    ...
    private final static /*Bit8u*/ int[] fault_table = {
        //	KR	KRW	UR	URW
        // WP 0
    ...
    */
    ...
    private static CPU.CPU_Decoder old_cpudecoder = null;
    ...
    static void PAGING_NewPageFault(/*PhysPt*/int lin_addr, /*Bitu*/int page_addr, boolean prepare_only,
        /*Bitu*/int faultcode) {
        cr2 = lin_addr;
        ...
        if (prepare_only) {
            ...
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_PAGING, LogSeverities.LOG_NORMAL,
                    "Left PageFault for " + Long.toString(lin_addr, 16) + " queue " + pf_queue.used);
    ...
    private static class PageFoilHandler extends PageHandler {
...
//			dirEntryAddr, tableEntryAddr, table_entry.load, wtest);
            ...
            if ((handler.flags & PFLAG_WRITEABLE) != 0)
                ...
                write[lin_page] = Paging.INVALID_ADDRESS;
    ...
    private static class ExceptionPageHandler extends PageHandler {
        ...
        void Exception(/*PhysPt*/int addr, boolean writing, boolean checked) {
            ...
            PAGING_NewPageFault(addr, tableaddr, checked,
                1 | (writing ? 2 : 0) | ((CPU.cpu.cpl & CPU.cpu.mpl) == 3 ? 4 : 0));
    ...
    private static class NewInitPageHandler extends PageHandler {
        ...
        boolean InitPage(/*PhysPt*/int lin_addr, boolean writing, boolean prepare_only) {
            ...
            if (enabled) {
                ...
                while (true) {
                    ...
                    /*PhysPt*/
                    int tableEntryAddr = GetPageTableEntryAddr(lin_addr, dir_entry);
                    table_entry.load(Memory.phys_readd(tableEntryAddr));
                    ...
                    /*Bit32u*/
                    int table_load = table_entry.load();
                ...
                if (lin_page < LINK_START)
                    phys_page = (int) firstmb[lin_page];
                else
                    phys_page = lin_page;
    ...
    public static boolean PAGING_MakePhysPage(/*Bitu*/IntRef page) {
        if (enabled) {
            /*Bitu*/
            ...
            if (page.value < LINK_START)
                page.value = (int) firstmb[page.value];
    ...
    public static /*Bitu*/int PAGING_GetDirBase() {
        return cr3;
    }
    ...
    private static void PAGING_LinkPageNew(/*Bitu*/int lin_page, /*Bitu*/int ppage, /*Bitu*/int linkmode, boolean dirty,
        boolean global) {
        /*Bitu*/
        int xlat_index = linkmode | (wp ? 8 : 0) | (CPU.cpu.cpl == 3 ? 4 : 0);
        /*Bit8u*/
    ...
    private static void PAGING_LinkPage(/*Bitu*/int lin_page, /*Bitu*/int ppage) {
        PageHandler handler = Memory.MEM_GetPageHandler(ppage);
        /*Bitu*/
        int lin_base = lin_page << 12;
        if (lin_page >= TLB_SIZE || ppage >= TLB_SIZE)
    ...
    public static void PAGING_SwitchCPL(boolean isUser) {
        ...
        if (isUser) {
            ...
            for (/*Bitu*/int i = 0; i < krw_links.used; i++) {
                /*Bitu*/
                int tlb_index = krw_links.entries[i];
        ...
        if (wp) {
            ...
            if (isUser) {
                ...
                for (/*Bitu*/int i = 0; i < kr_links.used; i++) {
                    /*Bitu*/
                    int tlb_index = kr_links.entries[i];
                    readhandler[tlb_index] = exception_handler;
                    read[tlb_index] = INVALID_ADDRESS;
                }
            } else {
                ...
                for (/*Bitu*/int i = 0; i < kr_links.used; i++) {
                    ...
                    int lin_base = tlb_index << 12;
                    /*Bitu*/
                    int ppage = phys_page[tlb_index] & PHYSPAGE_ADDR;
                    PageHandler handler = Memory.MEM_GetPageHandler(ppage);
                    readhandler[tlb_index] = handler;
                    if ((handler.flags & PFLAG_READABLE) != 0)
                        read[tlb_index] = handler.GetHostReadPt(ppage) - lin_base;
            ...
            if (isUser) {
                ...
                for (/*Bitu*/int i = 0; i < ur_links.used; i++) {
                    /*Bitu*/
                    int tlb_index = ur_links.entries[i];
                    writehandler[tlb_index] = exception_handler;
                    write[tlb_index] = INVALID_ADDRESS;
                }
            } else {
...
//		Log.log(LogTypes.LOG_PAGING,LogSeverities.LOG_NORMAL,"Enabled");
    ...
    public static Section.SectionFunction PAGING_ShutDown = section -> {
        //paging = null;
    };

    public static Section.SectionFunction PAGING_Init = section -> {
