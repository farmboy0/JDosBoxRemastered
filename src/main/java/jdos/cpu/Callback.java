...
public class Callback {
    ...
    public static final int CB_INT13 = 21;
    public static final int CB_MAX = 128;
    public static final int CB_SIZE = 32;
    public static final int CB_SEG = 0xF000;
    public static final int CB_SOFFSET = 0x1000;
    ...
    public static Handler[] CallBack_Handlers = new Handler[CB_MAX];
    public static String[] CallBack_Description = new String[CB_MAX];
        ...
        public /*Bitu*/int call() {
            if (Log.level <= LogSeverities.LOG_ERROR)
                Log.log(LogTypes.LOG_CPU, LogSeverities.LOG_ERROR,
                    "Illegal Unhandled Interrupt Called " + Integer.toString(CPU.lastint, 16));
            return CBRET_NONE;
        }
    ...
    private static final Handler stop_handler = new Handler() {
        @Override
        public /*Bitu*/int call() {
            return CBRET_STOP;
        }
    ...
    public static Section.SectionFunction CALLBACK_Init = section -> {
        /*Bitu*/
        int i;
        for (i = 0; i < CB_MAX; i++) {
        ...
        for (i = 0; i <= 11; i++)
            Memory.phys_writeb(CALLBACK_PhysPointer(call_idle) + i, 0x90);
        ...
        /*PhysPt*/
        int rint_base = CALLBACK_GetBase() + CB_MAX * CB_SIZE;
        for (i = 0; i <= 0xff; i++) {
    ...
    public static void CALLBACK_Idle() {
        ...
        /*Bitu*/
        int oldIF = CPU_Regs.GETFLAG(CPU_Regs.IF);
    ...
    public static void CALLBACK_RunRealFar(/*Bit16u*/int seg, /*Bit16u*/int off) {
        ...
        Memory.mem_writew(CPU_Regs.reg_ssPhys.dword + CPU_Regs.reg_esp.word(),
            Memory.RealOff(CALLBACK_RealPointer(call_stop)));
        Memory.mem_writew(CPU_Regs.reg_ssPhys.dword + CPU_Regs.reg_esp.word() + 2,
            Memory.RealSeg(CALLBACK_RealPointer(call_stop)));
        /*Bit32u*/
        int oldeip = CPU_Regs.reg_eip;
        /*Bit16u*/
        int oldcs = CPU_Regs.reg_csVal.dword;
    ...
    public static void CALLBACK_SZF(boolean val) {
        /*Bit16u*/
        int tempf = Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + CPU_Regs.reg_esp.word() + 4);
        if (val)
            tempf |= CPU_Regs.ZF;
        else
            tempf &= ~CPU_Regs.ZF;
    ...
    public static /*Bitu*/int CALLBACK_SetupExtra(/*Bitu*/int callback, /*Bitu*/int type, /*PhysPt*/int lphysAddress,
        ...
        switch (type) {
            ...
            case CB_INT16:
                ...
                Memory.phys_writeb(physAddress + 0x01, 0xCF); //An IRET Instruction
                for (/*Bitu*/int i = 0; i <= 0x0b; i++)
                    Memory.phys_writeb(physAddress + 0x02 + i, 0x90);
    ...
    public static /*Bitu*/int CALLBACK_Setup(/*Bitu*/int callback, Handler handler, /*Bitu*/int type,
        /*PhysPt*/int addr, String descr) {
        if (callback >= CB_MAX)
            return 0;
        /*Bitu*/
        int csize = CALLBACK_SetupExtra(callback, type, addr, handler != null);
        if (csize > 0) {
            CallBack_Handlers[callback] = handler;
            CALLBACK_SetDescription(callback, descr);
        }
    ...
    public void Uninstall() {
        ...
        if (m_type == SETUP) {
            if (vectorhandler.installed) {
                ...
                } else if (Log.level <= LogSeverities.LOG_WARN)
                    Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_WARN, "Interrupt vector changed on "
                        + Integer.toString(vectorhandler.interrupt, 16) + " " + CALLBACK_GetDescription(m_callback));
            }
    ...
    public void Install(Handler handler, /*Bitu*/int type, String description) {
        if (!installed) {
            ...
            Log.exit("Callback handler object already installed");
    }
