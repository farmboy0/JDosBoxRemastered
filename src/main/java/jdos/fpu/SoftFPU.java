...
public class SoftFPU {
    ...
    private static long FROUND(long in) {
        switch (fpu.round) {
            case ROUND_Nearest: {
                ...
                if (MicroDouble.gt(diff, MicroDouble.ONE_HALF))
                    return MicroDouble.ceil(in);
                else if (MicroDouble.lt(diff, MicroDouble.ONE_HALF))
                    return floor;
                else
                    return (MicroDouble.longValue(floor) & 1) != 0 ? MicroDouble.ceil(in) : floor;
            }
            case ROUND_Down:
    ...
    private static /*Real64*/long FPU_FLD80(/*PhysPt*/int addr) {
        ...
        /*Bit64s*/
        long mant64 = test.eind.ll() + 0x3FF >>> 11 & 0xfffffffffffffL;
        /*Bit64s*/
        long sign = (test.begin & 0x8000) != 0 ? 1 : 0;
    ...
    private static void FPU_ST80(/*PhysPt*/int addr, /*Bitu*/int reg) {
        Test test = new Test();
        /*Bit64s*/
        long sign80 = (fpu.regs[reg].ll() & 0x8000000000000000L) != 0 ? 1 : 0;
        /*Bit64s*/
        long exp80 = fpu.regs[reg].ll() & 0x7ff0000000000000L;
        /*Bit64s*/
        long exp80final = exp80 >> 52;
        /*Bit64s*/
        long mant80 = fpu.regs[reg].ll() & 0x000fffffffffffffL;
        /*Bit64s*/
    ...
    private static void FPU_FLD_F32(/*PhysPt*/int addr, /*Bitu*/int store_to) {
        fpu.regs[store_to].d = MicroDouble.floatToDouble(Memory.mem_readd(addr));
    }

    private static void FPU_FLD_F64(/*PhysPt*/int addr, /*Bitu*/int store_to) {
        fpu.regs[store_to].ll(Memory.mem_readd(addr), Memory.mem_readd(addr + 4));
    }

    private static void FPU_FLD_F80(/*PhysPt*/int addr) {
        fpu.regs[fpu.top].d = FPU_FLD80(addr);
    }

    private static void FPU_FLD_I16(/*PhysPt*/int addr, /*Bitu*/int store_to) {
        fpu.regs[store_to].d = MicroDouble.intToDouble((short) Memory.mem_readw(addr));
    }

    private static void FPU_FLD_I32(/*PhysPt*/int addr, /*Bitu*/int store_to) {
        fpu.regs[store_to].d = MicroDouble.intToDouble(Memory.mem_readd(addr));
    }

    private static void FPU_FLD_I64(/*PhysPt*/int addr, /*Bitu*/int store_to) {
    ...
    private static void FPU_FBLD(/*PhysPt*/int addr, /*Bitu*/int store_to) {
        /*Bit64u*/
        long val = 0;
        /*Bitu*/
        int in = 0;
    ...
    private static void FPU_FPREM() {
        /*Real64*/
        long valtop = fpu.regs[fpu.top].d;
        /*Real64*/
        long valdiv = fpu.regs[STV(1)].d;
        /*Bit64s*/
    ...
    private static void FPU_FPREM1() {
        ...
        /*Bit64s*/
        long ressaved;
        if (MicroDouble.gt(MicroDouble.sub(quot, quotf), MicroDouble.ONE_HALF))
            ressaved = MicroDouble.add(quotf, MicroDouble.ONE);
        else if (MicroDouble.lt(MicroDouble.sub(quot, quotf), MicroDouble.ONE_HALF))
            ressaved = quotf;
        else
            ressaved = (MicroDouble.longValue(quotf) & 1) != 0 ? MicroDouble.add(quotf, MicroDouble.ONE) : quotf;
        ressaved = MicroDouble.longValue(ressaved);
    ...
    private static void FPU_FYL2X() {
        fpu.regs[STV(1)].d = MicroDouble.mul(fpu.regs[STV(1)].d,
            MicroDouble.div(MicroDouble.log(fpu.regs[fpu.top].d), MicroDouble.log(MicroDouble.TWO)));
        FPU_FPOP();
    }

    private static void FPU_FYL2XP1() {
        fpu.regs[STV(1)].d = MicroDouble.mul(fpu.regs[STV(1)].d, MicroDouble.div(
            MicroDouble.log(MicroDouble.add(fpu.regs[fpu.top].d, MicroDouble.ONE)), MicroDouble.log(MicroDouble.TWO)));
        FPU_FPOP();
    }

    private static void FPU_FSCALE() {
        fpu.regs[fpu.top].d = MicroDouble.mul(fpu.regs[fpu.top].d,
            MicroDouble.pow(MicroDouble.TWO, MicroDouble.truncate(fpu.regs[STV(1)].d)));
    ...
    private static void FPU_FXTRACT() {
        ...
        /*Real64*/
        long mant = MicroDouble.div(test.d, MicroDouble.pow(MicroDouble.TWO, exp80final));
    ...
    private static void EATREE(/*Bitu*/int _rm) {
        /*Bitu*/
        int group = _rm >> 3 & 7;
        switch (group) {
            case 0x00: /* FADD */
