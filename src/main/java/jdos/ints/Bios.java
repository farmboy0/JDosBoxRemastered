...
public class Bios extends Module_base {
    ...
    private static final Callback.Handler INT17_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL, "INT17:Function " + Integer.toString(CPU_Regs.reg_eax.high(), 16));
    ...
    private static final Callback.Handler INT14_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            ...
            if (port == 0) {
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL, "BIOS INT14: port " + CPU_Regs.reg_edx.word() + " does not exist.");
                return Callback.CBRET_NONE;
            }
            switch (CPU_Regs.reg_eax.high()) {
                case 0x00: {
    ...
    private static final Vector<E820> e820table = new Vector<E820>();
        ...
        public /*Bitu*/int call() {
            switch (CPU_Regs.reg_eax.high() & 0xFF) {
                ...
                case 0x88:    /* SYSTEM - GET EXTENDED MEMORY SIZE (286+) */
                    CPU_Regs.reg_eax.word(other_memsystems != 0 ? 0 : size_extended);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL, "INT15:Function 0x88 Remaining " + Integer.toString(CPU_Regs.reg_eax.word(), 16) + " kb");
                    Callback.CALLBACK_SCF(false);
                    break;
                ...
                case 0xc4:    /* BIOS POS Programm option Select */
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL, "INT15:Function " + Integer.toString(CPU_Regs.reg_eax.high(), 16) + " called, bios mouse not supported");
                    Callback.CALLBACK_SCF(true);
                    break;
                case 0x53: // APM BIOS
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        default:
                            if (Log.level <= LogSeverities.LOG_NORMAL)
                                Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL, "unknown APM BIOS call " + Integer.toString(CPU_Regs.reg_eax.word(), 16));
                            break;
                    }
                    Callback.CALLBACK_SCF(false);
                    break;
...
//    #undef DOSBOX_CLOCKSYNC
    ...
    static public int BIOS_DEFAULT_HANDLER_LOCATION() {
        return Memory.RealMake(0xf000, 0xff53);
    }

    static public int BIOS_DEFAULT_IRQ0_LOCATION() {
        return Memory.RealMake(0xf000, 0xfea5);
    }

    static public int BIOS_DEFAULT_IRQ1_LOCATION() {
        return Memory.RealMake(0xf000, 0xe987);
    }

    static public int BIOS_DEFAULT_IRQ2_LOCATION() {
        return Memory.RealMake(0xf000, 0xff55);
    }

    static public int BIOS_DEFAULT_RESET_LOCATION() {
