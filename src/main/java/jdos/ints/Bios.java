...
public class Bios extends Module_base {
    ...
    private static final Callback[] tandy_DAC_callback = new Callback[2];
    ...
    public static Section.SectionFunction BIOS_Destroy = section -> {
        test.destroy();
        test = null;
        tandy_dac = null;
        tandy_sb = null;
            ...
            switch (CPU_Regs.reg_eax.high() & 0xFF) {
                ...
                case 0x80: /* Pcjr Setup Sound Multiplexer */
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR,
                            "INT1A:80:Setup tandy sound multiplexer to " + CPU_Regs.reg_eax.low());
                    break;
                ...
                case 0xb1: /* PCI Bios Calls */
                    if (Log.level <= LogSeverities.LOG_WARN)
                        Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_WARN,
                            "INT1A:PCI bios call " + Integer.toString(CPU_Regs.reg_eax.low(), 16));
                    if (!Config.PCI_FUNCTIONALITY_ENABLED) {
                        ...
                        switch (CPU_Regs.reg_eax.low()) {
                            ...
                            case 0x02: { // find device
                                ...
                                int count = 0x100;
                                /*Bit32u*/
                                int devicetag = CPU_Regs.reg_ecx.word() << 16 | CPU_Regs.reg_edx.dword;
                                /*Bits*/
                                int found = -1;
                            ...
                            case 0x03: { // find device by class code
                                ...
                                int count = 0x100;
                                /*Bit32u*/
                                int classtag = CPU_Regs.reg_ecx.dword & 0xffffff;
                                /*Bits*/
                                int found = -1;
                                for (/*Bitu*/int i = 0; i <= count; i++) {
                                    IO.IO_WriteD(0xcf8, 0x80000000 | i << 8); // query unique device/subdevice entries
                            ...
                            case 0x08: // read configuration byte
                                IO.IO_WriteD(0xcf8,
                                    0x80000000 | CPU_Regs.reg_ebx.word() << 8 | CPU_Regs.reg_edi.word() & 0xfc);
                            ...
                            default:
                                if (Log.level <= LogSeverities.LOG_ERROR)
                                    Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR,
                                        "INT1A:PCI BIOS: unknown function "
                                            + Integer.toString(CPU_Regs.reg_eax.word(), 16) + " ("
                                            + Integer.toString(CPU_Regs.reg_ebx.word(), 16) + " "
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR,
                            "INT1A:Undefined call " + Integer.toString(CPU_Regs.reg_eax.high(), 16));
            }
            return Callback.CBRET_NONE;
        }
    };
    ...
    private static final Callback.Handler INT8_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            ...
            /*Bit8u*/
            int val = Memory.mem_readb(BIOS_DISK_MOTOR_TIMEOUT);
            if (val != 0)
                Memory.mem_writeb(BIOS_DISK_MOTOR_TIMEOUT, (short) (val - 1));
            /* and running drive */
    ...
    private static final Callback.Handler INT17_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL,
                    "INT17:Function " + Integer.toString(CPU_Regs.reg_eax.high(), 16));
    ...
    private static final Callback.Handler INT14_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            if (CPU_Regs.reg_eax.high() > 0x3 || CPU_Regs.reg_edx.word() > 0x3) { // 0-3 serial port functions
                ...
                Log.log_msg("BIOS INT14: Unhandled call AH=" + Integer.toString(CPU_Regs.reg_eax.high(), 16) + " DX="
                    + Integer.toString(CPU_Regs.reg_edx.word(), 16));
                return Callback.CBRET_NONE;
            }

            /*Bit16u*/
            int port = Memory.real_readw(0x40, CPU_Regs.reg_edx.word() * 2); // DX is always port number
            /*Bit8u*/
            short timeout = (short) Memory.mem_readb(BIOS_COM1_TIMEOUT + CPU_Regs.reg_edx.word());
            if (port == 0) {
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL,
                        "BIOS INT14: port " + CPU_Regs.reg_edx.word() + " does not exist.");
                return Callback.CBRET_NONE;
            }
            switch (CPU_Regs.reg_eax.high()) {
                case 0x00: {
    ...
    public static Section.SectionFunction BIOS_Init = section -> {
        biosConfigSeg = 0;
        ...
        public /*Bitu*/int call() {
            switch (CPU_Regs.reg_eax.high() & 0xFF) {
                ...
                case 0xC0: /* Get Configuration*/ {
                    if (biosConfigSeg == 0)
                        biosConfigSeg = Dos_tables.DOS_GetMemory(1); //We have 16 bytes
                    /*PhysPt*/
                ...
                case 0x83: /* BIOS - SET EVENT WAIT INTERVAL */ {
                    ...
                    Memory.mem_writed(BIOS_WAIT_FLAG_POINTER,
                ...
                case 0x87: /* Copy extended memory */ {
                    ...
                    /*PhysPt*/
                    int data = CPU_Regs.reg_esPhys.dword + CPU_Regs.reg_esi.word();
                    /*PhysPt*/
                ...
                case 0xE8:
                    ...
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        case 0x20: { /* E820: MEMORY LISTING */
                            if (CPU_Regs.reg_edx.dword == 0x534D4150 && CPU_Regs.reg_ecx.dword >= 20
                                && Memory.MEM_TotalPages() * 4 >= 24000) {
                                if (CPU_Regs.reg_ebx.dword < e820table.size()) {
                                    E820 e820 = e820table.elementAt(CPU_Regs.reg_ebx.dword);

                                    /* write to ES:DI */
                                    Memory.real_writed(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_edi.word() + 0x00,
                                        e820.base);
                                    Memory.real_writed(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_edi.word() + 0x04, 0);
                                    Memory.real_writed(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_edi.word() + 0x08,
                                        e820.len);
                                    Memory.real_writed(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_edi.word() + 0x0C, 0);
                                    Memory.real_writed(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_edi.word() + 0x10,
                                        e820.reserved ? 2 : 1);
                        ...
                        default:
                            if (Log.level <= LogSeverities.LOG_ERROR)
                                Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR,
                                    "INT15:Unknown call " + Integer.toString(CPU_Regs.reg_eax.word(), 16));
                            CPU_Regs.reg_eax.high(0x86);
                            Callback.CALLBACK_SCF(true);
                            if (Dosbox.IS_EGAVGA_ARCH()
                                || Dosbox.machine == MachineType.MCH_CGA/* || (Dosbox.machine == MachineType.MCH_AMSTRAD)*/) {
                ...
                case 0x88: /* SYSTEM - GET EXTENDED MEMORY SIZE (286+) */
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL,
                            "INT15:Function 0x88 Remaining " + Integer.toString(CPU_Regs.reg_eax.word(), 16) + " kb");
                    Callback.CALLBACK_SCF(false);
                    break;
                case 0x89: /* SYSTEM - SWITCH TO PROTECTED MODE */ {
                    ...
                    Memory.MEM_A20_Enable(true);
                    /*PhysPt*/
                    int table = CPU_Regs.reg_esPhys.dword + CPU_Regs.reg_esi.word();
                ...
                case 0xc4: /* BIOS POS Programm option Select */
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL, "INT15:Function "
                            + Integer.toString(CPU_Regs.reg_eax.high(), 16) + " called, bios mouse not supported");
                    Callback.CALLBACK_SCF(true);
                    break;
                case 0x53: // APM BIOS
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        case 0x0f:
                            ...
                            if (CPU_Regs.reg_ecx.word() == 0x0)
                                Log.log_msg(
                                    "disengage APM for device " + Integer.toString(CPU_Regs.reg_ebx.word(), 16));
                            else if (CPU_Regs.reg_ecx.word() == 0x1)
                        ...
                        default:
                            if (Log.level <= LogSeverities.LOG_NORMAL)
                                Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL,
                                    "unknown APM BIOS call " + Integer.toString(CPU_Regs.reg_eax.word(), 16));
                            break;
                    }
                    Callback.CALLBACK_SCF(false);
                    break;
...
//    #endif
    private static final Callback.Handler Reboot_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            ...
            CPU_Regs.reg_ebx.word(0);
            for (byte element : text) {
                CPU_Regs.reg_eax.low(element);
                Callback.CALLBACK_RunRealInt(0x10);
            }
            Log.log_msg(new String(text));
            double start = Pic.PIC_FullIndex();
            while (Pic.PIC_FullIndex() - start < 3000)
                Callback.CALLBACK_Idle();
...
//    #undef DOSBOX_CLOCKSYNC
    ...
    public Bios(Section configuration) {
        ...
        for (/*Bit16u*/int i = 0; i < 0x200; i++)
            Memory.real_writeb(0x40, i, 0);
        ...
        Callback.CALLBACK_Setup(call_irq0, INT8_Handler, Callback.CB_IRQ0,
        ...
        if (Dosbox.IS_TANDY_ARCH()) {
            ...
            if (Dosbox.machine == MachineType.MCH_TANDY)
                Memory.mem_writew(BIOS_MEMORY_SIZE, 624);
            else
                Memory.mem_writew(BIOS_MEMORY_SIZE, 640);
        ...
        Callback.CALLBACK_Setup(call_irq2, null, Callback.CB_IRET_EOI_PIC1,
        ...
        Memory.phys_writeb(Memory.Real2Phys(BIOS_DEFAULT_HANDLER_LOCATION()),
        ...
        if (Dosbox.machine == MachineType.MCH_TANDY)
            Memory.phys_writeb(0xffffe, 0xff); /* Tandy model */
        else if (Dosbox.machine == MachineType.MCH_PCJR)
            Memory.phys_writeb(0xffffe, 0xfd); /* PCJr model */
        else
            Memory.phys_writeb(0xffffe, 0xfc); /* PC */

        // System BIOS identification
        byte[] b_type = "IBM COMPATIBLE 486 BIOS COPYRIGHT The DOSBox Team.".getBytes();
        for (/*Bitu*/int i = 0; i < b_type.length; i++)
            Memory.phys_writeb(0xfe00e + i, b_type[i]);

        // System BIOS version
        byte[] b_vers = "DOSBox FakeBIOS v1.0".getBytes();
        for (/*Bitu*/int i = 0; i < b_vers.length; i++)
            Memory.phys_writeb(0xfe061 + i, b_vers[i]);

        // write system BIOS date
        byte[] b_date = "01/01/92".getBytes();
        for (/*Bitu*/int i = 0; i < b_date.length; i++)
            Memory.phys_writeb(0xffff5 + i, b_date[i]);
        Memory.phys_writeb(0xfffff, 0x55); // signature

        byte[] ident = "ISA".getBytes();
        for (/*Bitu*/int i = 0; i < ident.length; i++)
            Memory.phys_writeb(0xfffd9 + i, ident[i]);
        ...
        if (use_tandyDAC) {
            ...
            if (tandy_dac_type != 0) {
                ...
                for (/*Bit16u*/int i = 0; i < 0x10; i++)
                    Memory.phys_writeb(Memory.PhysMake(0xf000, 0xa084 + i), 0x80);
            } else
    ...
    public static int BIOS_DEFAULT_RESET_LOCATION() {
        return Memory.RealMake(0xf000, 0xe05b);
    }
...
//        } else {
    ...
    private static boolean Tandy_TransferInProgress() {
        ...
        if (Memory.real_readb(0x40, 0xd4) == 0xff)
            return false; /* still in init-state */

        /*Bit8u*/
        short tandy_dma = 1;
        if (tandy_sb.port != 0)
            tandy_dma = tandy_sb.dma;
        else if (tandy_dac.port != 0)
            tandy_dma = tandy_dac.dma;

        IoHandler.IO_Write(0x0c, 0x00);
        /*Bit16u*/
        int datalen = (/*Bit8u*/short) (IO.IO_ReadB(tandy_dma * 2 + 1) & 0xff);
        datalen |= IO.IO_ReadB(tandy_dma * 2 + 1) << 8;
        /* stop already requested */
        if (datalen == 0xffff)
            return false; /* no DMA transfer */
        else
            return datalen >= 0x10 || Memory.real_readb(0x40, 0xd4) != 0x0f || Memory.real_readw(0x40, 0xd2) != 0x1c;
    }
    ...
    private static void Tandy_SetupTransfer(/*PhysPt*/int bufpt, boolean isplayback) {
        ...
        if ((length == 0) || (tandy_sb.port == 0 && tandy_dac.port == 0))
            return;
        ...
        if (isplayback)
            IoHandler.IO_Write(0x0b, 0x48 | tandy_dma);
        else
            IoHandler.IO_Write(0x0b, 0x44 | tandy_dma);
        /* set physical address of buffer */
        /*Bit8u*/
        short bufpage = (/*Bit8u*/short) (bufpt >> 16 & 0xff);
        ...
        /*Bit8u*/
        short amplitude = (/*Bit8u*/short) (Memory.real_readw(0x40, 0xd2) >> 13 & 0x7);
        if (tandy_sb.port != 0) {
            ...
            if (isplayback)
                IoHandler.IO_Write(tandy_sb.port + 0xc, 0x14);
            else
                IoHandler.IO_Write(tandy_sb.port + 0xc, 0x24);
            /* set transfer size */
            ...
            if (isplayback)
                IoHandler.IO_Write(tandy_dac.port, IoHandler.IO_Read(tandy_dac.port) & 0x7c | 0x03);
            else
                IoHandler.IO_Write(tandy_dac.port, IoHandler.IO_Read(tandy_dac.port) & 0x7c | 0x02);
            ...
            if (isplayback)
                IoHandler.IO_Write(tandy_dac.port, IoHandler.IO_Read(tandy_dac.port) & 0x7c | 0x1f);
            else
                IoHandler.IO_Write(tandy_dac.port, IoHandler.IO_Read(tandy_dac.port) & 0x7c | 0x1e);
            IoHandler.IO_Write(0x0a, tandy_dma); /* enable DMA channel */
        }
    ...
    static void TandyDAC_Handler(/*Bit8u*/short tfunction) {
        if (tandy_sb.port == 0 && tandy_dac.port == 0)
            return;
        switch (tfunction) {
            case 0x81: /* Tandy sound system check */
                if (tandy_dac.port != 0) {
            ...
            case 0x83: /* Tandy sound system start playback */
                ...
                Tandy_SetupTransfer(Memory.PhysMake(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_ebx.word()),
                    CPU_Regs.reg_eax.high() == 0x83);
                CPU_Regs.reg_eax.high(0x00);
                Callback.CALLBACK_SCF(false);
                break;
    ...
    static void BIOS_HostTimeSync() {
        ...
        /*Bit32u*/
        long ticks = (long) ((calendar.get(Calendar.HOUR_OF_DAY) * 3600 * 1000
            + calendar.get(Calendar.MINUTE) * 60 * 1000 + calendar.get(Calendar.SECOND) * 1000
            + calendar.get(Calendar.MILLISECOND)) * (Timer.PIT_TICK_RATE / 65536.0 / 1000.0));
    ...
    public static void BIOS_SetComPorts(/*Bit16u*/int[] baseaddr) {
        /*Bit16u*/
        int portcount = 0;
        /*Bit16u*/
        int equipmentword;
        for (/*Bitu*/int i = 0; i < 4; i++) {
            if (baseaddr[i] != 0)
                portcount++;
            if (i == 0)
                Memory.mem_writew(BIOS_BASE_ADDRESS_COM1, baseaddr[i]);
            else if (i == 1)
                Memory.mem_writew(BIOS_BASE_ADDRESS_COM2, baseaddr[i]);
            else if (i == 2)
                Memory.mem_writew(BIOS_BASE_ADDRESS_COM3, baseaddr[i]);
            else
                Memory.mem_writew(BIOS_BASE_ADDRESS_COM4, baseaddr[i]);
        }
        // set equipment word
