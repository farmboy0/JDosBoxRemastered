...
public class EMS extends Module_base {
            ...
            switch (CPU_Regs.reg_eax.high()) {
                ...
                case 0x4b:        /* Get Handle Count */
                    ...
                    for (i = 0; i < EMM_MAX_HANDLES; i++)
                        if (emm_handles[i].pages != NULL_HANDLE) CPU_Regs.reg_ebx.word(CPU_Regs.reg_ebx.word() + 1);
                    CPU_Regs.reg_eax.high(EMM_NO_ERROR);
                    break;
                    ...
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        {    /*PhysPt*/
                            ...
                            for (i = 0; i < CPU_Regs.reg_ecx.word(); i++) {
                                ...
                                int logPage = Memory.mem_readw(data);
                                data += 2;
                                CPU_Regs.reg_eax.high(EMM_MapSegment(Memory.mem_readw(data), CPU_Regs.reg_edx.word(), logPage));
                                data += 2;
                                if (CPU_Regs.reg_eax.high() != EMM_NO_ERROR) break;
                            }
                        }
                            ...
                            /* Page is in Pageframe, so check what EMS-page it is
                               and return the physical address */
                                    ...
                                    if (handle == 0xffff) {
                                        ...
                                        /*MemHandle*/
                                        int memh = Memory.MEM_NextHandleAt(
    ...
    private static final Callback.Handler V86_Monitor = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            ...
            /*Bitu*/
            int int_num = (Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword & CPU.cpu.stack.mask)) - 0x2803);
            ...
            if ((int_num == (0x0d * 4)) && ((CPU_Regs.reg_esp.word() & 0xffff) != 0x1fda)) {
                ...
                /*Bit16u*/
                int v86_cs = Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + ((CPU_Regs.reg_esp.dword + 4) & CPU.cpu.stack.mask));
                /*Bit16u*/
                int v86_ip = Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + ((CPU_Regs.reg_esp.dword) & CPU.cpu.stack.mask));
                /*Bit8u*/
            ...
            /*Bit16u*/
            int return_cs = Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + ((CPU_Regs.reg_esp.dword + 4) & CPU.cpu.stack.mask));
            /*Bit32u*/
            int return_eflags = Memory.mem_readd(CPU_Regs.reg_ssPhys.dword + ((CPU_Regs.reg_esp.dword + 8) & CPU.cpu.stack.mask));
            ...
            /*Bit16u*/
            int v86_ss = Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + ((CPU_Regs.reg_esp.dword + 0x10) & CPU.cpu.stack.mask));
            /*Bit16u*/
            int v86_sp = (Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + ((CPU_Regs.reg_esp.dword + 0x0c) & CPU.cpu.stack.mask)) - 6) & 0xFFFF;
    ...
    private static /*Bit8u*/short MemoryRegion() {
        ...
        if (region.src_type == 0) {
            ...
            if ((emm_handles[region.src_handle].pages * EMM_PAGE_SIZE) < ((region.src_page_seg * EMM_PAGE_SIZE) + region.src_offset + region.bytes))
                return EMM_LOG_OUT_RANGE;
        ...
        if (region.dest_type == 0) {
            ...
            if (emm_handles[region.dest_handle].pages * EMM_PAGE_SIZE < (region.dest_page_seg * EMM_PAGE_SIZE) + region.dest_offset + region.bytes)
                return EMM_LOG_OUT_RANGE;
