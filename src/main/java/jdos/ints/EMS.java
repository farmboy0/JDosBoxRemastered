...
public class EMS extends Module_base {
    ...
    private static final int EMM_PAGEFRAME4K = EMM_PAGEFRAME * 16 / 4096;
    private static final int EMM_MAX_HANDLES = 200; /* 255 Max */
    private static final int EMM_PAGE_SIZE = 16 * 1024;
    private static final int EMM_MAX_PAGES = 32 * 1024 / 16;
    private static final int EMM_MAX_PHYS = 4; /* 4 16kb pages in pageframe */

    private static final int EMM_VERSION = 0x40;
    private static final int EMM_MINOR_VERSION = 0x00;
    //static final private int EMM_MINOR_VERSION	0x30	// emm386 4.48
    private static final int GEMMIS_VERSION = 0x0001; // Version 1.0

    private static final int EMM_SYSTEM_HANDLE = 0x0000;
    private static final int NULL_HANDLE = 0xffff;
    private static final int NULL_PAGE = 0xffff;
    ...
    static EMS test;
    public static Section.SectionFunction EMS_Init = section -> {
        ...
        section.AddDestroyFunction(EMS.EMS_ShutDown, true);
    };
    private static int ems_type;
    private static /*Bit16u*/ int GEMMIS_seg;
    private static final Vcpi vcpi = new Vcpi();
        ...
        public /*Bitu*/int call() {
            ...
            switch (CPU_Regs.reg_eax.high()) {
                ...
                case 0x44: /* Map Expanded Memory Page */
                    CPU_Regs.reg_eax
                        .high(EMM_MapPage(CPU_Regs.reg_eax.low(), CPU_Regs.reg_edx.word(), CPU_Regs.reg_ebx.word()));
                    break;
                ...
                case 0x4c: /* Get Pages for one Handle */
                    if (!ValidHandle(CPU_Regs.reg_edx.word())) {
                        CPU_Regs.reg_eax.high(EMM_INVALID_HANDLE);
                        break;
                    }
                ...
                case 0x4d: /* Get Pages for all Handles */
                    ...
                    CPU_Regs.reg_eax
                        .high(EMM_GetPagesForAllHandles(CPU_Regs.reg_esPhys.dword + CPU_Regs.reg_edi.word(), bx));
                    ...
                    switch (CPU_Regs.reg_eax.low()) {
                        case 0x00: /* Save Page Map */ {
                            ...
                            for (int j = 0; j < emm_mappings.length; j++)
                                Memory.MEM_BlockWrite(offset + j * EMM_Mapping.size, emm_mappings[j].data,
                                    EMM_Mapping.size);
                            CPU_Regs.reg_eax.high(EMM_NO_ERROR);
                            break;
                        }
                        case 0x01: /* Restore Page Map */ {
                            ...
                            for (int j = 0; j < emm_mappings.length; j++)
                                Memory.MEM_BlockRead(offset + j * EMM_Mapping.size, emm_mappings[j].data,
                                    EMM_Mapping.size);
                        ...
                        default:
                            if (Log.level <= LogSeverities.LOG_ERROR)
                                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                                    "EMS:Call " + Integer.toString(CPU_Regs.reg_eax.high(), 16) + " Subfunction "
                                        + Integer.toString(CPU_Regs.reg_eax.low(), 16) + " not supported");
                    ...
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        { /*PhysPt*/
                            ...
                            for (i = 0; i < CPU_Regs.reg_ecx.word(); i++) {
                                ...
                                data += 2;
                                CPU_Regs.reg_eax
                                    .high(EMM_MapSegment(Memory.mem_readw(data), CPU_Regs.reg_edx.word(), logPage));
                                data += 2;
                                if (CPU_Regs.reg_eax.high() != EMM_NO_ERROR)
                                    break;
                            }
                ...
                case 0x57: /* Memory region */
                    ...
                    if (CPU_Regs.reg_eax.high() != 0)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR, "EMS:Function 57 move failed");
                    break;
                case 0x58: // Get mappable physical array address array
                    if (CPU_Regs.reg_eax.low() == 0x00) {
                        ...
                        /*Bit16u*/
                        int step = 0x1000 / EMM_MAX_PHYS;
                        for (i = 0; i < EMM_MAX_PHYS; i++) {
                            Memory.mem_writew(data, EMM_PAGEFRAME + step * i);
                            data += 2;
                            Memory.mem_writew(data, i);
                            data += 2;
                        }
                    }
                    // Set number of pages
                ...
                case 0x5A: /* Allocate standard/raw Pages */
                    if (CPU_Regs.reg_eax.low() <= 0x01) {
                        ...
                        if (Log.level <= LogSeverities.LOG_ERROR)
                            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR, "EMS:Call 5A subfct "
                                + Integer.toString(CPU_Regs.reg_eax.low(), 16) + " not supported");
                        CPU_Regs.reg_eax.high(EMM_INVALID_SUB);
                    }
                    break;
                case 0xDE: /* VCPI Functions */
                    if (!vcpi.enabled) {
                        if (Log.level <= LogSeverities.LOG_ERROR)
                            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                                "EMS:VCPI Call " + Integer.toString(CPU_Regs.reg_eax.low(), 16) + " not supported");
                        CPU_Regs.reg_eax.high(EMM_FUNC_NOSUP);
                    } else {
                        switch (CPU_Regs.reg_eax.low()) {
                            case 0x00: /* VCPI Installation Check */
                                if (CPU_Regs.reg_ecx.word() == 0 && CPU_Regs.reg_edi.word() == 0x0012
                                    || CPU.cpu.pmode && (CPU_Regs.flags & CPU_Regs.VM) != 0) {
                            ...
                            case 0x01: { /* VCPI Get Protected Mode Interface */
                                ...
                                for (ct = 0; ct < 0xff; ct++) {
                                    Memory.real_writeb(CPU_Regs.reg_esVal.dword,
                                        CPU_Regs.reg_edi.word() + ct * 4 + 0x00, 0x67); // access bits
                                    Memory.real_writew(CPU_Regs.reg_esVal.dword,
                                        CPU_Regs.reg_edi.word() + ct * 4 + 0x01, ct * 0x10); // mapping
                                    Memory.real_writeb(CPU_Regs.reg_esVal.dword,
                                        CPU_Regs.reg_edi.word() + ct * 4 + 0x03, 0x00);
                                }
                                for (ct = 0xff; ct < 0x100; ct++) {
                                    Memory.real_writeb(CPU_Regs.reg_esVal.dword,
                                        CPU_Regs.reg_edi.word() + ct * 4 + 0x00, 0x67); // access bits
                                    Memory.real_writew(CPU_Regs.reg_esVal.dword,
                                        CPU_Regs.reg_edi.word() + ct * 4 + 0x01, (ct - 0xff) * 0x10 + 0x1100); // mapping
                                    Memory.real_writeb(CPU_Regs.reg_esVal.dword,
                                        CPU_Regs.reg_edi.word() + ct * 4 + 0x03, 0x00);
                                }
                                ...
                                for (ct = 0; ct < 4; ct++) {
                                    ...
                                    if (handle != 0xffff) {
                                        /*Bit16u*/
                                        /*Bit16u*/
                                        int memh = Memory.MEM_NextHandleAt(emm_handles[handle].mem,
                                            emm_mappings[ct].page() * 4);
                                        /*Bit16u*/
                                        int entry_addr = CPU_Regs.reg_edi.word() + (EMM_PAGEFRAME >> 6) + ct * 0x10;
                                        Memory.real_writew(CPU_Regs.reg_esVal.dword, entry_addr + 0x00 + 0x01,
                                            (memh + 0) * 0x10); // mapping of 1/4 of page
                                        Memory.real_writew(CPU_Regs.reg_esVal.dword, entry_addr + 0x04 + 0x01,
                                            (memh + 1) * 0x10); // mapping of 2/4 of page
                                        Memory.real_writew(CPU_Regs.reg_esVal.dword, entry_addr + 0x08 + 0x01,
                                            (memh + 2) * 0x10); // mapping of 3/4 of page
                                        Memory.real_writew(CPU_Regs.reg_esVal.dword, entry_addr + 0x0c + 0x01,
                                            (memh + 3) * 0x10); // mapping of 4/4 of page
                                    }
                                }
                                ...
                                Memory.real_writed(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_esi.word() + 0x00,
                                    0x0000ffff | cbseg_low);
                                Memory.real_writed(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_esi.word() + 0x04,
                                    0x00009a00 | cbseg_high);
                                /* Descriptor 2 (data segment, full access) */
                                Memory.real_writed(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_esi.word() + 0x08,
                                    0x0000ffff);
                                Memory.real_writed(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_esi.word() + 0x0c,
                                    0x00009200);
                                /* Descriptor 3 (full access) */
                                Memory.real_writed(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_esi.word() + 0x10,
                                    0x0000ffff);
                                Memory.real_writed(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_esi.word() + 0x14,
                                    0x00009200);
                            ...
                            case 0x06: { /* VCPI Get Physical Address of Page in 1st MB */
                                if (CPU_Regs.reg_ecx.word() << 8 >= EMM_PAGEFRAME
                                    && CPU_Regs.reg_ecx.word() << 8 < EMM_PAGEFRAME + 0x1000) {
                                    ...
                                    if (mem_seg < EMM_PAGEFRAME + 0x400)
                                        phys_page = 0;
                                    else if (mem_seg < EMM_PAGEFRAME + 0x800)
                                        phys_page = 1;
                                    else if (mem_seg < EMM_PAGEFRAME + 0xc00)
                                        phys_page = 2;
                                    else
                                        phys_page = 3;
                                    /*Bit16u*/
                            ...
                            case 0x0c: { /* VCPI Switch from V86 to Protected Mode */
                                ...
                                /*Bit16u*/
                                int new_ldt = Memory.mem_readw(CPU_Regs.reg_esi.dword + 0x0c);
                                /*Bit16u*/
                                int new_tr = Memory.mem_readw(CPU_Regs.reg_esi.dword + 0x0e);
                                /*Bit32u*/
                                int new_eip = Memory.mem_readd(CPU_Regs.reg_esi.dword + 0x10);
                                /*Bit16u*/
                                int new_cs = Memory.mem_readw(CPU_Regs.reg_esi.dword + 0x14);
                                ...
                                /*Bit8u*/
                                int tb = Memory.mem_readb(tbaddr);
                            ...
                            default:
                                if (Log.level <= LogSeverities.LOG_ERROR)
                                    Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR, "EMS:VCPI Call "
                                        + Integer.toString(CPU_Regs.reg_eax.word(), 16) + " not supported");
                                CPU_Regs.reg_eax.high(EMM_FUNC_NOSUP);
                                break;
                        }
                    }
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                            "EMS:Call " + Integer.toString(CPU_Regs.reg_eax.high(), 16) + " not supported");
                    CPU_Regs.reg_eax.high(EMM_FUNC_NOSUP);
                    break;
            }
        ...
        public /*Bitu*/int call() {
            ...
            switch (CPU_Regs.reg_eax.word()) {
                ...
                case 0xDE0C: { /* VCPI Switch from Protected Mode to V86 */
                    ...
                    Memory.mem_writed(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword & CPU.cpu.stack.mask) + 0x10,
                        0x23002);
                    ...
                    /*PhysPt*/
                    int tbaddr = vcpi.private_area + 0x0000 + (0x10 & 0xfff8) + 5;
                    /*Bit8u*/
                    int tb = Memory.mem_readb(tbaddr);
                    Memory.mem_writeb(tbaddr, tb & 0xfd);
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_WARN)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_WARN, "Unhandled VCPI-function "
                            + Integer.toString(CPU_Regs.reg_eax.low(), 16) + " in protected mode");
                    break;
            }
            return Callback.CBRET_NONE;
        }
    ...
    private static final Callback.Handler V86_Monitor = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            ...
            /*Bitu*/
            int int_num = Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword & CPU.cpu.stack.mask))
                - 0x2803;
            ...
            if (int_num == 0x0d * 4 && (CPU_Regs.reg_esp.word() & 0xffff) != 0x1fda) {
                ...
                /*Bit16u*/
                int v86_cs = Memory
                    .mem_readw(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword + 4 & CPU.cpu.stack.mask));
                /*Bit16u*/
                int v86_ip = Memory
                    .mem_readw(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword & CPU.cpu.stack.mask));
                /*Bit8u*/
                int v86_opcode = Memory.mem_readb((v86_cs << 4) + v86_ip);
                ...
                switch (v86_opcode) {
                    case 0x0f: // double byte opcode
                        ...
                        switch (v86_opcode) {
                            case 0x20: { // mov reg,CRx
                                /*Bitu*/
                                int rm_val = Memory.mem_readb((v86_cs << 4) + v86_ip + 2);
                                /*Bitu*/
                                int which = rm_val >> 3 & 7;
                                if (rm_val < 0xc0 || rm_val >= 0xe8)
                                    Log.exit("Invalid opcode 0x0f 0x20 " + Integer.toString(rm_val, 16)
                                ...
                                Memory.mem_writew(
                                    CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword & CPU.cpu.stack.mask),
                                    v86_ip + 3);
                            }
                                break;
                            case 0x22: { // mov CRx,reg
                                ...
                                int which = rm_val >> 3 & 7;
                                if (rm_val < 0xc0 || rm_val >= 0xe8)
                                    Log.exit("Invalid opcode 0x0f 0x22 " + Integer.toString(rm_val, 16)
                                        + " caused a protection fault!");
                                /*Bit32u*/
                                int crx = 0;
                                switch (rm_val & 7) {
                                ...
                                break;
                            default:
                                Log.exit("Unhandled opcode 0x0f " + Integer.toString(v86_opcode, 16)
                                    + " caused a protection fault!");
                        }
                        break;
                    case 0xe4: // IN AL,Ib
                        CPU_Regs.reg_eax
                            .low((/*Bit8u*/short) (IO.IO_ReadB(Memory.mem_readb((v86_cs << 4) + v86_ip + 1)) & 0xff));
                        Memory.mem_writew(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword & CPU.cpu.stack.mask),
                            v86_ip + 2);
                        break;
                    case 0xe5: // IN AX,Ib
                        /*Bit16u*/
                    ...
                    default:
                        Log.exit(
                            "Unhandled opcode " + Integer.toString(v86_opcode, 16) + " caused a protection fault!");
                }
                return Callback.CBRET_NONE;
            }
            ...
            /*Bit16u*/
            int vint_vector_seg = Memory.mem_readw(CPU_Regs.reg_dsVal.dword + int_num + 2);
            /*Bit16u*/
            int vint_vector_ofs = Memory.mem_readw(int_num);
            if (CPU_Regs.reg_esp.word() != 0x1fda)
                CPU_Regs.reg_esp.dword += 2 + 3 * 4; // Interrupt from within protected mode
            else
                CPU_Regs.reg_esp.dword += 2;

            /* Read entries that were pushed onto the stack by the interrupt */
            /*Bit16u*/
            int return_ip = Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword & CPU.cpu.stack.mask));
            /*Bit16u*/
            int return_cs = Memory
                .mem_readw(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword + 4 & CPU.cpu.stack.mask));
            /*Bit32u*/
            int return_eflags = Memory
                .mem_readd(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword + 8 & CPU.cpu.stack.mask));

            /* Modify stack to call v86-interrupt handler */
            Memory.mem_writed(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword & CPU.cpu.stack.mask),
                vint_vector_ofs);
            Memory.mem_writed(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword + 4 & CPU.cpu.stack.mask),
                vint_vector_seg);
            Memory.mem_writed(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword + 8 & CPU.cpu.stack.mask),
                return_eflags & ~(CPU_Regs.IF | CPU_Regs.TF));

            /* Adjust SP of v86-stack */
            /*Bit16u*/
            int v86_ss = Memory
                .mem_readw(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword + 0x10 & CPU.cpu.stack.mask));
            /*Bit16u*/
            int v86_sp = Memory
                .mem_readw(CPU_Regs.reg_ssPhys.dword + (CPU_Regs.reg_esp.dword + 0x0c & CPU.cpu.stack.mask)) - 6
                & 0xFFFF;
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_WARN)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_WARN,
                            "Unhandled interrupt 4B function " + Integer.toString(CPU_Regs.reg_eax.high(), 16));
                    break;
            }
            return Callback.CBRET_NONE;
        }
    ...
    public static Section.SectionFunction EMS_ShutDown = section -> {
        test.close();
        test = null;
        for (int i = 0; i < emm_handles.length; i++)
            emm_handles[i] = null;
    ...
    EMS(Section configuration) {
        ...
        if (ems_baseseg == 0)
            ems_baseseg = Dos_tables.DOS_GetMemory(2); //We have 32 bytes
        ...
        Callback.CALLBACK_Setup(call_int67, INT67_Handler, Callback.CB_IRET, Memory.PhysMake(ems_baseseg, 4),
            "Int 67 ems");
    ...
    private static /*Bit8u*/short EMM_AllocateMemory(/*Bit16u*/int pages, /*Bit16u*/IntRef dhandle,
        boolean can_allocate_zpages) {
        /* Check for 0 page allocation */
        if (pages == 0) {
            if (!can_allocate_zpages)
    ...
    private static /*Bit8u*/short EMM_AllocateSystemHandle(/*Bit16u*/int pages) {
        ...
        /*Bit16u*/
        int handle = EMM_SYSTEM_HANDLE; // emm system handle (reserved for OS usage)
    ...
    private static /*Bit8u*/short EMM_MapPage(/*Bitu*/int phys_page, /*Bit16u*/int handle, /*Bit16u*/int log_page) {
        ...
        if (log_page < emm_handles[handle].pages) {
            ...
            /*MemHandle*/
            int memh = Memory.MEM_NextHandleAt(emm_handles[handle].mem, log_page * 4);
            for (/*Bitu*/int i = 0; i < 4; i++) {
    ...
    private static /*Bit8u*/short EMM_SavePageMap(/*Bit16u*/int handle) {
        ...
        if (emm_handles[handle].saved_page_map)
        ...
        switch (CPU_Regs.reg_eax.low()) {
            case 0x00: /* Save Partial Page Map */
                ...
                for (; count > 0; count--) {
                    ...
                    if (segment >= EMM_PAGEFRAME && segment < EMM_PAGEFRAME + 0x1000) {
                        /*Bit16u*/
                        int page = (segment - EMM_PAGEFRAME) / (EMM_PAGE_SIZE >> 4);
                        ...
                        || segment >= EMM_PAGEFRAME - 0x1000 && segment < EMM_PAGEFRAME
    ...
    private static /*Bit8u*/short MemoryRegion() {
        ...
        /*Bit8u*/
        byte[] buf_src = new byte[Paging.MEM_PAGE_SIZE];
        /*Bit8u*/
        byte[] buf_dest = new byte[Paging.MEM_PAGE_SIZE];
        ...
        /*PhysPt*/
        int src_mem = 0, dest_mem = 0;
        /*MemHandle*/
        int src_handle = 0, dest_handle = 0;
        /*Bitu*/
        int src_off = 0, dest_off = 0;/*Bitu*/
        int src_remain = 0, dest_remain = 0;
        if (region.src_type == 0) {
            src_mem = region.src_page_seg * 16 + region.src_offset;
        } else {
            if (!ValidHandle(region.src_handle))
                return EMM_INVALID_HANDLE;
            if (emm_handles[region.src_handle].pages * EMM_PAGE_SIZE < region.src_page_seg * EMM_PAGE_SIZE
                + region.src_offset + region.bytes)
                return EMM_LOG_OUT_RANGE;
            src_handle = emm_handles[region.src_handle].mem;
            /*Bitu*/
            int pages = region.src_page_seg * 4 + region.src_offset / Paging.MEM_PAGE_SIZE;
            for (; pages > 0; pages--)
        ...
        if (region.dest_type == 0) {
            ...
            if (!ValidHandle(region.dest_handle))
                return EMM_INVALID_HANDLE;
            if (emm_handles[region.dest_handle].pages * EMM_PAGE_SIZE < region.dest_page_seg * EMM_PAGE_SIZE
                + region.dest_offset + region.bytes)
                return EMM_LOG_OUT_RANGE;
            dest_handle = emm_handles[region.dest_handle].mem;
            /*Bitu*/
            int pages = region.dest_page_seg * 4 + region.dest_offset / Paging.MEM_PAGE_SIZE;
            for (; pages > 0; pages--)
        ...
        while (region.bytes > 0) {
            ...
            if (region.src_type == 0) {
                ...
                if (toread < src_remain) {
                    ...
                    Memory.MEM_BlockRead(Memory.MEM_NextHandle(src_handle) * Paging.MEM_PAGE_SIZE, buf_src, src_remain,
                        toread - src_remain);
                }
            }
            /* Check for a move */
            if (CPU_Regs.reg_eax.low() == 1) {
                ...
                if (region.dest_type == 0) {
                    ...
                    if (toread < dest_remain) {
                        ...
                        Memory.MEM_BlockRead(Memory.MEM_NextHandle(dest_handle) * Paging.MEM_PAGE_SIZE, buf_dest,
                            dest_remain, toread - dest_remain);
                    }
                }
                /* Write to the source */
                if (region.src_type == 0) {
                    ...
                    if (toread < src_remain) {
                        ...
                        Memory.MEM_BlockWrite(Memory.MEM_NextHandle(src_handle) * Paging.MEM_PAGE_SIZE, buf_dest,
                            src_remain, toread - src_remain);
                    }
                }
            }
            ...
            if (region.dest_type == 0) {
                ...
                if (toread < dest_remain) {
                    ...
                    Memory.MEM_BlockWrite(Memory.MEM_NextHandle(dest_handle) * Paging.MEM_PAGE_SIZE, buf_src,
                        dest_remain, toread - dest_remain);
                }
            }
    ...
    private static void SetupVCPI() {
        ...
        /*Bit32u*/
        int ldt_address = vcpi.private_area + 0x1000;
        /*Bit16u*/
        int ldt_limit = 0xff;
        /*Bit32u*/
        ...
        /*Bit32u*/
        int tss_address = vcpi.private_area + 0x3000;
        /*Bit32u*/
        int tss_desc_part = (tss_address & 0xffff) << 16 | 0x0068 + 0x200;
        ...
        /*Bit32u*/
        int cs_desc_part = (vcpi.private_area & 0xffff) << 16 | 0xffff;
        ...
        /*Bit32u*/
        int ds_desc_part = (vcpi.private_area & 0xffff) << 16 | 0xffff;
    ...
    private static class device_EMM extends DOS_Device {
        ...
        public boolean ReadFromControlChannel(/*PhysPt*/int bufptr, /*Bit16u*/int size, /*Bit16u*/IntRef retcode) {
            /*Bitu*/
            int subfct = Memory.mem_readb(bufptr);
            switch (subfct) {
                case 0x00:
                    if (size != 6)
                        return false;
                ...
                case 0x01: {
                    ...
                    if (size != 6)
                        return false;
                    if (GEMMIS_seg == 0)
                        GEMMIS_seg = Dos_tables.DOS_GetMemory(0x20);
                    /*PhysPt*/
                    ...
                    for (/*Bitu*/int frct = 0; frct < 0x10 / 4; frct++) {
                        /*Bitu*/
                        int frnr = (frct + EMM_PAGEFRAME4K / 4) * 6;
