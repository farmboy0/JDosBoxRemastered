...
public class Int10_vesa {
    public static final int VESA_SUCCESS = 0x00;
    private static final int VESA_FAIL = 0x01;
    private static final int VESA_HW_UNSUPPORTED = 0x02;
    private static final int VESA_MODE_UNSUPPORTED = 0x03;
    // internal definition to pass to the caller
    private static final int VESA_UNIMPLEMENTED = 0xFF;
    private static final String string_oem = "S3 Incorporated. Trio64\0";
    private static final String string_vendorname = "DOSBox Development Team\0";
    private static final String string_productname = "DOSBox - The DOS Emulator\0";
    private static final String string_productrev = "DOSBox " + Config.VERSION + "\0";
    // maximum offset for the S3 Trio64 is 10 bits
    private static final int S3_MAX_OFFSET = 0x3ff;
    private static Callback callback;
    private static final jdos.cpu.Callback.Handler VESA_SetWindow = new jdos.cpu.Callback.Handler() {
        @Override
        ...
        public /*Bitu*/int call() {
            if (CPU_Regs.reg_ebx.high() != 0) {
                CPU_Regs.reg_eax.high(VESA_GetCPUWindow((short) CPU_Regs.reg_ebx.low(), CPU_Regs.reg_edx));
            } else
                CPU_Regs.reg_eax
                    .high(VESA_SetCPUWindow((short) CPU_Regs.reg_ebx.low(), (short) CPU_Regs.reg_edx.low()));
            CPU_Regs.reg_eax.low(0x4f);
            return 0;
        }
    };
    private static final jdos.cpu.Callback.Handler VESA_PMSetWindow = new jdos.cpu.Callback.Handler() {
        @Override
        public String getName() {
    ...
    private static final jdos.cpu.Callback.Handler VESA_PMSetPalette = new jdos.cpu.Callback.Handler() {
        @Override
        public String getName() {
        ...
        @Override
        public /*Bitu*/int call() {
            VESA_SetPalette(CPU_Regs.reg_esPhys.dword + CPU_Regs.reg_edi.dword, CPU_Regs.reg_edx.word(),
                CPU_Regs.reg_ecx.word());
            return 0;
        }
    };
    private static final jdos.cpu.Callback.Handler VESA_PMSetStart = new jdos.cpu.Callback.Handler() {
        @Override
        public String getName() {
            return "Int10_vesa.VESA_PMSetStart 0x"
                + Integer.toHexString(CPU_Regs.reg_edx.word() << 16 | CPU_Regs.reg_ecx.word());
        }
        ...
        public /*Bitu*/int call() {
            ...
            /*Bit32u*/
            int start = CPU_Regs.reg_edx.word() << 16 | CPU_Regs.reg_ecx.word();
    ...
    public static /*Bit8u*/short VESA_GetSVGAInformation(/*Bit16u*/int seg, /*Bit16u*/int off) {
        ...
        /*Bitu*/
        int i;
        boolean vbe2 = false;/*Bit16u*/
        int vbe2_pos = 256 + off;
        /*Bitu*/
        int id = Memory.mem_readd(buffer);
        if ((id == 0x56424532 || id == 0x32454256) && !Int10.int10.vesa_oldvbe)
            vbe2 = true;
        if (vbe2) {
            for (i = 0; i < 0x200; i++)
        ...
        if (!Int10.int10.vesa_oldvbe)
            Memory.mem_writew(buffer + 0x04, 0x200); //Vesa version 2.0
        else
            Memory.mem_writew(buffer + 0x04, 0x102); //Vesa version 1.2
        if (vbe2) {
    ...
    public static /*Bit8u*/short VESA_GetSVGAModeInformation(/*Bit16u*/int mode, /*Bit16u*/int seg, /*Bit16u*/int off) {
    ...
    public static int VESA_GetSVGAMode() {
        if (Int10.int10.vesa_setmode != 0xffff)
    ...
    public static int VESA_GetCPUWindow(/*Bit8u*/short window, CPU_Regs.Reg reg) {
        if (window != 0)
            return VESA_FAIL;
        IoHandler.IO_Write(0x3d4, 0x6a);
...
//Structure is (vesa 3.0 doc): blue,green,red,alignment
    ...
    public static /*Bit8u*/short VESA_GetPalette(/*PhysPt*/int data, /*Bitu*/int index, /*Bitu*/int count) {
        /*Bit8u*/
        short r, g, b;
        if ((index > 255) || (index + count > 256))
            return VESA_FAIL;
    ...
    public static /*Bit8u*/short VESA_ScanLineLength(/*Bit8u*/short subcall, /*Bit16u*/int val, /*Bit16u*/IntRef bytes,
        /*Bit16u*/IntRef pixels, /*Bit16u*/IntRef lines) {
        // offset register: virtual scanline length
        /*Bitu*/
        int pixels_per_offset;
        /*Bitu*/
        int bytes_per_offset = 8;
        /*Bitu*/
        int vmemsize = VGA.vga.vmemsize;
        /*Bitu*/
        int new_offset = VGA.vga.config.scan_len;
        /*Bitu*/
        int screen_height = Int10_modes.CurMode.sheight;
    ...
    public static /*Bit8u*/short VESA_SetDisplayStart(/*Bit16u*/int x, /*Bit16u*/int y) {
        // TODO wait for retrace in case bl==0x80
        /*Bitu*/
        int pixels_per_offset;
        /*Bitu*/
        int panning_factor = 1;
        ...
        /*Bitu*/
        int virtual_screen_width = VGA.vga.config.scan_len * pixels_per_offset;
        /*Bitu*/
        int new_start_pixel = virtual_screen_width * y + x;
        /*Bitu*/
        int new_crtc_start = new_start_pixel / (pixels_per_offset / 2);
        /*Bitu*/
        int new_panning = new_start_pixel % (pixels_per_offset / 2);
    ...
    public static /*Bit8u*/short VESA_GetDisplayStart(/*Bit16u*/IntRef x, /*Bit16u*/IntRef y) {
        ...
        IoHandler.IO_Write(0x3c0, 0x13 | 0x20); // panning register, screen on
        /*Bit8u*/
        short panning = IoHandler.IO_Read(0x3c1);

        /*Bitu*/
        int virtual_screen_width = VGA.vga.config.scan_len * pixels_per_offset;
        /*Bitu*/
        int start_pixel = VGA.vga.config.display_start * (pixels_per_offset / 2) + panning / panning_factor;
    ...
    public static void INT10_SetupVESA() {
        ...
        while (Int10_modes.ModeList_VGA[i].mode != 0xffff) {
            ...
            if (VGA.svga == null || VGA.svga.accepts_mode == null)
                ...
                if (VGA.svga.accepts_mode.call(Int10_modes.ModeList_VGA[i].mode))
                    canuse_mode = true;
            }
...
//        }
        ...
        jdos.cpu.Callback.CALLBACK_Setup(callback.setwindow, VESA_SetWindow, jdos.cpu.Callback.CB_RETF,
        ...
        Memory.phys_writew(Memory.Real2Phys(Int10.int10.rom.pmode_interface) + 0,
            Int10.int10.rom.pmode_interface_window);
        callback.pmWindow = jdos.cpu.Callback.CALLBACK_Allocate();
        /*Bit16u*/
        Int10.int10.rom.used += jdos.cpu.Callback.CALLBACK_Setup(callback.pmWindow, VESA_PMSetWindow,
            jdos.cpu.Callback.CB_RETN, Memory.PhysMake(0xc000, Int10.int10.rom.used), "VESA PM Set Window");
        /* PM Set start call */
        Int10.int10.rom.pmode_interface_start = Int10.int10.rom.used - Memory.RealOff(Int10.int10.rom.pmode_interface);
        Memory.phys_writew(Memory.Real2Phys(Int10.int10.rom.pmode_interface) + 2,
            Int10.int10.rom.pmode_interface_start);
        callback.pmStart = jdos.cpu.Callback.CALLBACK_Allocate();
        /*Bit16u*/
        Int10.int10.rom.used += jdos.cpu.Callback.CALLBACK_Setup(callback.pmStart, VESA_PMSetStart,
            jdos.cpu.Callback.CB_RETN, Memory.PhysMake(0xc000, Int10.int10.rom.used), "VESA PM Set Start");
        /* PM Set Palette call */
        Int10.int10.rom.pmode_interface_palette = Int10.int10.rom.used
            - Memory.RealOff(Int10.int10.rom.pmode_interface);
        Memory.phys_writew(Memory.Real2Phys(Int10.int10.rom.pmode_interface) + 4,
            Int10.int10.rom.pmode_interface_palette);
        callback.pmPalette = jdos.cpu.Callback.CALLBACK_Allocate();
        /*Bit16u*/
        Int10.int10.rom.used += jdos.cpu.Callback.CALLBACK_Setup(callback.pmPalette, VESA_PMSetPalette,
            jdos.cpu.Callback.CB_RETN, Memory.PhysMake(0xc000, Int10.int10.rom.used), "VESA PM Set Palette");
