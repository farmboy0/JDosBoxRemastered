...
public class Mouse {
    ...
    private static final /*Bit16u*/ int defaultTextAndMask = 0x77FF;
    private static final /*Bit16u*/ int defaultTextXorMask = 0x7700;
    private static final /*Bit16u*/ int[] defaultScreenMask = { 0x3FFF, 0x1FFF, 0x0FFF, 0x07FF, 0x03FF, 0x01FF, 0x00FF,
        0x007F, 0x003F, 0x001F, 0x01FF, 0x00FF, 0x30FF, 0xF87F, 0xF87F, 0xFCFF };
    private static final /*Bit16u*/ int[] defaultCursorMask = { 0x0000, 0x4000, 0x6000, 0x7000, 0x7800, 0x7C00, 0x7E00,
    ...
    public static _mouse mouse = new _mouse();
    public static Section.SectionFunction MOUSE_Destroy = section -> mouse = new _mouse();
    static /*Bit8u*/ short[] gfxReg3CE = new short[9];
    static /*Bit8u*/ short index3C4, gfxReg3C5;
    private static /*Bitu*/ int call_int33, call_int74, int74_ret_callback, call_mouse_bd;
    private static /*Bit16u*/ int ps2cbseg, ps2cbofs;
    private static boolean useps2callback, ps2callbackinit;
    private static /*Bitu*/ int call_ps2;
    private static /*RealPt*/ int ps2_callback;
    private static /*Bit16s*/ short oldmouseX, oldmouseY;
    private static final /*Bit16u*/ int[] userdefScreenMask = new int[CURSORY];
    private static final /*Bit16u*/ int[] userdefCursorMask = new int[CURSORY];
    private static final Callback.Handler PS2_Handler = new Callback.Handler() {
        @Override
        ...
        public /*Bitu*/int call() {
            ...
            CPU.CPU_Pop16();
            CPU.CPU_Pop16();
            CPU.CPU_Pop16();// remove the 4 words
            return Callback.CBRET_NONE;
        }
    };
    private static final Pic.PIC_EventHandler MOUSE_Limit_Events = new Pic.PIC_EventHandler() {
        @Override
    ...
    private static final Callback.Handler INT33_Handler = new Callback.Handler() {
        @Override
        public String getName() {
        ...
        public /*Bitu*/int call() {
            ...
            switch (CPU_Regs.reg_eax.word()) {
                ...
                case 0x04: /* Position Mouse */
                    ...
                    if ((/*Bit16s*/short) CPU_Regs.reg_ecx.word() >= mouse.max_x)
                        mouse.x = mouse.max_x;
                    else if (mouse.min_x >= CPU_Regs.reg_ecx.word())
                        mouse.x = mouse.min_x;
                    else if (CPU_Regs.reg_ecx.word() != POS_X())
                        mouse.x = CPU_Regs.reg_ecx.word();

                    if ((/*Bit16s*/short) CPU_Regs.reg_edx.word() >= mouse.max_y)
                        mouse.y = mouse.max_y;
                    else if (mouse.min_y >= (/*Bit16s*/short) CPU_Regs.reg_edx.word())
                        mouse.y = mouse.min_y;
                    else if ((/*Bit16s*/short) CPU_Regs.reg_edx.word() != POS_Y())
                        mouse.y = CPU_Regs.reg_edx.word();
                    DrawCursor();
                    break;
                ...
                case 0x07: /* Define horizontal cursor range */ { //lemmings set 1-640 and wants that. iron seeds set 0-640 but doesn't like 640
                    ...
                    /*Bit16s*/
                    short max, min;
                    if ((/*Bit16s*/short) CPU_Regs.reg_ecx.word() < (/*Bit16s*/short) CPU_Regs.reg_edx.word()) {
                        min = (/*Bit16s*/short) CPU_Regs.reg_ecx.word();
                        max = (/*Bit16s*/short) CPU_Regs.reg_edx.word();
                    } else {
                        min = (/*Bit16s*/short) CPU_Regs.reg_edx.word();
                        max = (/*Bit16s*/short) CPU_Regs.reg_ecx.word();
                    }
                    mouse.min_x = min;
                    mouse.max_x = max;
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_MOUSE, LogSeverities.LOG_NORMAL,
                            "Define Hortizontal range min:" + min + " max:" + max);
                }
                    break;
                case 0x08: /* Define vertical cursor range */ { // not sure what to take instead of the Int10_modes.CurMode (see case 0x07 as well)
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_MOUSE, LogSeverities.LOG_NORMAL,
                            "Define Vertical range min:" + min + " max:" + max);
                }
                    break;
                case 0x09: /* Define GFX Cursor */ {
                    /*PhysPt*/
                    int src = CPU_Regs.reg_esPhys.dword + CPU_Regs.reg_edx.word();
                ...
                case 0x0c: /* Define interrupt subroutine parameters */
                    ...
                    MainBase.Mouse_AutoLock(true); //Some games don't seem to reset the mouse before using
                    break;
                ...
                case 0x14: /* Exchange event-handler */ {
                    /*Bit16u*/
                    int oldSeg = mouse.sub_seg;
                    /*Bit16u*/
                    int oldOfs = mouse.sub_ofs;
                    /*Bit16u*/
                    int oldMask = mouse.sub_mask;
                    // Set new values
                    mouse.sub_mask = CPU_Regs.reg_ecx.word();
                    mouse.sub_seg = CPU_Regs.reg_esVal.dword;
                    mouse.sub_ofs = CPU_Regs.reg_edx.word();
                ...
                case 0x16: /* Save driver state */ {
                    ...
                    /*PhysPt*/
                    int dest = CPU_Regs.reg_esPhys.dword + CPU_Regs.reg_edx.word();
                ...
                case 0x1a: /* Set mouse sensitivity */
                    ...
                    if (Log.level <= LogSeverities.LOG_WARN)
                        Log.log(LogTypes.LOG_MOUSE, LogSeverities.LOG_WARN,
                            "Set sensitivity used with " + CPU_Regs.reg_ebx.word() + " " + CPU_Regs.reg_ecx.word()
                ...
                case 0x1b: /* Get mouse sensitivity */
                    ...
                    if (Log.level <= LogSeverities.LOG_WARN)
                        Log.log(LogTypes.LOG_MOUSE, LogSeverities.LOG_WARN,
                            "Get sensitivity " + CPU_Regs.reg_ebx.word() + " " + CPU_Regs.reg_ecx.word());
                    break;
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_MOUSE, LogSeverities.LOG_ERROR,
                            "Mouse Function " + Integer.toString(CPU_Regs.reg_eax.word(), 16) + " not implemented!");
                    break;
            }
            return Callback.CBRET_NONE;
        }
    };
    private static final Callback.Handler MOUSE_BD_Handler = new Callback.Handler() {
        @Override
        public String getName() {
        ...
        public /*Bitu*/int call() {
            ...
            /*Bit16u*/
            int raxpt = Memory.real_readw(CPU_Regs.reg_ssVal.dword, CPU_Regs.reg_esp.word() + 0x0a);
            /*Bit16u*/
            int rbxpt = Memory.real_readw(CPU_Regs.reg_ssVal.dword, CPU_Regs.reg_esp.word() + 0x08);
            /*Bit16u*/
            int rcxpt = Memory.real_readw(CPU_Regs.reg_ssVal.dword, CPU_Regs.reg_esp.word() + 0x06);
            /*Bit16u*/
            int rdxpt = Memory.real_readw(CPU_Regs.reg_ssVal.dword, CPU_Regs.reg_esp.word() + 0x04);

            // read out the actual values, registers ARE overwritten
            /*Bit16u*/
            int rax = Memory.real_readw(CPU_Regs.reg_dsVal.dword, raxpt);
    ...
    private static final Callback.Handler INT74_Handler = new Callback.Handler() {
        @Override
        public String getName() {
        ...
        public /*Bitu*/int call() {
            if (mouse.events > 0) {
                ...
                if ((mouse.sub_mask & mouse.event_queue[mouse.events].type) != 0) {
                    ...
                    if (mouse.in_UIR)
                        Log.log(LogTypes.LOG_MOUSE, LogSeverities.LOG_ERROR, "Already in UIR!");
                    mouse.in_UIR = true;
    ...
    private static final Callback.Handler MOUSE_UserInt_CB_Handler = new Callback.Handler() {
        @Override
        public String getName() {
    ...
    public static Section.SectionFunction MOUSE_Init = section -> {
        // Callback for mouse interrupt 0x33
        call_int33 = Callback.CALLBACK_Allocate();
        //	/*RealPt*/int i33loc=RealMake(CB_SEG+1,(call_int33*CB_SIZE)-0x10);
        /*RealPt*/
        int i33loc = Memory.RealMake(Dos_tables.DOS_GetMemory(0x1) - 1, 0x10);
        ...
        /*Bit8u*/
        short hwvec = MOUSE_IRQ > 7 ? 0x70 + MOUSE_IRQ - 8 : 0x8 + MOUSE_IRQ;
    ...
    public static boolean Mouse_SetPS2State(boolean use) {
        ...
        useps2callback = use;
        MainBase.Mouse_AutoLock(useps2callback);
        Pic.PIC_SetIRQMask(MOUSE_IRQ, !useps2callback);
        return true;
    }

    public static void Mouse_ChangePS2Callback(/*Bit16u*/int pseg, /*Bit16u*/int pofs) {
        if (pseg == 0 && pofs == 0) {
            ps2callbackinit = false;
            MainBase.Mouse_AutoLock(false);
        } else {
            ps2callbackinit = true;
            ps2cbseg = pseg;
            ps2cbofs = pofs;
        }
        MainBase.Mouse_AutoLock(ps2callbackinit);
    }

    private static void DoPS2Callback(/*Bit16u*/int data, /*Bit16s*/short mouseX, /*Bit16s*/short mouseY) {
        if (useps2callback) {
            /*Bit16u*/
    ...
    private static void Mouse_AddEvent(/*Bit8u*/int type) {
    ...
    private static void RestoreCursorBackgroundText() {
        if (mouse.hidden != 0 || mouse.inhibit_draw)
            return;

        if (mouse.background) {
            Int10_char.WriteChar(mouse.backposx, mouse.backposy,
                (short) Memory.real_readb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_PAGE), mouse.backData[0],
    ...
    private static void DrawCursorText() {
        ...
        /*Bit8u*/
        short page = (short) Memory.real_readb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_PAGE);
        /*Bit16u*/
        int result;
        ...
        Int10_char.WriteChar(mouse.backposx, mouse.backposy, page, (/*Bit8u*/short) (result & 0xFF),
            (/*Bit8u*/short) (result >> 8), true);
    }
    ...
    private static void SaveVgaRegisters() {
        if (Dosbox.IS_VGA_ARCH()) {
            ...
            IoHandler.IO_Write(0x3CE, 3);
            IoHandler.IO_Write(0x3Cf, 0); //disable rotate and operation
            IoHandler.IO_Write(0x3CE, 5);
            IoHandler.IO_Write(0x3Cf, gfxReg3CE[5] & 0xf0); //Force read/write mode 0

            //Set Map to all planes. Celtic Tales
            index3C4 = IoHandler.IO_Read(0x3c4);
            IoHandler.IO_Write(0x3C4, 2);
            gfxReg3C5 = IoHandler.IO_Read(0x3C5);
    ...
    private static void ClipCursorArea(/*Bit16s*/ShortRef x1, /*Bit16s*/ShortRef x2, /*Bit16s*/ShortRef y1,
        /*Bit16s*/ShortRef y2, /*Bit16u*/ShortRef addx1, /*Bit16u*/ShortRef addx2, /*Bit16u*/ShortRef addy) {
    ...
    private static void RestoreCursorBackground() {
        ...
        if (mouse.background) {
            ...
            short x, y;
            /*Bit16u*/
            ShortRef addx1 = new ShortRef(), addx2 = new ShortRef(), addy = new ShortRef();
            /*Bit16s*/
            ShortRef x1 = new ShortRef(mouse.backposx);
            /*Bit16s*/
            ShortRef y1 = new ShortRef(mouse.backposy);
            /*Bit16s*/
            ShortRef x2 = new ShortRef(x1.value + CURSORX - 1);
            /*Bit16s*/
            ShortRef y2 = new ShortRef(y1.value + CURSORY - 1);

            ClipCursorArea(x1, x2, y1, y2, addx1, addx2, addy);

            int dataPos = addy.value * CURSORX;
    ...
    private static void DrawCursor() {
        ...
        if (Memory.real_readb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_PAGE) != mouse.page)
            return;
        ...
        if (Int10_modes.CurMode.swidth > 0)
            xratio /= Int10_modes.CurMode.swidth;
        if (xratio == 0)
            xratio = 1;
        ...
        ShortRef addx1 = new ShortRef(), addx2 = new ShortRef(), addy = new ShortRef();
        /*Bit16s*/
        ShortRef x1 = new ShortRef(POS_X() / xratio - mouse.hotx);
        /*Bit16s*/
        ShortRef y1 = new ShortRef(POS_Y() - mouse.hoty);
        /*Bit16s*/
        ShortRef x2 = new ShortRef(x1.value + CURSORX - 1);
        /*Bit16s*/
        ShortRef y2 = new ShortRef(y1.value + CURSORY - 1);
        ...
        for (y = y1.value; y <= y2.value; y++) {
            /*Bit16u*/
            int scMask = mouse.screenMask[addy.value + y - y1.value];
            /*Bit16u*/
            int cuMask = mouse.cursorMask[addy.value + y - y1.value];
            if (addx1.value > 0) {
                scMask <<= addx1.value;
                cuMask <<= addx1.value;
                dataPos += addx1.value;
    ...
    public static void Mouse_CursorMoved(float xrel, float yrel, float x, float y, boolean emulate) {
        ...
        if (Math.abs(xrel) > 1.0 || mouse.senv_x < 1.0)
            dx *= mouse.senv_x;
        if (Math.abs(yrel) > 1.0 || mouse.senv_y < 1.0)
            dy *= mouse.senv_y;
        if (useps2callback)
            dy *= 2;
    ...
    public static void Mouse_ButtonPressed(/*Bit8u*/int button) {
        if (Keyboard.KEYBOARD_AUX_Active()) {
            switch (button) {
                case 0:
                    mouse.buttons |= 1;
    ...
    public static void Mouse_ButtonReleased(/*Bit8u*/int button) {
        if (Keyboard.KEYBOARD_AUX_Active()) {
            switch (button) {
                case 0:
                    mouse.buttons &= ~1;
    ...
    private static void Mouse_SetMickeyPixelRate(/*Bit16s*/int px, /*Bit16s*/int py) {
        if (px != 0 && py != 0) {
    ...
    private static void Mouse_SetSensitivity(/*Bit16u*/int px, /*Bit16u*/int py, /*Bit16u*/int dspeed) {
        if (px > 100)
            px = 100;
        if (py > 100)
            py = 100;
    ...
    public static void Mouse_NewVideoMode() {
        ...
        /*Bit8u*/
        int mode = Memory.mem_readb(Bios.BIOS_VIDEO_MODE);
        if (mode == mouse.mode) {
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_MOUSE, LogSeverities.LOG_NORMAL, "New video is the same as the old"); /*return;*/
        }
        ...
        switch (mode) {
            ...
            case 0x07: {
                ...
                /*Bitu*/
                int rows = Memory.real_readb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_ROWS);
                if (rows == 0 || rows > 250)
                    rows = 25 - 1;
            ...
            default:
                if (Log.level <= LogSeverities.LOG_ERROR)
                    Log.log(LogTypes.LOG_MOUSE, LogSeverities.LOG_ERROR,
                        "Unhandled videomode " + Integer.toString(mode, 16) + " on reset");
    ...
    public static class _mouse {
        private static final int[][] maskholder = new int[128][];
        ...
        /*Bit8u*/ short events;
        //Increase if QUEUE_SIZE >255 (currently 32)
        ...
        public byte[] save() {
            try {
                ...
                write(os, x);
                write(os, y);
                for (button_event element : event_queue) {
                    write8u(os, element.type);
                    write8u(os, element.buttons);
                }
                write8u(os, events);
                write16u(os, sub_seg);
                write16u(os, sub_ofs);
