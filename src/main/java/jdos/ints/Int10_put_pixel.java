...
public class Int10_put_pixel {
    private static final short[] cga_masks = { 0x3f, 0xcf, 0xf3, 0xfc };
    private static final short[] cga_masks2 = { 0x7f, 0xbf, 0xdf, 0xef, 0xf7, 0xfb, 0xfd, 0xfe };
        ...
        switch (Int10_modes.CurMode.type) {
            case VGA.M_CGA4: {
                if (Memory.real_readb(Int10.BIOSMEM_SEG, Int10.BIOSMEM_CURRENT_MODE) <= 5) {
                    ...
                    /*Bit8u*/
                    int old = Memory.real_readb(0xb800, off);
                    if ((color & 0x80) != 0) {
                        color &= 3;
                        old ^= color << 2 * (3 - (x & 3));
                    } else {
                    ...
                    /*Bit16u*/
                    int off = (y >> 2) * 160 + (x >> 2 & ~1);
                    off += 8 * 1024 * (y & 3);

                    /*Bit16u*/
                    int old = Memory.real_readw(seg, off);
                    if ((color & 0x80) != 0) {
                        old ^= (color & 1) << 7 - (x & 7);
                        old ^= (color & 2) >> 1 << 7 - (x & 7) + 8;
                    } else {
                        old = old & ~(0x101 << 7 - (x & 7)) | (color & 1) << 7 - (x & 7)
                            | (color & 2) >> 1 << 7 - (x & 7) + 8;
                    }
            ...
            case VGA.M_TANDY16: {
                ...
                // update the pixel
                /*Bit8u*/
                int old = Memory.real_readb(segment, offset);
                /*Bit8u*/
                int[] p = new int[2];
                p[1] = old >> 4 & 0xf;
                p[0] = old & 0xf;
                ...
                if (Int10_modes.CurMode.plength != Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_PAGE_SIZE))
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR,
                            "PutPixel_EGA_p: " + Integer.toString(Int10_modes.CurMode.plength, 16) + "!="
                                + Integer.toString(Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_PAGE_SIZE), 16));
                if (Int10_modes.CurMode.swidth != Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8)
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR, "PutPixel_EGA_w: "
                            + Integer.toString(Int10_modes.CurMode.swidth, 16) + "!="
                            + Integer.toString(Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8, 16));
                /*PhysPt*/
                int off = 0xa0000 + Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_PAGE_SIZE) * page
            ...
            case VGA.M_LIN8: {
                if (Int10_modes.CurMode.swidth != Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8)
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR, "PutPixel_VGA_w: "
                            + Integer.toString(Int10_modes.CurMode.swidth, 16) + "!="
                            + Integer.toString(Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8, 16));
                /*PhysPt*/
                int off = Int10.S3_LFB_BASE + y * Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8 + x;
            ...
            default:
                if (!putpixelwarned) {
                    ...
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR,
                            "PutPixel unhandled mode type " + Int10_modes.CurMode.type);
                }
                break;
        }
    }
        ...
        switch (Int10_modes.CurMode.type) {
            case VGA.M_CGA4: {
                ...
                if ((y & 1) != 0)
                    off += 8 * 1024;
                /*Bit8u*/
                int val = Memory.real_readb(0xb800, off);
                color = val >> (3 - (x & 3)) * 2 & 3;
            }
                break;
            case VGA.M_CGA2: {
            ...
            case VGA.M_EGA: {
                ...
                if (Int10_modes.CurMode.plength != Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_PAGE_SIZE))
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR,
                            "GetPixel_EGA_p: " + Integer.toString(Int10_modes.CurMode.plength, 16) + "!="
                                + Integer.toString(Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_PAGE_SIZE), 16));
                if (Int10_modes.CurMode.swidth != Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8)
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR, "GetPixel_EGA_w: "
                            + Integer.toString(Int10_modes.CurMode.swidth, 16) + "!="
                            + Integer.toString(Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8, 16));
                /*PhysPt*/
                int off = 0xa0000 + Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_PAGE_SIZE) * page
                ...
                IoHandler.IO_Write(0x3ce, 0x4);
                IoHandler.IO_Write(0x3cf, 2);
                color |= (Memory.mem_readb(off) >> shift & 1) << 2;
                IoHandler.IO_Write(0x3ce, 0x4);
                IoHandler.IO_Write(0x3cf, 3);
            ...
            case VGA.M_LIN8: {
                if (Int10_modes.CurMode.swidth != Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8)
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR, "GetPixel_VGA_w: "
                            + Integer.toString(Int10_modes.CurMode.swidth, 16) + "!="
                            + Integer.toString(Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8, 16));
                /*PhysPt*/
                int off = Int10.S3_LFB_BASE + y * Memory.real_readw(Int10.BIOSMEM_SEG, Int10.BIOSMEM_NB_COLS) * 8 + x;
            ...
            default:
                if (Log.level <= LogSeverities.LOG_ERROR)
                    Log.log(LogTypes.LOG_INT10, LogSeverities.LOG_ERROR,
                        "GetPixel unhandled mode type " + Int10_modes.CurMode.type);
                break;
        }
        return (short) color;
    }
