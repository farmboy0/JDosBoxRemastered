...
// 16     } dest;
//    }
    public static Section.SectionFunction XMS_Init = section -> {
        test = new XMS(section);
        section.AddDestroyFunction(XMS.XMS_ShutDown, true);
    };
    ...
    private static final Dos_system.MultiplexHandler multiplex_xms = () -> {
    ...
    public static Section.SectionFunction XMS_ShutDown = section -> {
        test.ShutDown();
        test = null;
    ...
    private static final Callback.Handler XMS_Handler = new Callback.Handler() {
        @Override
        ...
        public /*Bitu*/int call() {
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                    "XMS: CALL " + Integer.toString(CPU_Regs.reg_eax.high(), 16));
            switch (CPU_Regs.reg_eax.high() & 0xFF) {
                ...
                case XMS_LOCK_EXTENDED_MEMORY_BLOCK: { /* 0c */
                    ...
                    /*Bitu*/
                    int res = XMS_LockMemory(CPU_Regs.reg_edx.word(), address);
                    if (res != 0)
                ...
                case XMS_RESIZE_ANY_EXTENDED_MEMORY_BLOCK: /* 0x8f */
                    if ((CPU_Regs.reg_ebx.dword & 0xFFFFFFFFL) > CPU_Regs.reg_ebx.word())
                ...
                case XMS_ALLOCATE_UMB: { /* 10 */
                    ...
                    /*Bit16u*/
                    IntRef size = new IntRef(CPU_Regs.reg_edx.word());/*Bit16u*/
                    ...
                    if (Dos_memory.DOS_AllocateMemory(seg, size)) {
                        ...
                        if (size.value == 0)
                            ...
                            CPU_Regs.reg_ebx.low(UMB_ONLY_SMALLER_BLOCK);
                ...
                case XMS_GET_EMB_HANDLE_INFORMATION_EXT: { /* 8e */
                    ...
                    IntRef dx = new IntRef(CPU_Regs.reg_edx.word());
                    /*Bitu*/
                    int result = XMS_GetHandleInformation(CPU_Regs.reg_edx.word(), bh, free_handles, dx);
                    CPU_Regs.reg_ebx.high(bh.value);
                    CPU_Regs.reg_edx.word(dx.value);
                    if (result != 0)
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                            "XMS: unknown function " + Integer.toString(CPU_Regs.reg_eax.high(), 16));
                    CPU_Regs.reg_eax.word(0);
                    CPU_Regs.reg_ebx.low(XMS_FUNCTION_NOT_IMPLEMENTED);
            }
    ...
    private static /*Bitu*/int XMS_EnableA20(boolean enable) {
        /*Bit8u*/
        int val = IoHandler.IO_Read(0x92);
        if (enable)
            IoHandler.IO_Write(0x92, val | 2);
        else
            IoHandler.IO_Write(0x92, val & ~2);
        return 0;
    }

    private static /*Bitu*/int XMS_GetEnabledA20() {
    ...
    private static /*Bitu*/int XMS_AllocateMemory(/*Bitu*/int size, /*Bit16u*/IntRef handle) { // size = kb
        ...
        if (size != 0) {
            ...
            if (mem == 0)
                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR, "XMS:Allocate zero pages with no memory left");
        }
    ...
    private static /*Bitu*/int XMS_MoveMemory(/*PhysPt*/int bpt) {
        ...
        /*Bitu*/
        int length = Memory.mem_readd(bpt + 0/*offsetof(XMS_MemMove,length)*/);
        /*Bitu*/
        int src_handle = Memory.mem_readw(bpt + 4/*offsetof(XMS_MemMove,src_handle)*/);
        int src;
        int dest;
        src = Memory.mem_readd(bpt + 6/*offsetof(XMS_MemMove,src.offset)*/);
        /*Bitu*/
        int dest_handle = Memory.mem_readw(bpt + 10/*offsetof(XMS_MemMove,dest_handle)*/);
    ...
    private static /*Bitu*/int XMS_LockMemory(/*Bitu*/int handle, /*Bit32u*/IntRef address) {
        if (InvalidHandle(handle))
            return XMS_INVALID_HANDLE;
    ...
    private static /*Bitu*/int XMS_UnlockMemory(/*Bitu*/int handle) {
        if (InvalidHandle(handle))
            return XMS_INVALID_HANDLE;
    ...
    private static /*Bitu*/int XMS_GetHandleInformation(/*Bitu*/int handle, /*Bit8u*/ShortRef lockCount,
        /*Bit8u*/ShortRef numFree, /*Bit16u*/IntRef size) {
        if (InvalidHandle(handle))
            return XMS_INVALID_HANDLE;
    ...
    private static /*Bitu*/int XMS_ResizeMemory(/*Bitu*/int handle, /*Bitu*/int newSize) {
        if (InvalidHandle(handle))
            return XMS_INVALID_HANDLE;
    ...
    private static void SET_RESULT(/*Bitu*/int res) {
        SET_RESULT(res, true);
    }

    private static void SET_RESULT(/*Bitu*/int res, boolean touch_bl_on_succes/*=true*/) {
