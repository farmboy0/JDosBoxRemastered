...
public class Bios_keyboard {
    ...
    private static /*Bitu*/ int call_int16, call_irq1, call_irq6;
    private static final Scan[] scan_to_scanascii = { new Scan(none, none, none, none),
        ...
        new Scan(none, none, none, none), new Scan(none, none, none, none),
        new Scan(0x565c, 0x567c, none, none), /* (102-key) */
        new Scan(0x8500, 0x8700, 0x8900, 0x8b00), /* F11 */
        new Scan(0x8600, 0x8800, 0x8a00, 0x8c00) /* F12 */
    };
    private static final Callback.Handler IRQ1_Handler = new Callback.Handler() {
        @Override
        ...
        public /*Bitu*/int call() {
             ...
             */
            /*Bitu*/
            int scancode = CPU_Regs.reg_eax.low(); /* Read the code */

            /*Bit8u*/
            int flags1, flags2, flags3, leds;
            ...
            switch (scancode) {
                ...
                case 0x45:
                    if ((flags3 & 0x01) != 0) {
                        ...
                        if ((flags2 & 1) != 0) {
                            ...
                            while ((Memory.mem_readb(Bios.BIOS_KEYBOARD_FLAGS2) & 8) != 0)
                                Callback.CALLBACK_Idle(); // pause loop
                ...
                default: /* Normal Key */
                    ...
                    /* Handle the actual scancode */
                     //goto irq1_end;
                    if (((scancode & 0x80) != 0) || (scancode > Bios.MAX_SCAN_CODE))
                        break; //goto irq1_end;
    ...
    private static final Callback.Handler INT16_Handler = new Callback.Handler() {
        @Override
        public String getName() {
        ...
        public /*Bitu*/int call() {
            ...
            switch (CPU_Regs.reg_eax.high()) {
                ...
                case 0x01: /* CHECK FOR KEYSTROKE */
                    ...
                    Memory.mem_writew(CPU_Regs.reg_ssPhys.dword + CPU_Regs.reg_esp.word() + 4,
                        Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + CPU_Regs.reg_esp.word() + 4) | CPU_Regs.IF);
                    for (;;) {
                        if (check_key(temp)) {
                    ...
                    if (CPU_Regs.reg_eax.low() == 0x00) { // set default delay and rate
                        ...
                        if (Log.level <= LogSeverities.LOG_ERROR)
                            Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR,
                                "INT16:Unhandled Typematic Rate Call " + Integer.toString(CPU_Regs.reg_eax.low(), 16)
                                    + " BX=" + Integer.toString(CPU_Regs.reg_ebx.word(), 16));
                    }
                    break;
                case 0x05: /* STORE KEYSTROKE IN KEYBOARD BUFFER */
                    if (BIOS_AddKeyToBuffer(CPU_Regs.reg_ecx.word()))
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR,
                            "INT16:Unhandled call " + Integer.toString(CPU_Regs.reg_eax.high(), 16));
                    break;
    ...
    public static boolean BIOS_AddKeyToBuffer(/*Bit16u*/int code) {
        synchronized (Bios_keyboard.lock) {
            if ((Memory.mem_readb(Bios.BIOS_KEYBOARD_FLAGS2) & 8) != 0)
                return true;
            /*Bit16u*/
    ...
    */
    ...
    private static boolean IsEnhancedKey(/*Bit16u*/IntRef key) {
    ...
    public static void BIOS_SetupKeyboard() {
        ...
        Callback.CALLBACK_Setup(call_irq1, IRQ1_Handler, Callback.CB_IRQ1,
