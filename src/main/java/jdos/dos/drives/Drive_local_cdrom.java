...
public class Drive_local_cdrom extends Drive_local {
    ...
    public Drive_local_cdrom(char driveLetter, String startdir, /*Bit16u*/int _bytes_sector,
        /*Bit8u*/short _sectors_cluster, /*Bit16u*/int _total_clusters, /*Bit16u*/int _free_clusters,
        /*Bit8u*/short _mediaid, IntRef error) {
        ...
        if (DosMSCDEX.MSCDEX_GetVolumeName(subUnit, name))
            dirCache.SetLabel(name.value, true, true);
    }
    ...
    public DOS_File FileOpen(String name, /*Bit32u*/int flags) {
        ...
        if (result != null)
            ((localFile) result).FlagReadOnlyMedium();
        return result;
    }
    ...
    public boolean GetFileAttr(String name, /*Bit16u*/IntRef attr) {
        ...
        if (result)
            attr.value |= Dos_system.DOS_ATTR_READ_ONLY;
        return result;
    }
