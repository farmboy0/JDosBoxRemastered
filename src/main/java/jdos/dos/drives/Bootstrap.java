...
public class Bootstrap {
    ...
    public void load(byte[] d) {
        ...
        Ptr p = new Ptr(d, 11);
        bytespersector = p.readw(0);
        p.inc(2);
        sectorspercluster = p.readb(0);
        p.inc(1);
        reservedsectors = p.readw(0);
        p.inc(2);
        fatcopies = p.readb(0);
        p.inc(1);
        rootdirentries = p.readw(0);
        p.inc(2);
        totalsectorcount = p.readw(0);
        p.inc(2);
        mediadescriptor = p.readb(0);
        p.inc(1);
        sectorsperfat = p.readw(0);
        p.inc(2);
        sectorspertrack = p.readw(0);
        p.inc(2);
        headcount = p.readw(0);
        p.inc(2);
        hiddensectorcount = p.readd(0);
        p.inc(4);
        totalsecdword = p.readd(0);
        p.inc(4);
        p.read(bootcode);
        p.inc(bootcode.length);
