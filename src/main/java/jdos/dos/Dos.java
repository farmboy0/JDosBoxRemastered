...
public class Dos extends Module_base {
    public static final int DOS_INFOBLOCK_SEG = 0x80; // sysvars (list of lists)
    public static final int DOS_CONDRV_SEG = 0xa0;
    public static final int DOS_CONSTRING_SEG = 0xa8;
    public static final int DOS_SDA_SEG = 0xb2; // dos swappable area
    public static final int DOS_SDA_OFS = 0;
    public static final int DOS_CDS_SEG = 0x108;
    public static final int DOS_FIRST_SHELL = 0x118;
    public static final int DOS_MEM_START = 0x16f; //First Segment that DOS can use

    /* Dos Error Codes */
    public static final int DOSERR_NONE = 0;
    public static final int DOSERR_FUNCTION_NUMBER_INVALID = 1;
    public static final int DOSERR_FILE_NOT_FOUND = 2;
    public static final int DOSERR_PATH_NOT_FOUND = 3;
    public static final int DOSERR_TOO_MANY_OPEN_FILES = 4;
    public static final int DOSERR_ACCESS_DENIED = 5;
    public static final int DOSERR_INVALID_HANDLE = 6;
    public static final int DOSERR_MCB_DESTROYED = 7;
    public static final int DOSERR_INSUFFICIENT_MEMORY = 8;
    public static final int DOSERR_MB_ADDRESS_INVALID = 9;
    public static final int DOSERR_ENVIRONMENT_INVALID = 10;
    public static final int DOSERR_FORMAT_INVALID = 11;
    public static final int DOSERR_ACCESS_CODE_INVALID = 12;
    public static final int DOSERR_DATA_INVALID = 13;
    public static final int DOSERR_RESERVED = 14;
    public static final int DOSERR_FIXUP_OVERFLOW = 14;
    public static final int DOSERR_INVALID_DRIVE = 15;
    public static final int DOSERR_REMOVE_CURRENT_DIRECTORY = 16;
    public static final int DOSERR_NOT_SAME_DEVICE = 17;
    public static final int DOSERR_NO_MORE_FILES = 18;
    public static final int DOSERR_FILE_ALREADY_EXISTS = 80;
    static final byte[] DOS_DATE_months = { 0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
    private static final int DOS_COPYBUFSIZE = 0x10000;
    private static final boolean DATA_TRANSFERS_TAKE_CYCLES = true;
    private static final boolean DOS_OVERHEAD = true;
    public static Dos_Block dos = new Dos_Block();
    public static Dos_InfoBlock dos_infoblock = new Dos_InfoBlock();
    static byte[] dos_copybuf = new byte[DOS_COPYBUFSIZE];
    private static final Callback.Handler DOS_21Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            if (CPU_Regs.reg_eax.high() != 0x50 && CPU_Regs.reg_eax.high() != 0x51 && CPU_Regs.reg_eax.high() != 0x62
                && CPU_Regs.reg_eax.high() != 0x64 && CPU_Regs.reg_eax.high() < 0x6c) {
            ...
            switch (CPU_Regs.reg_eax.high() & 0xFF) {
                case 0x00: /* Terminate Program */
                    Dos_execute.DOS_Terminate(Memory.mem_readw(CPU_Regs.reg_ssPhys.dword + CPU_Regs.reg_esp.word() + 2),
                        false, 0);
                    break;
                ...
                case 0x02: /* Write character to STDOUT */ {
                    /*Bit8u*/
                    byte[] c = { (byte) CPU_Regs.reg_edx.low() };/*Bit16u*/
                    IntRef n = new IntRef(1);
                ...
                case 0x09: /* Write string to STDOUT */ {
                    ...
                    IntRef n = new IntRef(1);
                    /*PhysPt*/
                    int buf = CPU_Regs.reg_dsPhys.dword + CPU_Regs.reg_edx.word();
                ...
                case 0x0a: /* Buffered Input */ {
                    ...
                    /*PhysPt*/
                    int data = CPU_Regs.reg_dsPhys.dword + CPU_Regs.reg_edx.word();
                    /*Bit8u*/
                    int free = Memory.mem_readb(data);
                    /*Bit8u*/
                    short read = 0;/*Bit8u*/
                    byte[] c = new byte[1];/*Bit16u*/
                    IntRef n = new IntRef(1);
                ...
                case 0x0c: /* Flush Buffer and read STDIN call */ {
                    ...
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        case 0xa: {
                            /*Bit8u*/
                            int oldah = CPU_Regs.reg_eax.high();
                ...
                case 0x0f: /* Open File using FCB */
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x0f FCB-fileopen used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                case 0x10: /* Close File using FCB */
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x10 FCB-fileclose used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                case 0x11: /* Find First Matching File using FCB */
                    if (Dos_files.DOS_FCBFindFirst(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word()))
                        CPU_Regs.reg_eax.low(0x00);
                    else
                        CPU_Regs.reg_eax.low(0xFF);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x11 FCB-FindFirst used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                case 0x12: /* Find Next Matching File using FCB */
                    if (Dos_files.DOS_FCBFindNext(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word()))
                        CPU_Regs.reg_eax.low(0x00);
                    else
                        CPU_Regs.reg_eax.low(0xFF);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x12 FCB-FindNext used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                case 0x13: /* Delete File using FCB */
                    if (Dos_files.DOS_FCBDeleteFile(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word()))
                        CPU_Regs.reg_eax.low(0x00);
                    else
                        CPU_Regs.reg_eax.low(0xFF);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x16 FCB-Delete used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                case 0x14: /* Sequential read from FCB */
                    CPU_Regs.reg_eax.low(Dos_files.DOS_FCBRead(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word(), 0));
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x14 FCB-Read used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                case 0x15: /* Sequential write to FCB */
                    CPU_Regs.reg_eax.low(Dos_files.DOS_FCBWrite(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word(), 0));
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x15 FCB-Write used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                case 0x16: /* Create or truncate file using FCB */
                    if (Dos_files.DOS_FCBCreate(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word()))
                        CPU_Regs.reg_eax.low(0x00);
                    else
                        CPU_Regs.reg_eax.low(0xFF);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x16 FCB-Create used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                case 0x17: /* Rename file using FCB */
                    if (Dos_files.DOS_FCBRenameFile(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word()))
                        CPU_Regs.reg_eax.low(0x00);
                    else
                        CPU_Regs.reg_eax.low(0xFF);
                    break;
                ...
                case 0x21: /* Read random record from FCB */ {
                    ...
                    CPU_Regs.reg_eax.low(
                        Dos_files.DOS_FCBRandomRead(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word(), toRead, true));
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x21 FCB-Random read used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                }
                case 0x22: /* Write random record to FCB */ {
                    IntRef toWrite = new IntRef(1);
                    CPU_Regs.reg_eax.low(
                        Dos_files.DOS_FCBRandomWrite(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word(), toWrite, true));
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x22 FCB-Random write used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                }
                case 0x23: /* Get file size for FCB */
                    if (Dos_files.DOS_FCBGetFileSize(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word()))
                        CPU_Regs.reg_eax.low(0x00);
                    else
                        CPU_Regs.reg_eax.low(0xFF);
                    break;
                ...
                case 0x27: /* Random block read from FCB */ {
                    ...
                    CPU_Regs.reg_eax.low(
                        Dos_files.DOS_FCBRandomRead(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word(), toRead, false));
                    CPU_Regs.reg_ecx.word(toRead.value);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x27 FCB-Random(block) read used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                }
                case 0x28: /* Random Block write to FCB */ {
                    IntRef toWrite = new IntRef(CPU_Regs.reg_ecx.word());
                    CPU_Regs.reg_eax.low(Dos_files.DOS_FCBRandomWrite(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word(),
                        toWrite, false));
                    CPU_Regs.reg_ecx.word(toWrite.value);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:0x28 FCB-Random(block) write used, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                }
                case 0x29: /* Parse filename into FCB */ {
                    ...
                    CPU_Regs.reg_eax.low(Dos_files.FCB_Parsename(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_edi.word(),
                        (short) CPU_Regs.reg_eax.low(), string, difference));
                    CPU_Regs.reg_esi.word(CPU_Regs.reg_esi.word() + difference.value);
                }
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_FCB, LogSeverities.LOG_NORMAL,
                            "DOS:29:FCB Parse Filename, result:al=" + CPU_Regs.reg_eax.low());
                    break;
                ...
                case 0x2a: /* Get System Date */ {
                    ...
                    if (CPU_Regs.reg_eax.low() != 0)
                        DOS_AddDays((short) CPU_Regs.reg_eax.low());
                ...
                case 0x2b: /* Set System Date */
                    ...
                    if (CPU_Regs.reg_edx.low() > DOS_DATE_months[CPU_Regs.reg_edx.high()]) {
                        if (!(CPU_Regs.reg_edx.high() == 2 && CPU_Regs.reg_ecx.word() % 4 == 0
                            && CPU_Regs.reg_edx.low() == 29)) // february pass
                        {
                            CPU_Regs.reg_eax.low(0xff);
                            break;
                        }
                ...
                case 0x2c: /* Get System Time */
                    ...
                    /*Bitu*/
                    long ticks = (long) CPU_Regs.reg_ecx.word() << 16 | CPU_Regs.reg_edx.word();
                    if (time_start <= ticks)
                        ticks -= time_start;
                    /*Bitu*/
                    long time = (long) (100.0 / (Timer.PIT_TICK_RATE / 65536.0) * ticks);
                ...
                case 0x2d: /* Set System Time */
                    ...
                    if (CPU_Regs.reg_ecx.high() > 23 || CPU_Regs.reg_ecx.low() > 59 || CPU_Regs.reg_edx.high() > 59
                        || CPU_Regs.reg_edx.low() > 99)
                        CPU_Regs.reg_eax.low(0xff);
                    else { //Allow time to be set to zero. Restore the orginal time for all other parameters. (QuickBasic)
                        if (CPU_Regs.reg_ecx.word() == 0 && CPU_Regs.reg_edx.word() == 0) {
                            time_start = Memory.mem_readd(Bios.BIOS_TIMER) & 0xFFFFFFFFL;
                            Log.log_msg("Warning: game messes with DOS time!");
                        } else
                            time_start = 0;
                        CPU_Regs.reg_eax.low(0);
                    }
                ...
                case 0x30: /* Get DOS Version */
                    if (CPU_Regs.reg_eax.low() == 0)
                        CPU_Regs.reg_ebx.high(0xFF); /* Fake Microsoft DOS */
                    if (CPU_Regs.reg_eax.low() == 1)
                        CPU_Regs.reg_ebx.high(0x10); /* DOS is in HMA */
                ...
                case 0x32: /* Get drive parameter block for specific drive */ { /* Officially a dpb should be returned as well. The disk detection part is implemented */
                    /*Bit8u*/
                    int drive = CPU_Regs.reg_edx.low();
                    if (drive == 0 || CPU_Regs.reg_eax.high() == 0x1f)
                        drive = Dos_files.DOS_GetDefaultDrive();
                    else
                        drive--;
                ...
                case 0x33: /* Extended Break Checking */
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        case 4: /* Set cpsw */
                            if (Log.level <= LogSeverities.LOG_ERROR)
                                Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,
                                    "Someone playing with cpsw " + Integer.toString(CPU_Regs.reg_eax.word(), 16));
                            break;
                        case 5:
                        ...
                        default:
                            if (Log.level <= LogSeverities.LOG_ERROR)
                                Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,
                                    "Weird 0x33 call " + Integer.toString(CPU_Regs.reg_eax.low(), 16));
                            CPU_Regs.reg_eax.low(0xff);
                            break;
                    }
                    break;
                ...
                case 0x36: /* Get Free Disk Space */ {
                    ...
                    if (Dos_files.DOS_GetFreeDiskSpace((short) CPU_Regs.reg_edx.low(), bytes, sectors, clusters,
                        free)) {
                ...
                case 0x38: /* Set Country Code */
                    if (CPU_Regs.reg_eax.low() == 0) { /* Get country specidic information */
                        /*PhysPt*/
                        int dest = CPU_Regs.reg_dsPhys.dword + CPU_Regs.reg_edx.word();
                ...
                case 0x3a: /* RMDIR Remove directory */ {
                    ...
                    if (Dos_files.DOS_RemoveDir(name1)) {
                        ...
                        Callback.CALLBACK_SCF(true);
                        if (Log.level <= LogSeverities.LOG_NORMAL)
                            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                                "Remove dir failed on " + name1 + " with error " + dos.errorcode);
                    }
                    break;
                }
                ...
                case 0x40: /* WRITE Write to file or device */ {
                    /*Bit16u*/
                    IntRef towrite = new IntRef(CPU_Regs.reg_ecx.word());
                    Memory.MEM_BlockRead(CPU_Regs.reg_dsPhys.dword + CPU_Regs.reg_edx.word(), dos_copybuf,
                        towrite.value);
                ...
                case 0x43: /* Get/Set file attributes */ {
                    ...
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        case 0x01: /* Set */
                            if (Log.level <= LogSeverities.LOG_ERROR)
                                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                                    "DOS:Set File Attributes for " + name1 + " not supported");
                        ...
                        default:
                            if (Log.level <= LogSeverities.LOG_ERROR)
                                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                                    "DOS:0x43:Illegal subfunction " + Integer.toString(CPU_Regs.reg_eax.low(), 16));
                ...
                case 0x47: /* CWD Get current directory */ {
                    ...
                    if (Dos_files.DOS_GetCurrentDir((short) CPU_Regs.reg_edx.low(), name1)) {
                        /*Bitu*/
                        Memory.MEM_BlockWrite(CPU_Regs.reg_dsPhys.dword + CPU_Regs.reg_esi.word(), name1.value,
                            name1.value.length() + 1);
                ...
                case 0x48: /* Allocate memory */ {
                    /*Bit16u*/
                    IntRef size = new IntRef(CPU_Regs.reg_ebx.word());/*Bit16u*/
                ...
                case 0x4b: /* EXEC Load and/or execute program */ {
                    String name1 = Memory.MEM_StrCopy(CPU_Regs.reg_dsPhys.dword + CPU_Regs.reg_edx.word(), 256);
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_EXEC, LogSeverities.LOG_ERROR,
                            "Execute " + name1 + " " + CPU_Regs.reg_eax.low());
                    if (!Dos_execute.DOS_Execute(name1, CPU_Regs.reg_esPhys.dword + CPU_Regs.reg_ebx.word(),
                        (short) CPU_Regs.reg_eax.low())) {
                        CPU_Regs.reg_eax.word(dos.errorcode);
                        Callback.CALLBACK_SCF(true);
                    }
                }
                ...
                case 0x52: { /* Get list of lists */
                    ...
                    Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_NORMAL,
                ...
                case 0x57: /* Get/Set File's Date and Time */
                    if (CPU_Regs.reg_eax.low() == 0x00) {
                        ...
                        if (Log.level <= LogSeverities.LOG_ERROR)
                            Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,
                                "DOS:57:Unsupported subtion " + Integer.toString(CPU_Regs.reg_eax.low(), 16));
                    }
                    break;
                case 0x58: /* Get/Set Memory allocation strategy */
                    switch (CPU_Regs.reg_eax.low()) {
                        case 0: /* Get Strategy */
                            ...
                            break;
                        case 1: /* Set Strategy */
                            if (Dos_memory.DOS_SetMemAllocStrategy(CPU_Regs.reg_ebx.word()))
                                Callback.CALLBACK_SCF(false);
                            else {
                                CPU_Regs.reg_eax.word(1);
                                Callback.CALLBACK_SCF(true);
                        ...
                        case 3: /* Set UMB Link Status */
                            if (Dos_memory.DOS_LinkUMBsToMemChain(CPU_Regs.reg_ebx.word()))
                                Callback.CALLBACK_SCF(false);
                            else {
                                CPU_Regs.reg_eax.word(1);
                                Callback.CALLBACK_SCF(true);
                        ...
                        default:
                            if (Log.level <= LogSeverities.LOG_ERROR)
                                Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,
                                    "DOS:58:Not Supported Set//Get memory allocation call "
                                        + Integer.toString(CPU_Regs.reg_eax.low(), 16));
                            CPU_Regs.reg_eax.word(1);
                            Callback.CALLBACK_SCF(true);
                    }
                    break;
                ...
                case 0x5a: /* Create temporary file */ {
                    ...
                    StringRef name1 = new StringRef(
                        Memory.MEM_StrCopy(CPU_Regs.reg_dsPhys.dword + CPU_Regs.reg_edx.word(), 256));
                    if (Dos_files.DOS_CreateTempFile(name1, handle)) {
                        CPU_Regs.reg_eax.word(handle.value);
                        /*Bitu*/
                        Memory.MEM_BlockWrite(CPU_Regs.reg_dsPhys.dword + CPU_Regs.reg_edx.word(), name1.value,
                            name1.value.length() + 1);
                        Callback.CALLBACK_SCF(false);
                    } else {
                        CPU_Regs.reg_eax.word(dos.errorcode);
                ...
                case 0x60: /* Canonicalize filename or path */ {
                    ...
                    if (Dos_files.DOS_Canonicalize(name1, name2)) {
                        /*Bitu*/
                        Memory.MEM_BlockWrite(CPU_Regs.reg_esPhys.dword + CPU_Regs.reg_edi.word(), name2.value,
                            name2.value.length() + 1);
                        Callback.CALLBACK_SCF(false);
                    } else {
                        CPU_Regs.reg_eax.word(dos.errorcode);
                        Callback.CALLBACK_SCF(true);
                ...
                case 0x63: /* DOUBLE BYTE CHARACTER SET */
                    if (CPU_Regs.reg_eax.low() == 0) {
                        ...
                        CPU_Regs.reg_eax.low(0xff); //Doesn't officially touch carry flag
                    break;
                case 0x64: /* Set device driver lookahead flag */
                    ...
                    break;
                case 0x65: /* Get extented country information and a lot of other useless shit*/ { /* Todo maybe fully support this for now we set it standard for USA */
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,
                            "DOS:65:Extended country information call "
                                + Integer.toString(CPU_Regs.reg_eax.word(), 16));
                    ...
                    /*PhysPt*/
                    int data = CPU_Regs.reg_esPhys.dword + CPU_Regs.reg_edi.word();
                    switch (CPU_Regs.reg_eax.low()) {
                        case 0x01:
                            Memory.mem_writeb(data + 0x00, CPU_Regs.reg_eax.low());
                            Memory.mem_writew(data + 0x01, 0x26);
                            Memory.mem_writew(data + 0x03, 1);
                            if (CPU_Regs.reg_ecx.word() > 0x06)
                                Memory.mem_writew(data + 0x05, dos.loaded_codepage);
                        ...
                        case 0x22: /* Capatilize ASCIZ string */
                            ...
                            if (CPU_Regs.reg_eax.low() == 0x21)
                                len = CPU_Regs.reg_ecx.word();
                            else
                                len = Memory.mem_strlen(data); /* Is limited to 1024 */

                            if (len > DOS_COPYBUFSIZE - 1)
                                Log.exit("DOS:0x65 Buffer overflow");
                            if (len > 0) {
                                Memory.MEM_BlockRead(data, dos_copybuf, len);
                                System.arraycopy(new String(dos_copybuf, 0, len).toUpperCase().getBytes(), 0,
                                    dos_copybuf, 0, len);
                            ...
                            break;
                        default:
                            Log.exit("DOS:0x65:Unhandled country information call "
                                + Integer.toString(CPU_Regs.reg_eax.low(), 16));
                    }
                    break;
                }
                ...
                case 0x69: /* Get/Set disk serial number */ {
                    switch (CPU_Regs.reg_eax.low()) {
                        ...
                        default:
                            Log.exit(
                                "DOS:Illegal Get Serial Number call " + Integer.toString(CPU_Regs.reg_eax.low(), 16));
                    }
                    break;
                }
                case 0x6c: /* Extended Open/Create */ {
                    ...
                    IntRef ax = new IntRef(CPU_Regs.reg_eax.word());
                    IntRef cx = new IntRef(CPU_Regs.reg_ecx.word());
                    if (Dos_files.DOS_OpenFileExtended(name1, CPU_Regs.reg_ebx.word(), CPU_Regs.reg_ecx.word(),
                        CPU_Regs.reg_edx.word(), ax, cx)) {
                        CPU_Regs.reg_eax.word(ax.value);
                        CPU_Regs.reg_ecx.word(cx.value);
                        Callback.CALLBACK_SCF(false);
                    } else {
                ...
                case 0x71: /* Unknown probably 4dos detection */
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_NORMAL,
                            "DOS:Windows long file name support call " + Integer.toString(CPU_Regs.reg_eax.low(), 16));
                    break;
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,
                            "DOS:Unhandled call " + Integer.toString(CPU_Regs.reg_eax.high(), 16) + " al="
                                + Integer.toString(CPU_Regs.reg_eax.low(), 16) + ". Set al to default of 0");
    ...
    private static final Callback.Handler DOS_20Handler = new Callback.Handler() {
        @Override
        public String getName() {
    ...
    private static final Callback.Handler DOS_27Handler = new Callback.Handler() {
        @Override
        public String getName() {
        ...
        public /*Bitu*/int call() {
            ...
            /*Bit16u*/
            IntRef para = new IntRef(CPU_Regs.reg_edx.word() / 16 + (CPU_Regs.reg_edx.word() % 16 > 0 ? 1 : 0));
            /*Bit16u*/
            int psp = dos.psp(); //mem_readw(CPU_Regs.SegPhys(ss)+CPU_Regs.reg_esp.word()+2);
            if (Dos_memory.DOS_ResizeMemory(psp, para))
                Dos_execute.DOS_Terminate(psp, true, 0);
            return Callback.CBRET_NONE;
        }
    };
    private static final Callback.Handler DOS_25Handler = new Callback.Handler() {
        @Override
        public String getName() {
        ...
        public /*Bitu*/int call() {
            ...
            if (drive >= Dos_files.Drives.length || Dos_files.Drives[CPU_Regs.reg_eax.low()] == null) {
                ...
                if (CPU_Regs.reg_ecx.word() != 1 || CPU_Regs.reg_edx.word() != 1)
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_NORMAL,
                            "int 25 called but not as diskdetection drive "
                                + Integer.toString(CPU_Regs.reg_eax.low(), 16));
    ...
    private static final Callback.Handler DOS_26Handler = new Callback.Handler() {
        @Override
        public String getName() {
    ...
    public static Section.SectionFunction DOS_ShutDown = section -> {
        test = null;
    ...
    public static Section.SectionFunction DOS_Init = section -> {
    ...
    static void DOS_AddDays(short days) {
        dos.date.day += days;
        /*Bit8u*/
        byte monthlimit = DOS_DATE_months[dos.date.month];
    ...
    public static /*Bit16u*/int DOS_PackTime(/*Bit16u*/int hour, /*Bit16u*/int min, /*Bit16u*/int sec) {
        return (hour & 0x1f) << 11 | (min & 0x3f) << 5 | sec / 2 & 0x1f;
    }

    public static /*Bit16u*/int DOS_PackDate(/*Bit16u*/int year, /*Bit16u*/int mon, /*Bit16u*/int day) {
        return (year - 1980 & 0x7f) << 9 | (mon & 0x3f) << 5 | day & 0x1f;
    }

    public static /*Bit8u*/int RealHandle(/*Bit16u*/int handle) {
        Dos_PSP psp = new Dos_PSP(dos.psp());
        return psp.GetFileHandle(handle);
    }

    private static void modify_cycles(/*Bits*/int value) {
