...
public class Dos_misc {
    public static Vector Multiplex;
    static Dos_system.MultiplexHandler DOS_MultiplexFunctions = () -> {
        switch (CPU_Regs.reg_eax.word()) {
            case 0x1216: /* GET ADDRESS OF SYSTEM FILE TABLE ENTRY */
                ...
                if (Log.level <= LogSeverities.LOG_ERROR)
                    Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,
                        "Some BAD filetable call used bx=" + Integer.toString(CPU_Regs.reg_ebx.word(), 16));
                Callback.CALLBACK_SCF(CPU_Regs.reg_ebx.word() > Dos_files.DOS_FILES);
                if (CPU_Regs.reg_ebx.word() < 16) {
                    /*RealPt*/
                    int sftrealpt = Memory.mem_readd(Memory.Real2Phys(Dos.dos_infoblock.GetPointer()) + 4);
                    /*PhysPt*/
                    int sftptr = Memory.Real2Phys(sftrealpt);
                    /*Bitu*/
                    int sftofs = 0x06 + CPU_Regs.reg_ebx.word() * 0x3b;

                    if (Dos_files.Files[CPU_Regs.reg_ebx.word()] != null)
                        Memory.mem_writeb(sftptr + sftofs, Dos_files.Files[CPU_Regs.reg_ebx.word()].refCtr);
                    else
                        Memory.mem_writeb(sftptr + sftofs, 0);

                    if (Dos_files.Files[CPU_Regs.reg_ebx.word()] == null)
                        return true;

                    /*Bit32u*/
                    long handle = Dos.RealHandle(CPU_Regs.reg_ebx.word());
                    if (handle >= Dos_files.DOS_FILES) {
                        ...
                        Memory.mem_writew(sftptr + sftofs + 0x05,
                            Dos_files.Files[CPU_Regs.reg_ebx.word()].GetInformation()); // device info word
                        ...
                        /*Bit8u*/
                        short drive = Dos_files.Files[CPU_Regs.reg_ebx.word()].GetDrive();

                        Memory.mem_writew(sftptr + sftofs + 0x02,
                            (/*Bit16u*/int) (Dos_files.Files[CPU_Regs.reg_ebx.word()].flags & 3)); // file open mode
                        Memory.mem_writeb(sftptr + sftofs + 0x04,
                            (/*Bit8u*/short) Dos_files.Files[CPU_Regs.reg_ebx.word()].attr); // file attribute
            ...
            case 0x1607:
                if (CPU_Regs.reg_ebx.word() == 0x15) {
                    ...
                    return CPU_Regs.reg_ebx.word() == 0x18; // idle callout
    ...
    private static final Callback.Handler INT2F_Handler = new Callback.Handler() {
        @Override
        ...
        public /*Bitu*/int call() {
            ...
            if (Log.level <= LogSeverities.LOG_ERROR)
                Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,
                    "DOS:Multiplex Unhandled call " + Integer.toString(CPU_Regs.reg_eax.word(), 16));
            return Callback.CBRET_NONE;
        }
    };
    private static final Callback.Handler INT2A_Handler = new Callback.Handler() {
        @Override
        public String getName() {
    ...
    private static /*Bitu*/ int call_int2f, call_int2a;

    public static void DOS_AddMultiplexHandler(Dos_system.MultiplexHandler handler) {
        Multiplex.add(handler);
    }

    public static void DOS_DelMultiplexHandler(Dos_system.MultiplexHandler handler) {
        Multiplex.remove(handler);
    }

    public static void DOS_SetupMisc() {
