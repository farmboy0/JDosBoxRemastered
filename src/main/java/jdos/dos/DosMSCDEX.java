...
public class DosMSCDEX {
    ...
    public static Section.SectionFunction MSCDEX_ShutDown = section -> {
        mscdex = null;
        curReqheaderPtr = 0;
    };
    private static final Callback.Handler MSCDEX_Strategy_Handler = new Callback.Handler() {
        @Override
    ...
    private static final Callback.Handler MSCDEX_Interrupt_Handler = new Callback.Handler() {
        @Override
        public String getName() {
        ...
        public /*Bitu*/int call() {
            ...
            /*Bit8u*/
            short subUnit = (short) Memory.mem_readb(curReqheaderPtr + 1);
            /*Bit8u*/
            short funcNr = (short) Memory.mem_readb(curReqheaderPtr + 2);
            /*Bit16u*/
            int errcode = 0;
            /*PhysPt*/
            int buffer = 0;
            ...
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                    "MSCDEX: Driver Function " + Integer.toString(funcNr, 16));

            if (funcNr == 0x03 || funcNr == 0x0c || funcNr == 0x80 || funcNr == 0x82) {
                buffer = Memory.PhysMake(Memory.mem_readw(curReqheaderPtr + 0x10),
                    Memory.mem_readw(curReqheaderPtr + 0x0E));
            }
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                            "MSCDEX: Unsupported Driver Request " + Integer.toString(funcNr, 16));
                    break;
            ...
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                    "MSCDEX: Status : " + Integer.toString(Memory.mem_readw(curReqheaderPtr + 3), 16));
            return Callback.CBRET_NONE;
        }
    };
    private static final Dos_system.MultiplexHandler MSCDEX_Handler = () -> {
        if (CPU_Regs.reg_eax.high() == 0x11) {
            if (CPU_Regs.reg_eax.low() == 0x00) {
                /*PhysPt*/
                int check = Memory.PhysMake(CPU_Regs.reg_ssVal.dword, CPU_Regs.reg_esp.word());
        ...
        if (CPU_Regs.reg_eax.high() != 0x15)
            return false; // not handled here, continue chain

        /*PhysPt*/
        int data = Memory.PhysMake(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_ebx.word());
        if (Log.level <= LogSeverities.LOG_NORMAL)
            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                "MSCDEX: INT 2F " + Integer.toString(CPU_Regs.reg_eax.word(), 16) + " BX= "
                    + Integer.toString(CPU_Regs.reg_ebx.word(), 16) + " CX="
                    + Integer.toString(CPU_Regs.reg_ecx.word(), 16));
        switch (CPU_Regs.reg_eax.word()) {
            ...
            case 0x150F: { // Get directory entry
                /*Bit16u*/
                IntRef error = new IntRef(0);
                boolean success = mscdex.GetDirectoryEntry(CPU_Regs.reg_ecx.low(),
                    (CPU_Regs.reg_ecx.high() & 1) != 0, data,
                    Memory.PhysMake(CPU_Regs.reg_esi.word(), CPU_Regs.reg_edi.word()), error);
                CPU_Regs.reg_eax.word(error.value);
        ...
        if (Log.level <= LogSeverities.LOG_ERROR)
            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                "MSCDEX: Unknwon call : " + Integer.toString(CPU_Regs.reg_eax.word(), 16));
        return true;
    };
    public static Section.SectionFunction MSCDEX_Init = section -> {
        // AddDestroy func
    ...
    private static final Dos_cdrom.TMSF[] leadOut = new Dos_cdrom.TMSF[MSCDEX_MAX_DRIVES];
    ...
    private static /*Bit16u*/int MSCDEX_IOCTL_Input(/*PhysPt*/int buffer, /*Bit8u*/short drive_unit) {
        ...
        if (Log.level <= LogSeverities.LOG_NORMAL)
            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                "MSCDEX: IOCTL INPUT Subfunction " + Integer.toString(ioctl_fct, 16));
        switch (ioctl_fct) {
            ...
            case 0x01: {/* Get current position */
                ...
                /*Bit8u*/
                int addr_mode = Memory.mem_readb(buffer + 1);
                if (addr_mode == 0) { // HSG
                    /*Bit32u*/
                    ...
                    if (frames < 150)
                        if (Log.level <= LogSeverities.LOG_ERROR)
                            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                                "MSCDEX: Get position: invalid position " + pos.min + ":" + pos.sec + ":" + pos.fr);
                        else
                            frames -= 150;
                    ...
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                            "MSCDEX: Get position: invalid address mode " + Integer.toString(addr_mode, 16));
            ...
            case 0x07: /* Get sector size */
                ...
                else if (Memory.mem_readb(buffer + 1) == 1)
            ...
            case 0x0B: {/* Audio Track Info */
                ...
                /*Bit8u*/
                int track = Memory.mem_readb(buffer + 1);
            ...
            default:
                if (Log.level <= LogSeverities.LOG_ERROR)
                    Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                        "MSCDEX: Unsupported IOCTL INPUT Subfunction " + Integer.toString(ioctl_fct, 16));
                return 0x03; // invalid function
        }
        return 0x00; // success
    }
    ...
    private static /*Bit16u*/int MSCDEX_IOCTL_Optput(/*PhysPt*/int buffer, /*Bit8u*/short drive_unit) {
        ...
        switch (ioctl_fct) {
            case 0x00: // Unload /eject media
                if (!mscdex.LoadUnloadMedia(drive_unit, true))
                    return 0x02;
                break;
            case 0x03: //Audio Channel control
                ...
                if (!mscdex.ChannelControl(drive_unit, ctrl))
                    return 0x01;
                break;
            ...
            case 0x02: // Reset Drive
                ...
                if (!mscdex.StopAudio(drive_unit))
                    return 0x02;
                break;
            ...
            default:
                if (Log.level <= LogSeverities.LOG_ERROR)
                    Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                        "MSCDEX: Unsupported IOCTL OUTPUT Subfunction " + Integer.toString(ioctl_fct, 16));
                return 0x03; // invalid function
        }
        return 0x00; // success
    }

    public static int MSCDEX_AddDrive(char driveLetter, String physicalPath, /*Bit8u*/ShortRef subUnit) {
    ...
    public static void MSCDEX_ReplaceDrive(Dos_cdrom.CDROM_Interface cdrom, /*Bit8u*/short subUnit) {
        mscdex.ReplaceDrive(cdrom, subUnit);
    }

    public static boolean MSCDEX_GetVolumeName(/*Bit8u*/short subUnit, StringRef name) {
        return mscdex.GetVolumeName(subUnit, name);
    }

    public static boolean MSCDEX_HasMediaChanged(/*Bit8u*/short subUnit) {
        Dos_cdrom.TMSF leadnew = new Dos_cdrom.TMSF();
        /*Bit8u*/
        ShortRef tr1 = new ShortRef(), tr2 = new ShortRef();
        if (mscdex.GetCDInfo(subUnit, tr1, tr2, leadnew)) {
            boolean changed = leadOut[subUnit].min != leadnew.min || leadOut[subUnit].sec != leadnew.sec
    ...
    private static class DOS_DeviceHeader extends MemStruct {
        ...
        public DOS_DeviceHeader(/*PhysPt*/int ptr) {
            pt = ptr;
        }

        public void SetNextDeviceHeader(/*RealPt*/int ptr) {
            SaveIt(4, 0, ptr);
        } //sSave(sDeviceHeader,nextDeviceHeader,ptr); }

        public /*RealPt*/int GetNextDeviceHeader() {
            return GetIt(4, 0);
        } //sGet(sDeviceHeader,nextDeviceHeader); }

        public void SetAttribute(/*Bit16u*/int atr) {
            SaveIt(2, 4, atr);
        } //sSave(sDeviceHeader,devAttributes,atr); }

        public void SetDriveLetter(/*Bit8u*/int letter) {
            SaveIt(1, 20, letter);
        } //sSave(sDeviceHeader,driveLetter,letter); }

        public void SetNumSubUnits(/*Bit8u*/short num) {
            SaveIt(1, 21, num);
        } //sSave(sDeviceHeader,numSubUnits,num); }

        public /*Bit8u*/short GetNumSubUnits() {
            return (short) GetIt(1, 21);
        } //sGet(sDeviceHeader,numSubUnits); }

        public void SetName(String _name) {
            Memory.MEM_BlockWrite(pt + 10, _name, 8);
        }

        public void SetInterrupt(/*Bit16u*/int ofs) {
            SaveIt(2, 8, ofs);
        } //sSave(sDeviceHeader,interrupt,ofs); }

        public void SetStrategy(/*Bit16u*/int ofs) {
            SaveIt(2, 6, ofs);
        } //sSave(sDeviceHeader,strategy,ofs); }
...
// 22     };
    ...
    private static class CMscdex {
        ...
        int RemoveDrive(/*Bit16u*/int _drive) {
            ...
            if (idx == MSCDEX_MAX_DRIVES || idx != 0 && idx != GetNumDrives() - 1)
                return 0;
            cdrom[idx].close();
            if (idx == 0) {
                for (/*Bit16u*/int i = 0; i < GetNumDrives(); i++) {
        ...
        int AddDrive(/*Bit16u*/int _drive, String physicalPath, /*Bit8u*/ShortRef subUnit) {
            ...
            switch (Dos_cdrom.CDROM_GetMountType(physicalPath, forceCD)) {
                case 0x00: {
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                            "MSCDEX: Mounting physical cdrom not supported: " + physicalPath);
...
//        #endif
                ...
                case 0x01: // iso cdrom interface
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                            "MSCDEX: Mounting iso file as cdrom: " + physicalPath);
                ...
                case 0x02: // fake cdrom interface (directories)
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                            "MSCDEX: Mounting directory as cdrom: " + physicalPath);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                            "MSCDEX: You wont have full MSCDEX support !");
                    result = 5;
                    break;
            ...
            if (rootDriverHeaderSeg == 0) {
                ...
                /*Bit16u*/
                int driverSize = DOS_DeviceHeader.size + 10; // 10 = Bytes for 3 callbacks
                ...
                /*Bit16u*/
                int segm = (/*Bit16u*/int) (start >> 16);
                /*Bit16u*/
                int offm = (/*Bit16u*/int) (start & 0xFFFF);
        ...
        boolean GetCDInfo(/*Bit8u*/short subUnit, /*Bit8u*/ShortRef tr1, /*Bit8u*/ShortRef tr2,
            Dos_cdrom.TMSF leadOut) {
        ...
        boolean GetTrackInfo(/*Bit8u*/short subUnit, /*Bit8u*/short track, /*Bit8u*/ShortRef attr,
            Dos_cdrom.TMSF start) {
            if (subUnit >= numDrives)
                return false;
        ...
        boolean PlayAudioMSF(/*Bit8u*/short subUnit, /*Bit32u*/long start, /*Bit32u*/long length) {
            if (subUnit >= numDrives)
                return false;
            /*Bit8u*/
            short min = (/*Bit8u*/short) (start >> 16 & 0xFF);
            /*Bit8u*/
            short sec = (/*Bit8u*/short) (start >> 8 & 0xFF);
            /*Bit8u*/
            short fr = (/*Bit8u*/short) (start >> 0 & 0xFF);
            /*Bit32u*/
            long sector = min * 60 * 75 + sec * 75 + fr - 150;
            return dinfo[subUnit].lastResult = PlayAudioSector(subUnit, sector, length);
        }

        boolean GetSubChannelData(/*Bit8u*/short subUnit, /*Bit8u*/ShortRef attr, /*Bit8u*/ShortRef track,
            /*Bit8u*/ShortRef index, Dos_cdrom.TMSF rel, Dos_cdrom.TMSF abs) {
            if (subUnit >= numDrives)
                return false;
        ...
        boolean GetAudioStatus(/*Bit8u*/short subUnit, BooleanRef playing, BooleanRef pause, Dos_cdrom.TMSF start,
            Dos_cdrom.TMSF end) {
            if (subUnit >= numDrives)
                return false;
        ...
        boolean ResumeAudio(/*Bit8u*/short subUnit) {
            if (subUnit >= numDrives)
                return false;
            return dinfo[subUnit].lastResult = PlayAudioSector(subUnit, dinfo[subUnit].audioStart,
                dinfo[subUnit].audioEnd);
        }
        ...
        boolean ReadVTOC(/*Bit16u*/int drive, /*Bit16u*/int volume, /*PhysPt*/int data, /*Bit16u*/IntRef error) {
            ...
            /*Bit8u*/
            int type = Memory.mem_readb(data);
        ...
        boolean GetVolumeName(/*Bit8u*/short subUnit, StringRef data) {
            ...
            boolean success = ReadVTOC(drive, 0x00, ptoc, error);
            if (success) {
        ...
        boolean GetCopyrightName(/*Bit16u*/int drive, /*PhysPt*/int data) {
            ...
            if (success) {
                ...
                for (len = 0; len < 37; len++) {
                    /*Bit8u*/
                    int c = Memory.mem_readb(ptoc + 702 + len);
                    if (c == 0 || c == 0x20)
                        break;
                }
        ...
        boolean GetAbstractName(/*Bit16u*/int drive, /*PhysPt*/int data) {
            ...
            if (success) {
                ...
                for (len = 0; len < 37; len++) {
                    /*Bit8u*/
                    int c = Memory.mem_readb(ptoc + 739 + len);
                    if (c == 0 || c == 0x20)
                        break;
                }
        ...
        boolean GetDocumentationName(/*Bit16u*/int drive, /*PhysPt*/int data) {
            ...
            if (success) {
                ...
                for (len = 0; len < 37; len++) {
                    /*Bit8u*/
                    int c = Memory.mem_readb(ptoc + 776 + len);
                    if (c == 0 || c == 0x20)
                        break;
                }
        ...
        boolean ReadSectors(/*Bit8u*/short subUnit, boolean raw, /*Bit32u*/long sector, /*Bit16u*/int num,
            /*PhysPt*/int data) {
            if (subUnit >= numDrives)
                return false;
            if (4 * num * 2048 + 5 < CPU.CPU_Cycles)
                CPU.CPU_Cycles -= 4 * num * 2048;
            else
                CPU.CPU_Cycles = 5;
        ...
        boolean ReadSectorsMSF(/*Bit8u*/short subUnit, boolean raw, /*Bit32u*/long start, /*Bit16u*/int num,
            /*PhysPt*/int data) {
            if (subUnit >= numDrives)
                return false;
            /*Bit8u*/
        ...
        boolean GetDirectoryEntry(/*Bit16u*/int drive, boolean copyFlag, /*PhysPt*/int pathname, /*PhysPt*/int buffer,
            /*Bit16u*/IntRef error) {
            String volumeID;
            String searchName;
            String entryName;
            ...
            if (!ReadSectors(GetSubUnit(drive), false, 16, 1, defBuffer))
                return false;
            ...
            while (dirSize > 0) {
                index = 0;
                if (!ReadSectors(GetSubUnit(drive), false, dirEntrySector, 1, defBuffer))
                    return false;
                // Get string part
                foundName = false;
                if (nextPart) {
                    ...
                    if (foundComplete) {
                        if (copyFlag) {
                            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_WARN,
                                "MSCDEX: GetDirEntry: Copyflag structure not entirely accurate maybe");
                            /*Bit8u*/
                            ...
                            writeBuf[5] = 0;
                            writeBuf[6] = 8; // 05h	WORD	size of disk in logical blocks
        ...
        /*Bit32u*/long GetDeviceStatus(/*Bit8u*/short subUnit) {
            ...
            return (trayOpen.value ? 1 : 0) << 0 | // Drive is open ?
        ...
        /*Bit16u*/int GetStatusWord(/*Bit8u*/short subUnit, /*Bit16u*/int status) {
            if (subUnit >= numDrives)
                return REQUEST_STATUS_ERROR | 0x02; // error : Drive not ready

            if (dinfo[subUnit].lastResult)
                status |= REQUEST_STATUS_DONE; // ok
            else
