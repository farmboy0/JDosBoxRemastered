...
public class DosMSCDEX {
    ...
    static private final Callback.Handler MSCDEX_Interrupt_Handler = new Callback.Handler() {
        ...
        public /*Bitu*/int call() {
            ...
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL, "MSCDEX: Driver Function " + Integer.toString(funcNr, 16));
            ...
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL, "MSCDEX: Status : " + Integer.toString(Memory.mem_readw(curReqheaderPtr + 3), 16));
            return Callback.CBRET_NONE;
        }
    };
    static private final Dos_system.MultiplexHandler MSCDEX_Handler = new Dos_system.MultiplexHandler() {
        public boolean call() {
            ...
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL, "MSCDEX: INT 2F " + Integer.toString(CPU_Regs.reg_eax.word(), 16) + " BX= " + Integer.toString(CPU_Regs.reg_ebx.word(), 16) + " CX=" + Integer.toString(CPU_Regs.reg_ecx.word(), 16));
    ...
    private static /*Bit16u*/int MSCDEX_IOCTL_Input(/*PhysPt*/int buffer,/*Bit8u*/short drive_unit) {
        ...
        if (Log.level <= LogSeverities.LOG_NORMAL)
            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL, "MSCDEX: IOCTL INPUT Subfunction " + Integer.toString(ioctl_fct, 16));
        switch (ioctl_fct) {
            ...
            case 0x01: {/* Get current position */
                ...
                if (addr_mode == 0) {            // HSG
                    ...
                    if (frames < 150) if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR, "MSCDEX: Get position: invalid position " + pos.min + ":" + pos.sec + ":" + pos.fr);
                    else frames -= 150;
...
//        #endif
                ...
                case 0x01:    // iso cdrom interface
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL, "MSCDEX: Mounting iso file as cdrom: " + physicalPath);
                ...
                case 0x02:    // fake cdrom interface (directories)
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL, "MSCDEX: Mounting directory as cdrom: " + physicalPath);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
        ...
        /*Bit32u*/long GetDeviceStatus(/*Bit8u*/short subUnit) {
            ...
            /*Bit32u*/
            long status = ((trayOpen.value ? 1 : 0) << 0) |    // Drive is open ?
