            ...
            case 0x06:      /* Get Input Status */
                if ((Dos_files.Files[handle].GetInformation() & 0x8000) != 0) {        //Check for device
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_IOCTL, LogSeverities.LOG_NORMAL, "06:Used Get Input Status on regular file with handle " + handle);
                }
                return true;
            case 0x07:        /* Get Output Status */
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_IOCTL, LogSeverities.LOG_NORMAL, "07:Fakes output status is ready for handle " + handle);
                CPU_Regs.reg_eax.low(0xff);
                return true;
