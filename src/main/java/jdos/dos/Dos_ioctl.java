...
public class Dos_ioctl {
    public static boolean DOS_IOCTL() {
        /*Bitu*/
        int handle = 0;/*Bit8u*/
        short drive = 0;
        /* calls 0-4,6,7,10,12,16 use a file handle */
        if (CPU_Regs.reg_eax.low() < 4 || CPU_Regs.reg_eax.low() == 0x06 || CPU_Regs.reg_eax.low() == 0x07
            || CPU_Regs.reg_eax.low() == 0x0a || CPU_Regs.reg_eax.low() == 0x0c || CPU_Regs.reg_eax.low() == 0x10) {
            ...
            if (CPU_Regs.reg_eax.low() != 0x0b) {
                drive = (short) CPU_Regs.reg_ebx.low();
                if (drive == 0)
            ...
            if (Log.level <= LogSeverities.LOG_ERROR)
                Log.log(LogTypes.LOG_DOSMISC, LogSeverities.LOG_ERROR,
                    "DOS:IOCTL Call " + Integer.toString(CPU_Regs.reg_eax.low(), 16) + " unhandled");
        ...
        switch (CPU_Regs.reg_eax.low()) {
            case 0x00: /* Get Device Information */
                if ((Dos_files.Files[handle].GetInformation() & 0x8000) != 0) { //Check for device
                    ...
                    /*Bit8u*/
                    short hdrive = Dos_files.Files[handle].GetDrive();
                    if (hdrive == 0xff) {
            ...
            case 0x02: /* Read from Device Control Channel */
                if ((Dos_files.Files[handle].GetInformation() & 0xc000) != 0) {
                    ...
                    /*PhysPt*/
                    int bufptr = Memory.PhysMake(CPU_Regs.reg_dsVal.dword, CPU_Regs.reg_edx.word());
                    /*Bit16u*/
                    IntRef retcode = new IntRef(0);
                    if (((DOS_Device) Dos_files.Files[handle]).ReadFromControlChannel(bufptr, CPU_Regs.reg_ecx.word(),
                        retcode)) {
            ...
            case 0x03: /* Write to Device Control Channel */
                if ((Dos_files.Files[handle].GetInformation() & 0xc000) != 0) {
                    ...
                    /*Bit16u*/
                    IntRef retcode = new IntRef(0);
                    if (((DOS_Device) Dos_files.Files[handle]).WriteToControlChannel(bufptr, CPU_Regs.reg_ecx.word(),
                        retcode)) {
                        CPU_Regs.reg_eax.word(retcode.value);
                        return true;
                    }
            ...
            case 0x06: /* Get Input Status */
                if ((Dos_files.Files[handle].GetInformation() & 0x8000) != 0) { //Check for device
                    ...
                    /*Bit32u*/
                    LongRef oldlocation = new LongRef(0);
                    Dos_files.Files[handle].Seek(oldlocation, Dos_files.DOS_SEEK_CUR);
                    /*Bit32u*/
                    LongRef endlocation = new LongRef(0);
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_IOCTL, LogSeverities.LOG_NORMAL,
                            "06:Used Get Input Status on regular file with handle " + handle);
                }
                return true;
            case 0x07: /* Get Output Status */
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_IOCTL, LogSeverities.LOG_NORMAL,
                        "07:Fakes output status is ready for handle " + handle);
                CPU_Regs.reg_eax.low(0xff);
                return true;
            ...
            case 0x0D: /* Generic block device request */ {
                ...
                /*PhysPt*/
                int ptr = CPU_Regs.reg_dsPhys.dword + CPU_Regs.reg_edx.word();
                switch (CPU_Regs.reg_ecx.low()) {
                    ...
                    case 0x66: /* Volume label */ {
                        ...
                        if (CPU_Regs.reg_ecx.low() == 0x66)
                            Memory.MEM_BlockWrite(ptr + 0x11, new String(buf2), 8);//filesystem
                    }
                        break;
                    default:
                        if (Log.level <= LogSeverities.LOG_ERROR)
                            Log.log(LogTypes.LOG_IOCTL, LogSeverities.LOG_ERROR,
                                "DOS:IOCTL Call 0D:" + Integer.toString(CPU_Regs.reg_ecx.low(), 16) + " Drive "
                                    + Integer.toString(drive, 16) + " unhandled");
                        Dos.DOS_SetError(Dos.DOSERR_FUNCTION_NUMBER_INVALID);
                        return false;
                }
                return true;
    ...
    public static boolean DOS_GetSTDINStatus() {
        /*Bit32u*/
        int handle = Dos.RealHandle(Dos_files.STDIN);
        if (handle == 0xFF)
            return false;
