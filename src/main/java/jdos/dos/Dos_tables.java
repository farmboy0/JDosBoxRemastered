...
public class Dos_tables {
    private static final int DOS_PRIVATE_SEGMENT = 0xc800;
    private static final int DOS_PRIVATE_SEGMENT_END = 0xd000;
    static /*Bitu*/ int call_casemap;
    private static int dos_memseg = DOS_PRIVATE_SEGMENT;
    private static final Callback.Handler DOS_CaseMapFunc = new Callback.Handler() {
        @Override
    ...
    private static final byte[] country_info = { /* Date format      */ 0x00, 0x00, /* Currencystring   */ 0x24, 0x00,
        0x00, 0x00, 0x00, /* Thousands sep    */ 0x2c, 0x00, /* Decimal sep      */ 0x2e, 0x00,
        /* Date sep         */ 0x2d, 0x00, /* time sep         */ 0x3a, 0x00, /* currency form    */ 0x00,
        /* digits after dec */ 0x02, /* Time format      */ 0x00, /* Casemap          */ 0x00, 0x00, 0x00, 0x00,
        /* Data sep         */ 0x2c, 0x00, /* Reservered 5     */ 0x00, 0x00, 0x00, 0x00, 0x00,
        /* Reservered 5     */ 0x00, 0x00, 0x00, 0x00, 0x00 };
    /*RealPt*/ int DOS_TableUpCase;
    /*RealPt*/ int DOS_TableLowCase;

    public static /*Bit16u*/int DOS_GetMemory(/*Bit16u*/int pages) {
    ...
    public static void DOS_SetupTables() {
        /*Bit16u*/
        int seg;/*Bitu*/
        int i;
        ...
        for (i = 0; i < Dos_files.DOS_DRIVES; i++)
            Memory.mem_writew(Memory.Real2Phys(Dos.dos.tables.mediaid) + i * 2, 0);
        ...
        for (i = 0; i < 256; i++)
            Memory.mem_writeb(Memory.Real2Phys(Dos.dos.tables.collatingseq) + i + 2, i);
        Dos.dos.tables.upcase = Dos.dos.tables.collatingseq + 258;
        Memory.mem_writew(Memory.Real2Phys(Dos.dos.tables.upcase), 0x80);
        for (i = 0; i < 128; i++)
            Memory.mem_writeb(Memory.Real2Phys(Dos.dos.tables.upcase) + i + 2, 0x80 + i);
        ...
        for (/*Bitu*/int d = 0; d < 26; d++)
            Memory.real_writeb(Dos.dos.tables.dpb, d, d);

        /* Create a fake disk buffer head */
        seg = DOS_GetMemory(6);
        for (/*Bitu*/int ct = 0; ct < 0x20; ct++)
            Memory.real_writeb(seg, ct, 0);
