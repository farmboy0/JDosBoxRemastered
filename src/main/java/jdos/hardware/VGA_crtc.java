...
public class VGA_crtc {
    public static IoHandler.IO_WriteHandler vga_write_p3d4 = (port, val, iolen) -> VGA.vga.crtc.index = (short) val;

    public static IoHandler.IO_ReadHandler vga_read_p3d4 = (port, iolen) -> VGA.vga.crtc.index;

    public static IoHandler.IO_WriteHandler vga_write_p3d5 = (port, val, iolen) -> {
        ...
        switch (VGA.vga.crtc.index) {
            ...
            case 0x11: /* Vertical Retrace End Register */
                ...
                if (Dosbox.IS_VGA_ARCH())
                    VGA.vga.crtc.read_only = (val & 128) > 0;
                else
                ...
                if (VGA.svga.write_p3d5 != null) {
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                            "VGA:CRTC:Write to unknown index " + Integer.toString(VGA.vga.crtc.index, 16));
                }
                break;
        }
    };

    public static IoHandler.IO_ReadHandler vga_read_p3d5 = (port, iolen) -> {
        ...
        switch (VGA.vga.crtc.index) {
            ...
            default:
                if (VGA.svga.read_p3d5 != null) {
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                            "VGA:CRTC:Read from unknown index " + Integer.toString(VGA.vga.crtc.index, 16));
                    return 0x0;
                }
        }
    };
