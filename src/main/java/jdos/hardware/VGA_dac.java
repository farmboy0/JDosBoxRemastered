...
public class VGA_dac {
    ...
    */
    ...
    private static final IoHandler.IO_WriteHandler write_p3c6 = (port, val, iolen) -> {
        if (VGA.vga.dac.pel_mask != val) {
            if (Log.level <= LogSeverities.LOG_NORMAL)
                Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                    "VGA:DCA:Pel Mask set to " + Integer.toString(val, 16));
            VGA.vga.dac.pel_mask = (short) val;
            for ( /*Bitu*/int i = 0; i < 256; i++)
                VGA_DAC_UpdateColor(i);
        }
    };
    private static final IoHandler.IO_ReadHandler read_p3c6 = (port, iolen) -> VGA.vga.dac.pel_mask;
    private static final IoHandler.IO_WriteHandler write_p3c7 = (port, val, iolen) -> {
    ...
    private static final IoHandler.IO_ReadHandler read_p3c7 = (port, iolen) -> {
        if (VGA.vga.dac.state == DAC_READ)
            return 0x3;
        else
            return 0x0;
    };
    private static final IoHandler.IO_WriteHandler write_p3c8 = (port, val, iolen) -> {
        VGA.vga.dac.write_index = (short) (val & 0xFF);
        VGA.vga.dac.pel_index = 0;
        VGA.vga.dac.state = DAC_WRITE;
    };
    private static final IoHandler.IO_ReadHandler read_p3c8 = (port, iolen) -> VGA.vga.dac.write_index;
    private static final IoHandler.IO_WriteHandler write_p3c9 = (port, val, iolen) -> {
        val &= 0x3f;
        switch (VGA.vga.dac.pel_index) {
            ...
            case 2:
                ...
                switch (VGA.vga.mode) {
                    ...
                    case VGA.M_LIN8:
                        ...
                        if (VGA.vga.dac.pel_mask != 0xff) {
                            /*Bitu*/
                            int index = VGA.vga.dac.write_index;
                ...
                VGA.vga.dac.write_index++;
                VGA.vga.dac.write_index &= 0xFF;
    ...
    private static final IoHandler.IO_ReadHandler read_p3c9 = (port, iolen) -> {
        /*Bitu*/
        short ret;
        switch (VGA.vga.dac.pel_index) {
            case 0:
            ...
            case 2:
                ...
                VGA.vga.dac.read_index++;
                VGA.vga.dac.read_index &= 0xFF;
                VGA.vga.dac.pel_index = 0;
    ...
    private static void VGA_DAC_SendColor( /*Bitu*/int index, /*Bitu*/int src) {
    ...
    private static void VGA_DAC_UpdateColor( /*Bitu*/int index) {
        /*Bitu*/
    ...
    public static void VGA_SetupDAC() {
        ...
        if (Dosbox.IS_VGA_ARCH()) {
            ...
            for (/*Bitu*/int i = 0; i < 64; i++) {
                if ((i & 4) > 0)
                    VGA.vga.dac.rgb[i].red = 0x2a;
                else
                    VGA.vga.dac.rgb[i].red = 0;
                if ((i & 32) > 0)
                    VGA.vga.dac.rgb[i].red += 0x15;

                if ((i & 2) > 0)
                    VGA.vga.dac.rgb[i].green = 0x2a;
                else
                    VGA.vga.dac.rgb[i].green = 0;
                if ((i & 16) > 0)
                    VGA.vga.dac.rgb[i].green += 0x15;

                if ((i & 1) > 0)
                    VGA.vga.dac.rgb[i].blue = 0x2a;
                else
                    VGA.vga.dac.rgb[i].blue = 0;
                if ((i & 8) > 0)
                    VGA.vga.dac.rgb[i].blue += 0x15;
            }
        }
    }
}
