...
public class VGA_tseng {
    ...
    private static final VGA.tWritePort write_p3d5_et4k = (reg, val, iolen) -> {
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:CRTC:ET4K:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
...
//            return et4k.store_##port##_##index;
    private static final VGA.tReadPort read_p3d5_et4k = (reg, iolen) -> {
        if (!et4k.extensionsEnabled && reg != 0x33)
            return 0x0;
        switch (reg) {
            case 0x31:
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:CRTC:ET4K:Read from illegal index " + Integer.toString(reg, 16));
                break;
        }
        return 0x0;
    };
    private static final VGA.tWritePort write_p3c5_et4k = (reg, val, iolen) -> {
        switch (reg) {
            /*
            ...
            */
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:SEQ:ET4K:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
    private static final VGA.tReadPort read_p3c5_et4k = (reg, iolen) -> {
        switch (reg) {
            case 0x06:
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:SEQ:ET4K:Read from illegal index " + Integer.toString(reg, 16));
                break;
        }
        return 0x0;
    };
    private static final VGA.tWritePort write_p3cd_et4k = (reg, val, iolen) -> {
    ...
    */
    private static final IoHandler.IO_WriteHandler io_write_p3cd_et4k = (port, val, iolen) -> write_p3cd_et4k.call(port, val, iolen);
    private static final IoHandler.IO_ReadHandler read_p3cd_et4k = (port, iolen) -> VGA.vga.svga.bank_read << 4 | VGA.vga.svga.bank_write;
    private static final VGA.tWritePort write_p3c0_et4k = (reg, val, iolen) -> {
        switch (reg) {
            ...
            */
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:ATTR:ET4K:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
    private static final VGA.tReadPort read_p3c1_et4k = (reg, iolen) -> {
        switch (reg) {
            case 0x16:
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:ATTR:ET4K:Read from illegal index " + Integer.toString(reg, 16));
                break;
        }
        return 0x0;
    };
    private static final VGA.tFinishSetMode FinishSetMode_ET4K = (crtc_base, modeData) -> {
        ...
        /*Bit8u*/
        int et4k_hor_overflow = modeData.hor_overflow & 0x01 | modeData.hor_overflow & 0x04
        ...
        /*Bit8u*/
        int et4k_ver_overflow = (modeData.ver_overflow & 0x01) << 1 | // vtotal10
        ...
        IoHandler.IO_Write(crtc_base + 1,
            0x0c | (VGA.vga.vmemsize == 1024 * 1024 ? 3 : VGA.vga.vmemsize == 512 * 1024 ? 2 : 1));
        // Clear ext SEQ
    ...
    */
    private static final VGA.tDetermineMode DetermineMode_ET4K = () -> {
        ...
        if ((VGA.vga.attr.mode_control & 1) != 0) {
            if ((VGA.vga.gfx.mode & 0x40) != 0)
                VGA.VGA_SetMode(et4k.biosMode <= 0x13 ? VGA.M_VGA : VGA.M_LIN8); // Ugly...
            else if ((VGA.vga.gfx.mode & 0x20) != 0)
                VGA.VGA_SetMode(VGA.M_CGA4);
            else if ((VGA.vga.gfx.miscellaneous & 0x0c) == 0x0c)
                VGA.VGA_SetMode(VGA.M_CGA2);
            else
                VGA.VGA_SetMode(et4k.biosMode <= 0x13 ? VGA.M_EGA : VGA.M_LIN4);
        } else {
            VGA.VGA_SetMode(VGA.M_TEXT);
        }
    };
    private static final VGA.tSetClock SetClock_ET4K = (which, target) -> {
        et4k.clockFreq[which] = 1000 * target;
        VGA.VGA_StartResize();
    };
    private static final VGA.tGetClock GetClock_ET4K = () -> et4k.clockFreq[get_clock_index_et4k()];
    private static final VGA.tAcceptsMode AcceptsMode_ET4K = modeNo -> Int10_modes.VideoModeMemSize(modeNo) < VGA.vga.vmemsize;
    private static final SVGA_ET3K_DATA et3k = new SVGA_ET3K_DATA();
    private static final VGA.tWritePort write_p3d5_et3k = (reg, val, iolen) -> {
        switch (reg) {
            ...
            */
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:CRTC:ET3K:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
    private static final VGA.tReadPort read_p3d5_et3k = (reg, iolen) -> {
        switch (reg) {
            case 0x1b:
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:CRTC:ET3K:Read from illegal index " + Integer.toString(reg, 16));
                break;
        }
        return 0x0;
    };
    private static final VGA.tWritePort write_p3c5_et3k = (reg, val, iolen) -> {
        switch (reg) {
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:SEQ:ET3K:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
    private static final VGA.tReadPort read_p3c5_et3k = (reg, iolen) -> {
        switch (reg) {
            case 0x06:
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:SEQ:ET3K:Read from illegal index " + Integer.toString(reg, 16));
                break;
        }
        return 0x0;
    };
    ...
    */
    private static final IoHandler.IO_WriteHandler write_p3cd_et3k = (port, val, iolen) -> {
...
//            return et3k.store_##port##_##index;
    private static final IoHandler.IO_ReadHandler read_p3cd_et3k = (port, iolen) -> VGA.vga.svga.bank_read << 3 | VGA.vga.svga.bank_write
        | (VGA.vga.svga.bank_size == 128 * 1024 ? 0 : 0x40);
    private static final VGA.tWritePort write_p3c0_et3k = (reg, val, iolen) -> {
        // See ET4K notes.
        switch (reg) {
            case 0x16:
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:ATTR:ET3K:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
    private static final VGA.tReadPort read_p3c1_et3k = (reg, iolen) -> {
        switch (reg) {
            case 0x16:
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:ATTR:ET3K:Read from illegal index " + Integer.toString(reg, 16));
                break;
        }
        return 0x0;
    };
    private static final VGA.tFinishSetMode FinishSetMode_ET3K = (crtc_base, modeData) -> {
        ...
        IoHandler.IO_Write(0x3c5, 0);
        IoHandler.IO_Write(0x3c4, 0x07);
        IoHandler.IO_Write(0x3c5, 0x40); // 0 in this register breaks WHATVGA
        // Clear ext ATTR
        IoHandler.IO_Write(0x3c0, 0x16);
        IoHandler.IO_Write(0x3c0, 0);
        IoHandler.IO_Write(0x3c0, 0x17);
    ...
    private static final VGA.tDetermineMode DetermineMode_ET3K = () -> {
        // Close replica from the base implementation. It will stay here
        // until I figure a way to either distinguish M_VGA and M_LIN8 or
        // merge them.
        if ((VGA.vga.attr.mode_control & 1) != 0) {
            if ((VGA.vga.gfx.mode & 0x40) != 0)
                VGA.VGA_SetMode(et3k.biosMode <= 0x13 ? VGA.M_VGA : VGA.M_LIN8); // Ugly...
            else if ((VGA.vga.gfx.mode & 0x20) != 0)
                VGA.VGA_SetMode(VGA.M_CGA4);
            else if ((VGA.vga.gfx.miscellaneous & 0x0c) == 0x0c)
                VGA.VGA_SetMode(VGA.M_CGA2);
            else
                VGA.VGA_SetMode(et3k.biosMode <= 0x13 ? VGA.M_EGA : VGA.M_LIN4);
        } else {
            VGA.VGA_SetMode(VGA.M_TEXT);
        }
    };
    private static final VGA.tSetClock SetClock_ET3K = (which, target) -> {
        et3k.clockFreq[which] = 1000 * target;
        VGA.VGA_StartResize();
    };
    private static final VGA.tGetClock GetClock_ET3K = () -> et3k.clockFreq[get_clock_index_et3k()];
    private static final VGA.tAcceptsMode AcceptsMode_ET3K = mode -> mode <= 0x37 && mode != 0x2f && Int10_modes.VideoModeMemSize(mode) < VGA.vga.vmemsize;
    ...
    */
    ...
    private static /*Bitu*/int get_clock_index_et4k() {
    ...
    private static void set_clock_index_et4k(/*Bitu*/int index) {
    ...
    private static /*Bitu*/int get_clock_index_et3k() {
        return VGA.vga.misc_output >> 2 & 3 | et3k.store_3d4_24 << 1 & 4;
    }

    private static void set_clock_index_et3k(/*Bitu*/int index) {
        // Shortwiring register reads/writes for simplicity
        IoHandler.IO_Write(0x3c2, VGA.vga.misc_output & ~0x0c | (index & 3) << 2);
