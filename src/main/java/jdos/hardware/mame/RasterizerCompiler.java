...
public class RasterizerCompiler extends RasterizerCompilerCommon {
    ...
    public static void save(ZipOutputStream out) throws IOException {
    ...
    private static void getRegR(StringBuilder method, String value) {
        method.append("(").append(value).append(" >> 16) & 0xFF");
    }

    private static void getRegG(StringBuilder method, String value) {
        method.append("(").append(value).append(" >> 8) & 0xFF");
    }

    private static void getRegB(StringBuilder method, String value) {
        method.append(value).append(" & 0xFF");
    }

    private static void getRegA(StringBuilder method, String value) {
        method.append("(").append(value).append(" >> 24) & 0xFF");
    }

    private static void CLAMP(StringBuilder method, String val, String min, String max) {
    ...
    private static void CLAMPED_Z(StringBuilder method, String iterz, int fbzcp, String result) {
    ...
    private static void CLAMPED_W(StringBuilder method, String iterw, int fbzcp, String result) {
    ...
    private static void TEXTURE_PIPELINE(StringBuilder method, int texMode, String lodBase, String iters, String itert,
        String iterw) {
...
//        int c_local;
        ...
        if (VoodooCommon.TEXMODE_ENABLE_PERSPECTIVE(texMode)) {
            ...
            method.append("        temp = (int)(")
                .append(iterw)
                .append(" >> 16);\n" + "        exp -= 16;\n" + "     } else {\n" + "         temp = (int)")
                .append(iterw)
                .append(";\n" + "     }\n");
            method.append("     if (temp == 0) {\n" + "         lod = 1000 << ")
                .append(VoodooCommon.LOG_OUTPUT_PREC)
                .append(";\n" + "         oow = neg ? 0x80000000 : 0x7fffffff;\n" + "     } else {\n");
            method
                .append("         lz = Integer.numberOfLeadingZeros(temp);\n" + "         temp <<= lz;\n"
                    + "         exp += lz;\n" + "         tablePos = (temp >>> (31 - ")
                .append(VoodooCommon.RECIPLOG_LOOKUP_BITS)
                .append(" - 1)) & ((2 << ")
                .append(VoodooCommon.RECIPLOG_LOOKUP_BITS)
                .append(") - 2);\n" + "         interp = (temp >>> (31 - ")
                .append(VoodooCommon.RECIPLOG_LOOKUP_BITS)
                .append(" - 8)) & 0xff;\n"
                    ...
                    + "         rlog = (rlog + (1 << (")
                .append(VoodooCommon.RECIPLOG_LOOKUP_PREC)
                .append(" - ")
                .append(VoodooCommon.LOG_OUTPUT_PREC)
                .append(" - 1))) >> (")
                .append(VoodooCommon.RECIPLOG_LOOKUP_PREC)
                ...
                .append(VoodooCommon.LOG_OUTPUT_PREC)
                .append(");\n" + "         lod = ((exp - (31 - ")
                .append(VoodooCommon.RECIPLOG_INPUT_PREC)
                .append(")) << ")
                .append(VoodooCommon.LOG_OUTPUT_PREC)
                .append(") - rlog;\n" + "         exp += (")
                .append(VoodooCommon.RECIP_OUTPUT_PREC)
                .append(" - ")
                .append(VoodooCommon.RECIPLOG_LOOKUP_PREC)
                .append(") - (31 - ")
                .append(VoodooCommon.RECIPLOG_INPUT_PREC)
        ...
        if (!VoodooCommon.TEXMODE_MAGNIFICATION_FILTER(texMode) && !VoodooCommon.TEXMODE_MINIFICATION_FILTER(texMode)) {
            pointSampled = 0;
        } else if (VoodooCommon.TEXMODE_MAGNIFICATION_FILTER(texMode)
            && VoodooCommon.TEXMODE_MINIFICATION_FILTER(texMode)) {
            pointSampled = 2;
        }
        ...
        if (pointSampled >= 1) {
            ...
            method
                .append("c_local = VoodooCommon.rgba_bilinear_filter(texel0, texel1, texel2, texel3, sfrac, tfrac);\n");
        }
        if (pointSampled == 1)
            method.append("}\n");
        ...
        if (VoodooCommon.TEXMODE_TC_MSELECT(texMode) == 0 && VoodooCommon.TEXMODE_TCA_MSELECT(texMode) == 0
            && !VoodooCommon.TEXMODE_TC_REVERSE_BLEND(texMode) && !VoodooCommon.TEXMODE_TCA_REVERSE_BLEND(texMode)) {
    ...
    public static void compile(raster_info info, int tmuCount, int colorPath, int alphaMode, int fogMode, int fbzMode,
        int textureMode0, int textureMode1) {
...
//                "");
    ...
    private static void compile(StringBuilder method, int tmuCount, int colorPath, int alphaMode, int fogMode,
        int fbzMode, int textureMode0, int textureMode1) {
        method
            .append("final VoodooCommon v = extra.state;\n" + "final stats_block stats = v.thread_stats[threadid];\n");
        if (VoodooCommon.FBZMODE_ENABLE_DITHERING(fbzMode)) {
            method.append("byte[] dither_lookup = null;\n" + "int dither_lookupPos = 0;\n" + "byte[] dither4 = null;\n"
                + "int dither4Pos = 0;\n" + "byte[] dither = null;\n" + "int ditherPos=0;");
        }
        boolean uses_depthPos = VoodooCommon.FBZMODE_AUX_BUFFER_MASK(fbzMode)
            || VoodooCommon.FBZMODE_ENABLE_DEPTHBUF(fbzMode);
        ...
        if (VoodooCommon.FBZMODE_ENABLE_CLIPPING(fbzMode)) {
            method.append("if (scry < ((v.reg[")
                .append(VoodooCommon.clipLowYHighY)
                .append("] >> 16) & 0x3ff) || scry >= (v.reg[")
                .append(VoodooCommon.clipLowYHighY)
                .append("] & 0x3ff)) {\n" + "    stats.pixels_in += stopx - startx;\n"
                    + "    stats.clip_fail += stopx - startx;\n" + "    return;\n" + "}\n" + "int tempclip = (v.reg[")
                .append(VoodooCommon.clipLeftRight)
                .append(
        ...
        method.append(
            ", iterr += extra.drdx, iterg += extra.dgdx, iterb += extra.dbdx, itera += extra.dadx, iterz += extra.dzdx, iterw += extra.dwdx");
        if (tmuCount >= 1) {
        ...
        method.append("){\n" + "int texel = 0;\n" + "int iterargb = 0;\n" + "int r, g, b;\n");
        boolean uses_a = VoodooCommon.FBZMODE_AUX_BUFFER_MASK(fbzMode)
            && VoodooCommon.FBZMODE_ENABLE_ALPHA_PLANES(fbzMode) || VoodooCommon.ALPHAMODE_SRCRGBBLEND(alphaMode) == 1
            || VoodooCommon.ALPHAMODE_SRCRGBBLEND(alphaMode) == 5 || VoodooCommon.ALPHAMODE_SRCRGBBLEND(alphaMode) == 15
        ...
        if (VoodooCommon.FBZMODE_ENABLE_STIPPLE(fbzMode)) {
            ...
            if (!VoodooCommon.FBZMODE_STIPPLE_PATTERN(fbzMode)) {
                method.append("v.reg[")
                    .append(VoodooCommon.stipple)
                    .append("] = (v.reg[")
                    .append(VoodooCommon.stipple)
                    .append("] << 1) | (v.reg[")
                    .append(VoodooCommon.stipple)
                    .append("] >> 31);\n");
                method.append("if ((v.reg[")
                    .append(VoodooCommon.stipple)
                    .append(
                        "] & 0x80000000) == 0) {\n" + "    v.stats.total_stippled++;\n" + "    continue;\n" + "}\n");
            } else { /* pattern mode */
                method.append("if (((reg[")
                    .append(VoodooCommon.stipple)
                    .append("] >> (((y & 3) << 3) | (~x & 7))) & 1) == 0) {\n" + "    v.stats.total_stippled++;\n"
                        + "    continue;\n" + "}\n");
            }
        }

        boolean needDepthVal = VoodooCommon.FBZMODE_ENABLE_DEPTHBUF(fbzMode)
            && !VoodooCommon.FBZMODE_DEPTH_SOURCE_COMPARE(fbzMode)
            || VoodooCommon.FBZMODE_AUX_BUFFER_MASK(fbzMode) && !VoodooCommon.FBZMODE_ENABLE_ALPHA_PLANES(fbzMode);
        boolean needWFloat = needDepthVal && VoodooCommon.FBZMODE_WBUFFER_SELECT(fbzMode)
            && !VoodooCommon.FBZMODE_DEPTH_FLOAT_SELECT(fbzMode)
            || VoodooCommon.FOGMODE_ENABLE_FOG(fogMode) && VoodooCommon.FOGMODE_FOG_ZALPHA(fogMode) == 0;
        ...
        boolean uses_iterargb = VoodooCommon.FBZCP_CC_RGBSELECT(colorPath) == 0
            || VoodooCommon.FBZCP_CC_ASELECT(colorPath) == 0 || VoodooCommon.FBZCP_CCA_LOCALSELECT(colorPath) == 0
            || VoodooCommon.FBZCP_CC_LOCALSELECT_OVERRIDE(colorPath)
            || !VoodooCommon.FBZCP_CC_LOCALSELECT_OVERRIDE(colorPath) && !VoodooCommon.FBZCP_CC_LOCALSELECT(colorPath)
            || VoodooCommon.FOGMODE_ENABLE_FOG(fogMode) && !VoodooCommon.FOGMODE_FOG_CONSTANT(fogMode)
                && VoodooCommon.FOGMODE_FOG_ZALPHA(fogMode) == 1;
        if (uses_iterargb) {
        ...
        boolean uses_c_local = VoodooCommon.FBZCP_CC_SUB_CLOCAL(colorPath)
            || VoodooCommon.FBZCP_CCA_SUB_CLOCAL(colorPath) || VoodooCommon.FBZCP_CC_MSELECT(colorPath) == 1
            || VoodooCommon.FBZCP_CC_MSELECT(colorPath) == 3 || VoodooCommon.FBZCP_CCA_MSELECT(colorPath) == 1
            || VoodooCommon.FBZCP_CCA_MSELECT(colorPath) == 3 || VoodooCommon.FBZCP_CC_ADD_ACLOCAL(colorPath) == 1
            || VoodooCommon.FBZCP_CC_ADD_ACLOCAL(colorPath) == 2 || VoodooCommon.FBZCP_CCA_ADD_ACLOCAL(colorPath) != 0;
        ...
        if (VoodooCommon.FBZCP_CC_MSELECT(colorPath) == 0 && VoodooCommon.FBZCP_CCA_MSELECT(colorPath) == 0
            && !VoodooCommon.FBZCP_CC_REVERSE_BLEND(colorPath)
            && (!VoodooCommon.FBZCP_CCA_REVERSE_BLEND(colorPath) || !uses_a)) {
        ...
        if (VoodooCommon.ALPHAMODE_ALPHABLEND(alphaMode)) {
            method
                .append("int dpix = dest[destPos+x] & 0xFFFF;\n" + "int dr = (dpix >> 8) & 0xf8;\n"
                    + "int dg = (dpix >> 3) & 0xfc;\n" + "int db = (dpix << 3) & 0xf8;\n" + "int da = ")
                .append(VoodooCommon.FBZMODE_ENABLE_ALPHA_PLANES(fbzMode) ? "dest[destPos+x] & 0xFFFF\n" : "0xff;\n"
                    + "int sr = r;\n" + "int sb = b;\n" + "int sg = g;\n");
            if (uses_a)
    ...
    private static raster_info compileMethod(StringBuilder method, raster_info info) {
        //System.out.println(method.toString());
        try {
            ...
            String constructor = "{this.eff_color_path = " + info.eff_color_path + ";" + "this.eff_alpha_mode = "
                + info.eff_alpha_mode + ";" + "this.eff_fog_mode = " + info.eff_fog_mode + ";" + "this.eff_fbz_mode = "
                + info.eff_fbz_mode + ";" + "this.eff_tex_mode_0 = " + info.eff_tex_mode_0 + ";"
                + "this.eff_tex_mode_1 = " + info.eff_tex_mode_1 + ";" + "this.callback = this;}";
