...
public class Atapi extends Internal {
    private static final AtapiCmdCallback cmd_test_unit_ready = (s, buf) -> ide_atapi_cmd_ok(s);
    private static final AtapiCmdCallback cmd_prevent_allow_medium_removal = (s, buf) -> {
        s.tray_locked = (buf[4] & 1) != 0;
        Block.bdrv_lock_medium(s.bs, (buf[4] & 1) != 0);
        ide_atapi_cmd_ok(s);
    };
    private static final AtapiCmdCallback cmd_seek = (s, buf) -> {
        long lba;
    ...
    private static final AtapiCmdCallback cmd_start_stop_unit = (s, buf) -> {
        int sense;
    ...
    private static final AtapiCmdCallback cmd_set_speed = (s, buf) -> ide_atapi_cmd_ok(s);
    /*
     * Only commands flagged as ALLOW_UA are allowed to run under a
     * unit attention condition. (See MMC-5, section 4.1.6.1)
     */
    private static final int ALLOW_UA = 0x01;
    /*
     ...
     * 4.1.8)
     */
    private static final int CHECK_READY = 0x02;
    private static final AtapiCmd[] atapi_cmd_table = new AtapiCmd[0x100];
    /* XXX: handle read errors */
    private static final Block.BlockDriverCompletionFunc ide_atapi_cmd_read_dma_cb = (opaque, ret) -> {
        ...
        while (true) { // eot label
            ...
            s.bus.dma.aiocb = Block.bdrv_aio_readv(s.bs, s.lba << 2, s.bus.dma.qiov, n * 4,
                Atapi.ide_atapi_cmd_read_dma_cb, s);
            return;
    ...
    public static Internal.EndTransferFunc ide_atapi_cmd_reply_end = s -> {
        int byte_count_limit, size, ret;
        if (DEBUG_IDE_ATAPI)
            System.out.println("reply: tx_size=" + s.packet_transfer_size + " elem_tx_size="
                + s.elementary_transfer_size + " index=" + s.io_buffer_index);
        if (s.packet_transfer_size <= 0) {
            /* end of transfer */
            ...
            if (s.elementary_transfer_size > 0) {
                ...
                IDE.ide_transfer_start(s, s.io_buffer, s.io_buffer_index - size, size, Atapi.ide_atapi_cmd_reply_end);
            } else {
    ...
    private static final AtapiCmdCallback cmd_get_event_status_notification = (s, buf) -> {
...
//            gesn_event_header.len = cpu_to_be16(used_len - sizeof(*gesn_event_header));
    ...
    private static final AtapiCmdCallback cmd_request_sense = (s, buf) -> {
    ...
    private static final AtapiCmdCallback cmd_inquiry = (s, buf) -> {
        int max_len = buf[4] & 0xFF;
    ...
    private static final AtapiCmdCallback cmd_get_configuration = (s, buf) -> {
        int len;
        int max_len;
    ...
    private static final AtapiCmdCallback cmd_mode_sense = (s, buf) -> {
        int action, code;
        int max_len;
    ...
    private static final AtapiCmdCallback cmd_read = (s, buf) -> {
        int nb_sectors;
        long lba;
    ...
    private static final AtapiCmdCallback cmd_read_cd = (s, buf) -> {
    ...
    private static final AtapiCmdCallback cmd_mechanism_status = (s, buf) -> {
    ...
    private static final AtapiCmdCallback cmd_read_toc_pma_atip = (s, buf) -> {
    ...
    private static final AtapiCmdCallback cmd_read_cdvd_capacity = (s, buf) -> {
        long total_sectors = s.nb_sectors >> 2;
    ...
    private static final AtapiCmdCallback cmd_read_disc_information = (s, buf) -> {
        int type = buf[1] & 7;
    ...
    private static final AtapiCmdCallback cmd_read_dvd_structure = (s, buf) -> {
        int max_len;
        ...
        java.util.Arrays.fill(buf, 0,
            max_len > IDE_DMA_BUF_SECTORS * 512 + 4 ? IDE_DMA_BUF_SECTORS * 512 + 4 : max_len, (byte) 0);
    ...
    public static Internal.EndTransferFunc ide_atapi_cmd = s -> {
...
//        #endif
    ...
    private static int event_status_media(IDEState s, byte[] buf) {
    ...
    private static boolean media_present(IDEState s) {
        return !s.tray_open && s.nb_sectors > 0;
    }

    /* XXX: DVDs that could fit on a CD will be reported as a CD */
    private static boolean media_is_dvd(IDEState s) {
        return media_present(s) && s.nb_sectors > Scsi.CD_MAX_SECTORS;
    }

    private static boolean media_is_cd(IDEState s) {
        return media_present(s) && s.nb_sectors <= Scsi.CD_MAX_SECTORS;
    }
    ...
    private static void ide_atapi_cmd_ok(IDEState s) {
        s.error = 0;
        s.status = READY_STAT | SEEK_STAT;
        s.nsector = s.nsector & ~7 | ATAPI_INT_REASON_IO | ATAPI_INT_REASON_CD;
        ide_set_irq(s.bus);
    ...
    private static void ide_atapi_cmd_error(IDEState s, int sense_key, int asc) {
        if (DEBUG_IDE_ATAPI)
            System.out.println(
                "atapi_cmd_error: sense=0x" + Integer.toHexString(sense_key) + " asc=0x" + Integer.toHexString(asc));
        s.error = sense_key << 4;
    ...
    private static void ide_atapi_io_error(IDEState s, int ret) {
    ...
    private static void ide_atapi_cmd_reply(IDEState s, int size, int max_size) {
        if (size > max_size)
            size = max_size;
    ...
    private static void ide_atapi_cmd_read_pio(IDEState s, long lba, int nb_sectors, int sector_size) {
        s.lba = lba;
    ...
    private static void ide_atapi_cmd_check_status(IDEState s) {
        if (DEBUG_IDE_ATAPI)
    ...
    private static void ide_atapi_cmd_read(IDEState s, long lba, int nb_sectors, int sector_size) {
        if (DEBUG_IDE_ATAPI)
            System.out
                .println("read " + (s.atapi_dma != 0 ? "dma" : "pio") + ": LBA=" + lba + " nb_sectors=" + nb_sectors);
        if (s.atapi_dma != 0) {
    ...
    private static int ide_atapi_set_profile(byte[] buf, IntRef index, int profile) {
    ...
    private static final class AtapiCmd {
