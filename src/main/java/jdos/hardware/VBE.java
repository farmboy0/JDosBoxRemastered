...
public class VBE {
    ...
    private static final int[] vbeRegs = new int[VBE_DISPI_INDEX_NB];
    public static boolean initialized = false;
    public static int pageCount = 0;
    public static int vbeLineOffset = 0;
    public static int vbeStartAddress = 0;
    public static Paging.PageHandler handler = new Bochs_LFB_Handler();
    private static int vbeIndex;
    private static final IoHandler.IO_ReadHandler vbe_read_index = (port, iolen) -> vbeIndex;
    private static final IoHandler.IO_ReadHandler vbe_read_data = (port, iolen) -> {
        int val = 0;
        ...
        System.out
            .println("VBE Read " + vbeIndex + " = " + val + " eip=0x" + Integer.toHexString(CPU_Regs.reg_eip));
        return val;
    };

    private static final IoHandler.IO_WriteHandler vbe_write_index = (port, val, iolen) -> vbeIndex = val;

    private static final IoHandler.IO_WriteHandler vbe_write_data = (port, data, iolen) -> {
        System.out
            .println("VBE Write " + vbeIndex + "=" + data + " eip=0x" + Integer.toHexString(CPU_Regs.reg_eip));
        if (vbeIndex <= VBE_DISPI_INDEX_NB) {
            switch (vbeIndex) {
                case VBE_DISPI_INDEX_ID:
                    if (data == VBE_DISPI_ID0 || data == VBE_DISPI_ID1 || data == VBE_DISPI_ID2
                        || data == VBE_DISPI_ID3 || data == VBE_DISPI_ID4)
                ...
                case VBE_DISPI_INDEX_BPP:
                    if (data == 0)
                        data = 8;
                    if (data == 4 || data == 8 || data == 15 || data == 16 || data == 32) {
                        vbeRegs[vbeIndex] = data;
                    }
                    break;
                case VBE_DISPI_INDEX_ENABLE:
                    if ((data & VBE_DISPI_ENABLED) != 0) {
                        ...
                        if (vbeRegs[VBE_DISPI_INDEX_BPP] == 4)
                            ...
                            vbeLineOffset = vbeRegs[VBE_DISPI_INDEX_XRES]
                                * (vbeRegs[VBE_DISPI_INDEX_BPP] + 7 >>> 3);
    ...
    public static void registerIoPorts() {
