...
public class Gus extends Module_base {
    ...
    private static final byte[] irqtable = { 0, 2, 5, 3, 7, 11, 12, 15 };
    private static final byte[] dmatable = { 0, 1, 3, 5, 6, 7, 0, 0 };
    ...
    private static final Pic.PIC_EventHandler GUS_TimerEvent = val -> {
        ...
        if (myGUS.timers[val].running)
            Pic.PIC_AddEvent(Gus.GUS_TimerEvent, myGUS.timers[val].delay, val);
    };
    private static final IoHandler.IO_ReadHandler read_gus = (port, iolen) -> {
        ...
        switch (port - myGUS.portbase) {
            ...
            case 0x208:
                ...
                if ((tmptime & 0x60) != 0)
                    tmptime |= 1 << 7;
                if ((myGUS.IRQStatus & 0x04) != 0)
                    tmptime |= 1 << 2;
                if ((myGUS.IRQStatus & 0x08) != 0)
                    tmptime |= 1 << 1;
                return tmptime;
            case 0x20a:
    ...
    private static final DMA.DMA_CallBack GUS_DMA_Callback = (chan, event) -> {
        if (event != DMAEvent.DMA_UNMASKED)
            return;
        ...
        if ((myGUS.DMAControl & 0x2) == 0) {
            ...
            if ((myGUS.DMAControl & 0x80) != 0) {
                ...
                if ((myGUS.DMAControl & 0x40) == 0) {
                    ...
                    // 16-bit data
                    for (i = dmaaddr + 1; i < dmaaddr + read; i += 2)
                        GUSRam[i] ^= 0x80;
                }
            }
        } else {
    ...
    private static final IoHandler.IO_WriteHandler write_gus = (port, val16, iolen) -> {
        ...
        switch (port - myGUS.portbase) {
            ...
            case 0x307:
                if (myGUS.gDramAddr < GUSRam.length)
                    GUSRam[(int) myGUS.gDramAddr] = (byte) val;
                break;
            default:
    ...
    private static final Mixer.MIXER_Handler GUS_CallBack = len -> {
    ...
    private static Gus test = null;
    private static final Section.SectionFunction GUS_ShutDown = section -> {
        GUSRam = null;
        test = null;
    };
    public static Section.SectionFunction GUS_Init = section -> {
    ...
    public Gus(Section configuration) {
        ...
        String temp = "SET ULTRASND=" + portat + "," + myGUS.dma1 + "," + myGUS.dma2 + "," + myGUS.irq1 + ","
            + myGUS.irq2;
...
//    	LOG_MSG("Read global reg %x",myGUS.gRegSelect);
        switch (myGUS.gRegSelect) {
            ...
            case 0x80: // Channel voice control read register
                if (curchan != null)
                    return (char) (curchan.ReadWaveCtrl() << 8);
                else
                    return 0x0300;

            case 0x82: // Channel MSB start address register
                if (curchan != null)
                    return (char) (curchan.WaveStart >> WAVE_BITS + 16);
                else
                    return 0x0000;
            case 0x83: // Channel LSW start address register
                if (curchan != null)
                    return (char) (curchan.WaveStart >> WAVE_BITS);
                else
                    return 0x0000;

            case 0x89: // Channel volume register
                if (curchan != null)
                    return (char) (curchan.RampVol >> RAMP_FRACT << 4);
                else
                    return 0x0000;
            case 0x8a: // Channel MSB current address register
                if (curchan != null)
                    return (char) (curchan.WaveAddr >> WAVE_BITS + 16);
                else
                    return 0x0000;
            case 0x8b: // Channel LSW current address register
                if (curchan != null)
                    return (char) (curchan.WaveAddr >> WAVE_BITS);
                else
                    return 0x0000;

            case 0x8d: // Channel volume control register
                if (curchan != null)
                    return (char) (curchan.ReadRampCtrl() << 8);
                else
                    return 0x0300;
            case 0x8f: // General channel IRQ status register
                ...
                if ((myGUS.RampIRQ & mask) == 0)
                    tmpreg |= 0x40;
                if ((myGUS.WaveIRQ & mask) == 0)
                    tmpreg |= 0x80;
                myGUS.RampIRQ &= ~mask;
                myGUS.WaveIRQ &= ~mask;
                CheckVoiceIrq();
...
//    	if (myGUS.gRegSelect|1!=0x44) LOG_MSG("write global register %x with %x", myGUS.gRegSelect, myGUS.gRegData);
        switch (myGUS.gRegSelect) {
            case 0x0: // Channel voice control register
                if (curchan != null)
                    curchan.WriteWaveCtrl((short) (myGUS.gRegData >> 8));
                break;
            ...
            case 0xC: // Channel pan pot register
                if (curchan != null)
                    curchan.WritePanPot((short) (myGUS.gRegData >> 8));
                break;
            case 0xD: // Channel volume control register
                if (curchan != null)
                    curchan.WriteRampCtrl((short) (myGUS.gRegData >> 8));
                break;
            case 0xE: // Set active channel register
                ...
                myGUS.basefreq = (long) (1000000.0 / (1.619695497d * myGUS.ActiveChannels));
                //Log.log_msg("GUS set to " + myGUS.ActiveChannels + " channels");
                for (int i = 0; i < myGUS.ActiveChannels; i++)
            ...
            case 0x41: // Dma control register
                ...
                DMA.GetDMAChannel(myGUS.dma1)
                    .Register_Callback((myGUS.DMAControl & 0x1) != 0 ? GUS_DMA_Callback : null);
                break;
            ...
            case 0x49: // DMA sampling control register
                ...
                DMA.GetDMAChannel(myGUS.dma1)
                    .Register_Callback((myGUS.SampControl & 0x1) != 0 ? GUS_DMA_Callback : null);
                break;
    ...
    private static void MakeTables() {
        ...
        for (i = 1; i < 16; i++) {
            pantable[i] = (long) (-128.0 * (Math.log(i / 15.0) / Math.log(2.0)) * (1 << RAMP_FRACT));
        }
    }

    private static class GusTimer {
        short value;
        boolean reached;
        boolean raiseirq;
        boolean masked;
    ...
    private class GUSChannels {
        ...
        private void WriteWaveFreq(char val) {
            ...
            double realadd = frameadd * myGUS.basefreq / myGUS.rate * (1 << WAVE_FRACT);
        ...
        private void WriteRampRate(short val) {
            ...
            double realadd = frameadd * myGUS.basefreq / myGUS.rate * (1 << RAMP_FRACT);
