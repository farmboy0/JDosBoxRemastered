...
public class TandySound extends Module_base {
    ...
    */
    ...
    private static final IoHandler.IO_ReadHandler TandyDACRead = (port, iolen) -> {
        switch (port) {
            case 0xc4:
    ...
    private static final SN76496 sn = new SN76496();
    ...
    private static final Section.SectionFunction TANDYSOUND_ShutDown = section -> test = null;
    public static Section.SectionFunction TANDYSOUND_Init = section -> {
...
//				ReadHandler[3].Install(0x1e4,TandyDACRead,IO_MB,4);
        ...
        switch (tandy.dac.mode & 3) {
            ...
            case 3:
                ...
                if (tandy.dac.frequency != 0) {
                    float freq = 3579545.0f / tandy.dac.frequency;
                    tandy.dac.chan.SetFreq((/*Bitu*/int) freq);
                    float vol = tandy.dac.amplitude / 7.0f;
    ...
    private static void TandyDACGenerateDMASound(/*Bitu*/int length) {
        if (length != 0) {
            /*Bitu*/
            int read = tandy.dac.dma.chan.Read(length, tandy.dac.dma.buf, 0);
            tandy.dac.chan.AddSamples_m8(read, tandy.dac.dma.buf);
            if (read < length) {
                if (read > 0)
                    tandy.dac.dma.last_sample[0] = tandy.dac.dma.buf[read - 1];
