                ...
                if (VGA.vga.gfx.index == 9 && !index9warned) {
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                            "VGA:3CF:Write " + Integer.toString(val, 16) + " to illegal index 9");
                    index9warned = true;
                    break;
                }
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:3CF:Write " + Integer.toString(val, 16) + " to illegal index "
                            + Integer.toString(VGA.vga.gfx.index, 16));
                break;
        }
    };
    ...
    private static final IoHandler.IO_ReadHandler read_p3cf = (port, iolen) -> {
        switch (VGA.vga.gfx.index) {
            ...
            default:
                ...
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL, "Reading from illegal index "
                        + Integer.toString(VGA.vga.gfx.index, 16) + " in port " + Integer.toString(port, 16));
                break;
        }
