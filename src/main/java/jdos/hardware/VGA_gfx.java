...
public class VGA_gfx {
    ...
    private static final IoHandler.IO_WriteHandler write_p3ce = new IoHandler.IO_WriteHandler() {
        public void call(/*Bitu*/int port, /*Bitu*/int val, /*Bitu*/int iolen) {
            VGA.vga.gfx.index = (short) (val & 0x0f);
        }
    };

    private static final IoHandler.IO_ReadHandler read_p3ce = new IoHandler.IO_ReadHandler() {
        public /*Bitu*/int call(/*Bitu*/int port, /*Bitu*/int iolen) {
            return VGA.vga.gfx.index;
        }
    };

    private static final IoHandler.IO_WriteHandler write_p3cf = new IoHandler.IO_WriteHandler() {
        public void call(/*Bitu*/int port, /*Bitu*/int val, /*Bitu*/int iolen) {
                ...
                default:
                    ...
                    if (VGA.vga.gfx.index == 9 && !index9warned) {
                        if (Log.level <= LogSeverities.LOG_NORMAL)
                            Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL, "VGA:3CF:Write " + Integer.toString(val, 16) + " to illegal index 9");
                        index9warned = true;
                        break;
                    }
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL, "VGA:3CF:Write " + Integer.toString(val, 16) + " to illegal index " + Integer.toString(VGA.vga.gfx.index, 16));
                    break;
            }
        }
    };

    private static final IoHandler.IO_ReadHandler read_p3cf = new IoHandler.IO_ReadHandler() {
        public /*Bitu*/int call(/*Bitu*/int port, /*Bitu*/int iolen) {
            switch (VGA.vga.gfx.index) {
                case 0:    /* Set/Reset Register */
                ...
                default:
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL, "Reading from illegal index " + Integer.toString(VGA.vga.gfx.index, 16) + " in port " + Integer.toString(port, 16));
                    break;
            }
