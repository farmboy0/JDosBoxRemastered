    ...
    private static final IoHandler.IO_WriteHandler MPU401_WriteCommand = new IoHandler.IO_WriteHandler() {
        public void call(/*Bitu*/int port, /*Bitu*/int val, /*Bitu*/int iolen) {
            ...
            if (val <= 0x2f) {
                ...
                if ((val & 0x20) != 0) if (Log.level <= LogSeverities.LOG_ERROR)
                    Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR, "MPU-401:Unhandled Recording Command " + Integer.toString(val, 16));
                switch (val & 0xc) {
                    case 0x4:    /* Stop */
                        Pic.PIC_RemoveEvents(MPU401_Event);
                ...
                switch (val) {
                    ...
                    case 0xff:    /* Reset MPU-401 */
                        if (Log.level <= LogSeverities.LOG_NORMAL)
                            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL, "MPU-401:Reset " + Integer.toString(val, 16));
                    ...
                    case 0x3f:    /* UART mode */
                        if (Log.level <= LogSeverities.LOG_NORMAL)
                            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL, "MPU-401:Set UART mode " + Integer.toString(val, 16));
                        mpu.mode = M_UART;
                        break;
                    default:
