...
public class MPU401 extends Module_base {
    ...
    private static final IoHandler.IO_ReadHandler MPU401_ReadStatus = (port, iolen) -> {
        /*Bit8u*/
        short ret = 0x3f; /* Bits 6 and 7 clear */
    ...
    private static final Pic.PIC_EventHandler MPU401_EOIHandler = val -> {
        ...
        if (!mpu.state.playing || mpu.state.req_mask == 0)
            return;
        /*Bitu*/
        int i = 0;
        do {
    ...
    private static final Pic.PIC_EventHandler MPU401_Event = val -> {
        if (mpu.mode == M_UART)
            return;
        if (!mpu.state.irq_pending) {
            ...
            if (!mpu.state.irq_pending && mpu.state.req_mask != 0)
                MPU401_EOIHandler.call(0);
        }
        Pic.PIC_RemoveEvents(MPU401.MPU401_Event);
        /*Bitu*/
        int new_time;
        if ((new_time = mpu.clock.tempo * mpu.clock.timebase) == 0)
            return;
        Pic.PIC_AddEvent(MPU401.MPU401_Event, MPU401_TIMECONSTANT / new_time);
    };
    private static final Pic.PIC_EventHandler MPU401_ResetDone = val -> {
        mpu.state.reset = false;
        if (mpu.state.cmd_pending != 0) {
            MPU401.MPU401_WriteCommand.call(0x331, mpu.state.cmd_pending - 1, 1);
            mpu.state.cmd_pending = 0;
        }
    };
    private static final IoHandler.IO_WriteHandler MPU401_WriteCommand = (port, val, iolen) -> {
        if (mpu.state.reset) {
        ...
        if (val <= 0x2f) {
            switch (val & 3) { /* MIDI stop, start, continue */
                case 1: {
                    Midi.MIDI_RawOutByte(0xfc);
                    break;
                }
                case 2: {
                    Midi.MIDI_RawOutByte(0xfa);
                    break;
                }
                case 3: {
                    Midi.MIDI_RawOutByte(0xfb);
                    break;
                }
            }
            if ((val & 0x20) != 0)
                if (Log.level <= LogSeverities.LOG_ERROR)
                    Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_ERROR,
                        "MPU-401:Unhandled Recording Command " + Integer.toString(val, 16));
            switch (val & 0xc) {
                case 0x4: /* Stop */
            ...
            switch (val) {
                ...
                case 0xff: /* Reset MPU-401 */
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                            "MPU-401:Reset " + Integer.toString(val, 16));
                ...
                case 0x3f: /* UART mode */
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL,
                            "MPU-401:Set UART mode " + Integer.toString(val, 16));
                    mpu.mode = M_UART;
                    break;
                default:
    ...
    private static final IoHandler.IO_WriteHandler MPU401_WriteData = (port, val, iolen) -> {
        if (mpu.mode == M_UART) {
    ...
    private static final IoHandler.IO_ReadHandler MPU401_ReadData = (port, iolen) -> {
        /*Bit8u*/
        short ret = MSG_MPU_ACK;
        if (mpu.queue_used != 0) {
        ...
        if (ret == MSG_MPU_COMMAND_REQ) {
            ...
            if (mpu.condbuf.type != T_OVERFLOW) {
                ...
                if (mpu.state.command_byte != 0)
                    MPU401_WriteData.call(0x330, mpu.condbuf.value[1], 1);
            }
            mpu.condbuf.type = T_OVERFLOW;
        }
    ...
    public static Section.SectionFunction MPU401_Destroy = section -> {
        if (!test.installed)
            return;
    ...
    public static Section.SectionFunction MPU401_Init = section -> {
    ...
    public MPU401(Section configuration) {
        ...
        if (s_mpu.equalsIgnoreCase("none") || s_mpu.equalsIgnoreCase("off") || s_mpu.equalsIgnoreCase("false") || !Midi.MIDI_Available())
            return;
    ...
    private static void QueueByte(/*Bit8u*/int data) {
        ...
        if (mpu.queue_used < MPU401_QUEUE) {
            ...
            Log.log(LogTypes.LOG_MISC, LogSeverities.LOG_NORMAL, "MPU401:Data queue full");
    }
