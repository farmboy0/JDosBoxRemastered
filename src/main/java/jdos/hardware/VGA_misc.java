...
public class VGA_misc {
    public static IoHandler.IO_ReadHandler vga_read_p3da = (port, iolen) -> {
        /*Bit8u*/
        int retval = 0;
    ...
    private static final IoHandler.IO_ReadHandler read_p3cc = (port, iolen) -> VGA.vga.misc_output;

    // VGA feature control register
    private static final IoHandler.IO_ReadHandler read_p3ca = (port, iolen) -> 0;

    private static final IoHandler.IO_ReadHandler read_p3c8 = (port, iolen) -> 0x10;
    ...
    private static final IoHandler.IO_ReadHandler read_p3c2 = (port, iolen) -> {
        ...
        if (Dosbox.machine == MachineType.MCH_EGA)
            ...
            retval = 0x60;
        if (Dosbox.machine == MachineType.MCH_VGA || (VGA.vga.misc_output >> 2 & 3) == 0
            || (VGA.vga.misc_output >> 2 & 3) == 3) {
            retval |= 0x10;
        }
