...
public class VGA_paradise {
    ...
    static private final VGA.tWritePort write_p3cf_pvga1a = new VGA.tWritePort() {
            ...
            switch (reg) {
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL, "VGA:GFX:PVGA1A:Write to illegal index " + Integer.toString(reg, 16));
                    break;
            }
        }
    };
    static private final VGA.tReadPort read_p3cf_pvga1a = new VGA.tReadPort() {
        public /*Bitu*/int call(/*Bitu*/int reg,/*Bitu*/int iolen) {
            ...
            switch (reg) {
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL, "VGA:GFX:PVGA1A:Read from illegal index " + Integer.toString(reg, 16));
                    break;
            }
            return 0x0;
        }
    };
    private static final VGA.tFinishSetMode FinishSetMode_PVGA1A = new VGA.tFinishSetMode() {
    ...
    private static final VGA.tDetermineMode DetermineMode_PVGA1A = new VGA.tDetermineMode() {
        public void call() {
    ...
    private static final VGA.tSetClock SetClock_PVGA1A = new VGA.tSetClock() {
    ...
    private static final VGA.tGetClock GetClock_PVGA1A = new VGA.tGetClock() {
        public /*Bitu*/int call() {
            return pvga1a.clockFreq[(VGA.vga.misc_output >> 2) & 3];
        }
    };
    private static final VGA.tAcceptsMode AcceptsMode_PVGA1A = new VGA.tAcceptsMode() {
