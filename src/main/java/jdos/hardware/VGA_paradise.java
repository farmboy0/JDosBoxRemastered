...
public class VGA_paradise {
    ...
    private static final VGA.tWritePort write_p3cf_pvga1a = (reg, val, iolen) -> {
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:GFX:PVGA1A:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
    private static final VGA.tReadPort read_p3cf_pvga1a = (reg, iolen) -> {
        if (pvga1a.locked() && reg >= 0x09 && reg <= 0x0e)
            return 0x0;
        ...
        switch (reg) {
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:GFX:PVGA1A:Read from illegal index " + Integer.toString(reg, 16));
                break;
        }
        return 0x0;
    };
    ...
    private static final VGA.tDetermineMode DetermineMode_PVGA1A = () -> {
        ...
        if ((VGA.vga.attr.mode_control & 1) != 0) {
            if ((VGA.vga.gfx.mode & 0x40) != 0)
                VGA.VGA_SetMode(pvga1a.biosMode <= 0x13 ? VGA.M_VGA : VGA.M_LIN8);
                ...
                VGA.VGA_SetMode(pvga1a.biosMode <= 0x13 ? VGA.M_EGA : VGA.M_LIN4);
        } else {
