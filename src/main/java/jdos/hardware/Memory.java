...
public class Memory extends Module_base {
    ...
    static Memory test;
    public static Section.SectionFunction MEM_ShutDown = section -> {
        RAM.free();
        test = null;
    };

    /* The folowing functions are all shortcuts to the above functions using physical addressing */
    public static Section.SectionFunction MEM_Init = section -> {
    ...
    private static final IoHandler.IO_WriteHandler write_p92 = (port, val, iolen) -> {
    ...
    private static final IoHandler.IO_ReadHandler read_p92 = (port, iolen) -> memory.a20.controlport | (memory.a20.enabled ? 0x02 : 0);
    ...
    public Memory(Section configuration) {
        ...
        try {
            ...
            videoCacheSize *= 1024;
            if (videoCacheSize == 0)
                videoCacheSize = videosize * 2;
            videosize += videoCacheSize;
            System.out.println(
                "About to allocate memory " + (highwaterMark + EXTRA_MEM + VGA_draw.TEMPLINE_SIZE + videosize) / 1024
                    + "kb: " + Runtime.getRuntime().freeMemory() / 1024 + "kb free");
    ...
    */
    ...
    public static void MEM_AddROM(/*Bitu*/int page, /*Bitu*/int pages, byte[] data) {
    ...
    public static void MEM_AddPCIPageHandler(PCI_PageHandler handler) {
        memory.pci.add(handler);
    }

    public static void MEM_SetLFB(/*Bitu*/int page, /*Bitu*/int pages, Paging.PageHandler handler,
    ...
    public static Paging.PageHandler MEM_GetPageHandler(/*Bitu*/int phys_page) {
    ...
    public static void MEM_SetPageHandler(/*Bitu*/int phys_page, /*Bitu*/int pages, Paging.PageHandler handler) {
        for (; pages > 0; pages--) {
    ...
    public static void MEM_ResetPageHandler(/*Bitu*/int phys_page, /*Bitu*/int pages) {
        for (; pages > 0; pages--) {
    ...
    public static /*Bitu*/int mem_strlen(/*PhysPt*/int pt) {
        /*Bitu*/
        int x = 0;
        while (x < 1024) {
            if (Paging.mem_readb_inline(pt + x) == 0)
                return x;
            x++;
        }
    ...
    private static void mem_strcpy(/*PhysPt*/int dest, /*PhysPt*/int src) {
        /*Bit8u*/
        int r;
        while ((r = mem_readb(src++)) != 0)
            Paging.mem_writeb_inline(dest++, r);
        Paging.mem_writeb_inline(dest, (short) 0);
    }

    public static void mem_memmove(/*PhysPt*/int dest, /*PhysPt*/int src, /*Bitu*/int size) {
        while (size-- != 0)
            Paging.mem_writeb_inline(dest++, Paging.mem_readb_inline(src++));
    }

    public static void mem_memcpy(/*PhysPt*/int dest, /*PhysPt*/int src, /*Bitu*/int size) {
        while (size-- != 0)
            Paging.mem_writeb_inline(dest++, Paging.mem_readb_inline(src++));
    }

    public static void mem_memcpy(byte[] dest, int destOffset, /*PhysPt*/int src, /*Bitu*/int size) {
        while (size-- != 0)
            dest[destOffset++] = (byte) Paging.mem_readb_inline(src++);
    }

    public static void mem_memcpy(/*PhysPt*/int dest, byte[] src, int srcOffset, /*Bitu*/int size) {
        while (size-- != 0)
            Paging.mem_writeb_inline(dest++, src[srcOffset++]);
    }

    public static void mem_zero(int dest, int len) {
        while (len-- != 0)
            Paging.mem_writeb_inline(dest++, 0);
    }

    public static void mem_memset(int dest, int value, int len) {
        while (len-- != 0)
            Paging.mem_writeb_inline(dest++, value);
    }

    public static void phys_zero(int dest, int len) {
        while (len-- != 0)
            RAM.writeb(dest++, (short) 0);
    }

    public static void phys_memcpy(int dest, byte[] buffer, int offset, int len) {
        while (len-- != 0)
            RAM.writeb(dest++, buffer[offset++]);
    }

    public static void MEM_BlockRead(/*PhysPt*/int pt, short[] data, int offset, /*Bitu*/int size) {
    ...
    public static void MEM_BlockRead16u(/*PhysPt*/int pt, int[] data, int offset, /*Bitu*/int size) {
        for (int i = 0; i < size; i++) {
            int v1 = Paging.mem_readb_inline(pt++);
            int v2 = Paging.mem_readb_inline(pt++);
    ...
    public static void MEM_BlockRead(/*PhysPt*/int pt, short[] data, /*Bitu*/int size) {
        for (int i = 0; i < size; i++) {
            int v1 = Paging.mem_readb_inline(pt++);
            int v2 = Paging.mem_readb_inline(pt++);
    ...
    public static String MEM_BlockRead(/*PhysPt*/int pt, /*Bitu*/int size) {
        byte[] b = new byte[size];
        MEM_BlockRead(pt, b, size);
        return new String(b, 0, StringHelper.strlen(b));
    }

    public static void MEM_BlockRead(/*PhysPt*/int pt, byte[] data, /*Bitu*/int size) {
        for (int i = 0; i < size; i++) {
            data[i] = (byte) (Paging.mem_readb_inline(pt++) & 0xFF);
        }
    }

    public static void MEM_BlockRead(/*PhysPt*/int pt, byte[] data, int offset, /*Bitu*/int size) {
        for (int i = 0; i < size; i++) {
            data[i + offset] = (byte) (Paging.mem_readb_inline(pt++) & 0xFF);
        }
    }

    public static void MEM_BlockWrite(/*PhysPt*/int pt, byte[] read, /*Bitu*/int size) {
        int i;
    ...
    public static void MEM_BlockWrite(/*PhysPt*/int pt, byte[] read, int offset, /*Bitu*/int size) {
        int i;
        for (i = 0; i < size && i < read.length; i++) {
    ...
    public static void MEM_BlockWrite(/*PhysPt*/int pt, String data, /*Bitu*/int size) {
        byte[] read = data.getBytes();
        MEM_BlockWrite(pt, read, size);
    }

    public static void MEM_BlockCopy(/*PhysPt*/int dest, /*PhysPt*/int src, /*Bitu*/int size) {
        mem_memcpy(dest, src, size);
    }

    public static String MEM_StrCopy(/*PhysPt*/int pt, /*Bitu*/int size) {
    ...
    public static /*Bitu*/int MEM_TotalPages() {
        return memory.pages;
    }

    public static /*Bitu*/int MEM_ExtraPages() {
        return highwaterMark * 4 / 4096 - memory.pages;
    }

    //TODO Maybe some protection for this whole allocation scheme

    public static /*Bitu*/int MEM_FreeLargest() {
        /*Bitu*/
        int size = 0;/*Bitu*/
        int largest = 0;
        /*Bitu*/
    ...
    public static /*Bitu*/int MEM_FreeTotal() {
        /*Bitu*/
        int free = 0;
        /*Bitu*/
        int index = Paging.XMS_START;
    ...
    private static /*Bitu*/int MEM_AllocatedPages(/*MemHandle*/int handle) {
        /*Bitu*/
        int pages = 0;
        while (handle > 0) {
            pages++;
    ...
    private static /*Bitu*/int BestMatch(/*Bitu*/int size) {
        /*Bitu*/
        int index = Paging.XMS_START;
        /*Bitu*/
        int first = 0;
    ...
    public static /*MemHandle*/int MEM_AllocatePages(/*Bitu*/int pages, boolean sequence) {
        /*MemHandle*/
        int ret = -1;
        if (pages == 0)
            return 0;
    ...
    public static /*MemHandle*/int MEM_GetNextFreePage() {
        return BestMatch(1);
    }

    public static void MEM_ReleasePages(/*MemHandle*/int handle) {
        while (handle > 0) {
            /*MemHandle*/
    ...
    public static boolean MEM_ReAllocatePages(/*MemHandle*/IntRef handle, /*Bitu*/int pages, boolean sequence) {
        if (handle.value <= 0) {
            if (pages == 0)
                return true;
    ...
    */
    ...
    public static void MEM_A20_Enable(boolean enabled) {
        ...
        for (/*Bitu*/int i = 0; i < 16; i++)
            Paging.PAGING_MapPage(1024 / 4 + i, phys_base + i);
    ...
    public static /*Bit32u*/long mem_readq(/*PhysPt*/int address) {
        return Paging.mem_readd_inline(address) & 0xFFFFFFFFL
            | (Paging.mem_readd_inline(address + 4) & 0xFFFFFFFFL) << 32;
    }

    public static void mem_writeb(/*PhysPt*/int address, /*Bit8u*/int val) {
        Paging.mem_writeb_inline(address, (short) val);
    }

    public static void mem_writew(/*PhysPt*/int address, /*Bit16u*/int val) {
        Paging.mem_writew_inline(address, val);
    }

    public static void mem_writed(/*PhysPt*/int address, /*Bit32u*/int val) {
        Paging.mem_writed_inline(address, val);
    }

    public static void mem_writeq(/*PhysPt*/int address, long val) {
    ...
    private static class IllegalPageHandler extends Paging.PageHandler {
        ...
        public /*Bitu*/int readb(/*PhysPt*/int addr) {
            if (Config.C_DEBUG)
                Log.log_msg(
                    StringHelper.sprintf("Illegal read from %x, CS:IP %8x:%8x", new Object[] { Integer.valueOf(addr),
                        Integer.valueOf(CPU_Regs.reg_csVal.dword), Integer.valueOf(CPU_Regs.reg_eip) }));
            else {
                if (r_lcount < 1000) {
                    r_lcount++;
                    Log.log_msg(StringHelper.sprintf("Illegal read from %x, CS:IP %8x:%8x",
                        new Object[] { Integer.valueOf(addr), Integer.valueOf(CPU_Regs.reg_csVal.dword),
                            Integer.valueOf(CPU_Regs.reg_eip) }));
                }
            }
            return 0;
        }
        ...
        public void writeb(/*PhysPt*/int addr, /*Bitu*/int val) {
            if (Config.C_DEBUG)
                Log.log_msg(
                    StringHelper.sprintf("Illegal write to %x, CS:IP %8x:%8x", new Object[] { Integer.valueOf(addr),
                        Integer.valueOf(CPU_Regs.reg_csVal.dword), Integer.valueOf(CPU_Regs.reg_eip) }));
            else {
        ...
        public void writeb(/*PhysPt*/int addr, /*Bitu*/int val) {
            if (Log.level <= LogSeverities.LOG_ERROR)
                Log.log(LogTypes.LOG_CPU, LogSeverities.LOG_ERROR,
                    "Write " + Integer.toString(val, 16) + " to rom at " + Integer.toString(addr, 16));
        }
