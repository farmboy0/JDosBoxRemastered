...
public class VGA_s3 {
            ...
            */
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:S3:CRTC:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
    ...
    private static final VGA.tReadPort SVGA_S3_ReadCRTC = (reg, iolen) -> {
        switch (reg) {
            ...
            case 0x51: /* Extended System Control 2 */
                return VGA.vga.config.display_start >> 16 & 3 | (VGA.vga.svga.bank_read & 0x30) >> 2
                    | (VGA.vga.config.scan_len & 0x300) >> 4 | VGA.vga.s3.reg_51;
    ...
    private static final VGA.tWritePort SVGA_S3_WriteSEQ = (reg, val, iolen) -> {
        ...
        switch (reg) {
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:S3:SEQ:Write to illegal index " + Integer.toString(reg, 16));
                break;
        }
    };
    ...
    private static final VGA.tReadPort SVGA_S3_ReadSEQ = (reg, iolen) -> {
        ...
        switch (reg) {
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "VGA:S3:SEQ:Read from illegal index " + Integer.toString(reg, 16));
                return 0;
        }
    };
