...
public class Keyboard {
    public static final int ACMD_NONE = 0;
    public static final int ACMD_SET_RATE = 1;
    public static final int ACMD_SET_RESOLUTION = 2;
    private static final int KEYBUFSIZE = 32;
    private static final float KEYDELAY = 0.300f; //Considering 20-30 khz serial clock and 11 bits/char
    private static final int AUX = 0x100;
    private static final int RESETDELAY = 100;
    ...
    private static final Pic.PIC_EventHandler KEYBOARD_ResetDelay = val -> {
        keyb.reset = false;
        KEYBOARD_SetLEDs(0);
    ...
    private static final IoHandler.IO_WriteHandler write_p60 = (port, val, iolen) -> {
        switch (keyb.command) {
                ...
                switch (val) {
                    ...
                    default:
                        ...
                        if (Log.level <= LogSeverities.LOG_ERROR)
                            Log.log(LogTypes.LOG_KEYBOARD, LogSeverities.LOG_ERROR,
                                "60:Unhandled command " + Integer.toString(val, 16));
                        KEYBOARD_AddBuffer(0xfa); /* Acknowledge */
                }
                return;
            case KeyCommands.CMD_SETSCANSET:
                ...
                if (val == 0) { /* just asking */
                    if (keyb.cb_xlat) {
                        switch (keyb.scanset) {
                            case 1:
                                KEYBOARD_AddBuffer(0x43);
                                break;
                            case 2:
                                KEYBOARD_AddBuffer(0x41);
                                break;
                            case 3:
                                KEYBOARD_AddBuffer(0x3F);
                                break;
                        }
                    } else {
            ...
            case KeyCommands.CMD_SETTYPERATE: {
                ...
                final int[] repeat = { 33, 37, 42, 46, 50, 54, 58, 63, 67, 75, 83, 92, 100, 109, 118, 125, 133, 149,
    ...
    private static final IoHandler.IO_WriteHandler write_p61 = (port, val, iolen) -> {
        if (((port_61_data ^ val) & 3) != 0) {
            if (((port_61_data ^ val) & 1) != 0)
                Timer.TIMER_SetGate2((val & 0x1) != 0);
        ...
        switch (val) {
            case 0x20: /* read command byte */
                KEYBOARD_Add8042Response(
                    (keyb.cb_xlat ? 1 : 0) << 6 | (keyb.auxactive ? 0 : 1) << 5 | (keyb.active ? 0 : 1) << 4
                        | (keyb.cb_sys ? 1 : 0) << 2 | (keyb.cb_irq12 ? 1 : 0) << 1 | (keyb.cb_irq1 ? 1 : 0));
                break;
            case 0x60:
            ...
            case 0xd3: /* write AUX output */
                if (keyb.enable_aux)
                    ...
                    Log.log(LogTypes.LOG_KEYBOARD, LogSeverities.LOG_NORMAL,
                        "Program is writing 8042 AUX. If you intend to use PS/2 mouse emulation you may consider adding aux=1 to your dosbox.conf");
                break;
    ...
    private static final Timer.TIMER_TickHandler KEYBOARD_TickHandler = () -> {
    ...
    public static Section.SectionFunction KEYBOARD_ShutDown = section -> {
    ...
    public static Section.SectionFunction KEYBOARD_Init = sec -> {
        keyb = new Keyb();
        ...
        if (sbtype != null) {
            if (sbtype.equals("2button"))
                ...
                keyb.ps2mouse.type = MOUSE_INTELLIMOUSE;
                Log.log(LogTypes.LOG_KEYBOARD, LogSeverities.LOG_ERROR,
                    "Assuming PS/2 intellimouse, I don't know what '" + sbtype + "' is");
            }
        }
    ...
    public static void KEYBOARD_AUX_Event(float x1, float y1, int buttons) {
        ...
        if (keyb.ps2mouse.reporting && keyb.ps2mouse.mode == MM_STREAM) {
            if (keyb.used + 4 < KEYBUFSIZE) {
                ...
                KEYBOARD_AddBuffer(AUX | (y == -256 || y == 255 ? 0x80 : 0x00) | /* Y overflow */
    ...
    public static boolean KEYBOARD_AUX_Active() {
    ...
    private static void updateIRQ() {
        if (keyb.p60changed || keyb.buf8042_len > 0) {
            ...
            if (Dosbox.machine == MachineType.MCH_PCJR)
                Pic.PIC_DeActivateIRQ(6);
            else
                Pic.PIC_DeActivateIRQ(1);
        }
    }
        ...
        switch (keyb.aux_command) {
            case ACMD_NONE:
                switch (val) {
                    ...
                    case 0xeb: /* read data */
                        KEYBOARD_AddBuffer(AUX | 0xfa); /* ack */
                        KEYBOARD_AUX_Event(0, 0, (keyb.ps2mouse.m ? 1 : 0) << 2 | (keyb.ps2mouse.r ? 1 : 0) << 1
                    ...
                    case 0xe9: /* status request */
                        KEYBOARD_AddBuffer(AUX | 0xfa); /* ack */
                        KEYBOARD_AddBuffer(AUX | (keyb.ps2mouse.mode == MM_REMOTE ? 0x40 : 0x00)
                            | (keyb.ps2mouse.reporting ? 1 : 0) << 5 | (keyb.ps2mouse.scale21 ? 1 : 0) << 4
                            | (keyb.ps2mouse.m ? 1 : 0) << 2 | (keyb.ps2mouse.r ? 1 : 0) << 1
                            | (keyb.ps2mouse.l ? 1 : 0) << 0);
                ...
                if (keyb.ps2mouse.type >= MOUSE_INTELLIMOUSE) {
                    if (keyb.ps2mouse.last_srate[0] == 200 && keyb.ps2mouse.last_srate[2] == 80) {
                        if (keyb.ps2mouse.last_srate[1] == 100) {
                            ...
                            if (!keyb.ps2mouse.intellimouse_btn45) {
                                Log.log(LogTypes.LOG_KEYBOARD, LogSeverities.LOG_NORMAL,
                                    "Intellimouse 4/5-button mode enabled");
            ...
            case ACMD_SET_RESOLUTION:
                ...
                Log.log(LogTypes.LOG_KEYBOARD, LogSeverities.LOG_NORMAL,
                    "PS/2 mouse resolution set to " + (1 << (val & 3)));
                break;
        }
    }
    ...
    public static void KEYBOARD_AddKey(int keytype, boolean pressed) {
        ...
        switch (keytype) {
            ...
            case KBD_KEYS.KBD_minus:
                ret = 12;
                break;
            case KBD_KEYS.KBD_equals:
                ret = 13;
                break;
            case KBD_KEYS.KBD_backspace:
                ret = 14;
                break;
            case KBD_KEYS.KBD_tab:
                ret = 15;
                break;
            ...
            case KBD_KEYS.KBD_leftbracket:
                ret = 26;
                break;
            case KBD_KEYS.KBD_rightbracket:
                ret = 27;
                break;
            case KBD_KEYS.KBD_enter:
                ret = 28;
                break;
            case KBD_KEYS.KBD_leftctrl:
                ret = 29;
                break;
            ...
            case KBD_KEYS.KBD_semicolon:
                ret = 39;
                break;
            case KBD_KEYS.KBD_quote:
                ret = 40;
                break;
            case KBD_KEYS.KBD_grave:
                ret = 41;
                break;
            case KBD_KEYS.KBD_leftshift:
                ret = 42;
                break;
            case KBD_KEYS.KBD_backslash:
                ret = 43;
                break;
            case KBD_KEYS.KBD_z:
                ret = 44;
            ...
            case KBD_KEYS.KBD_comma:
                ret = 51;
                break;
            case KBD_KEYS.KBD_period:
                ret = 52;
                break;
            case KBD_KEYS.KBD_slash:
                ret = 53;
                break;
            case KBD_KEYS.KBD_rightshift:
                ret = 54;
                break;
            case KBD_KEYS.KBD_kpmultiply:
                ret = 55;
                break;
            case KBD_KEYS.KBD_leftalt:
                ret = 56;
                break;
            case KBD_KEYS.KBD_space:
                ret = 57;
                break;
            case KBD_KEYS.KBD_capslock:
                ret = 58;
                break;

            case KBD_KEYS.KBD_f1:
                ret = 59;
                break;
            case KBD_KEYS.KBD_f2:
                ret = 60;
                break;
            case KBD_KEYS.KBD_f3:
                ret = 61;
                break;
            case KBD_KEYS.KBD_f4:
                ret = 62;
                break;
            case KBD_KEYS.KBD_f5:
                ret = 63;
                break;
            case KBD_KEYS.KBD_f6:
                ret = 64;
                break;
            case KBD_KEYS.KBD_f7:
                ret = 65;
                break;
            case KBD_KEYS.KBD_f8:
                ret = 66;
                break;
            case KBD_KEYS.KBD_f9:
                ret = 67;
                break;
            case KBD_KEYS.KBD_f10:
                ret = 68;
                break;

            case KBD_KEYS.KBD_numlock:
                ret = 69;
                break;
            case KBD_KEYS.KBD_scrolllock:
                ret = 70;
                break;

            case KBD_KEYS.KBD_kp7:
                ret = 71;
                break;
            case KBD_KEYS.KBD_kp8:
                ret = 72;
                break;
            case KBD_KEYS.KBD_kp9:
                ret = 73;
                break;
            case KBD_KEYS.KBD_kpminus:
                ret = 74;
                break;
            case KBD_KEYS.KBD_kp4:
                ret = 75;
                break;
            case KBD_KEYS.KBD_kp5:
                ret = 76;
                break;
            case KBD_KEYS.KBD_kp6:
                ret = 77;
                break;
            case KBD_KEYS.KBD_kpplus:
                ret = 78;
                break;
            case KBD_KEYS.KBD_kp1:
                ret = 79;
                break;
            case KBD_KEYS.KBD_kp2:
                ret = 80;
                break;
            case KBD_KEYS.KBD_kp3:
                ret = 81;
                break;
            case KBD_KEYS.KBD_kp0:
                ret = 82;
                break;
            case KBD_KEYS.KBD_kpperiod:
                ret = 83;
                break;

            case KBD_KEYS.KBD_extra_lt_gt:
                ret = 86;
                break;
            case KBD_KEYS.KBD_f11:
                ret = 87;
                break;
            case KBD_KEYS.KBD_f12:
                ret = 88;
                break;

            //The Extended keys

            case KBD_KEYS.KBD_kpenter:
                extend = true;
                ret = 28;
                break;
            case KBD_KEYS.KBD_rightctrl:
                extend = true;
                ret = 29;
                break;
            case KBD_KEYS.KBD_kpdivide:
                extend = true;
                ret = 53;
                break;
            case KBD_KEYS.KBD_rightalt:
                extend = true;
                ret = 56;
                break;
            case KBD_KEYS.KBD_home:
                extend = true;
                ret = 71;
                break;
            case KBD_KEYS.KBD_up:
                extend = true;
                ret = 72;
                break;
            case KBD_KEYS.KBD_pageup:
                extend = true;
                ret = 73;
                break;
            case KBD_KEYS.KBD_left:
                extend = true;
                ret = 75;
                break;
            case KBD_KEYS.KBD_right:
                extend = true;
                ret = 77;
                break;
            case KBD_KEYS.KBD_end:
                extend = true;
                ret = 79;
                break;
            case KBD_KEYS.KBD_down:
                extend = true;
                ret = 80;
                break;
            case KBD_KEYS.KBD_pagedown:
                extend = true;
                ret = 81;
                break;
            case KBD_KEYS.KBD_insert:
                extend = true;
                ret = 82;
                break;
            case KBD_KEYS.KBD_delete:
                extend = true;
                ret = 83;
                break;
    ...
    public static void AUX_INT33_Takeover() {
        if (keyb.ps2mouse.type != MOUSE_NONE && keyb.ps2mouse.int33_taken)
            Log.log(LogTypes.LOG_KEYBOARD, LogSeverities.LOG_NORMAL,
                "PS/2 mouse emulation: Program is using INT 33h, disabling direct AUX emulation");
        keyb.ps2mouse.int33_taken = true;
    }

    private static void KEYBOARD_Reset() {
    ...
    private static final class KeyCommands {
        ...
        public static final int CMD_SETCOMMAND = 4;
        public static final int CMD_WRITEOUTPUT = 5;
        public static final int CMD_WRITEAUXOUT = 6;
        public static final int CMD_SETSCANSET = 7;
        public static final int CMD_WRITEAUX = 8;
    }
    ...
    private static class Keyb {
        /*Bit8u*/ int[] buf8042 = new int[8];
        /* for 8042 responses, taking priority over keyboard responses */
