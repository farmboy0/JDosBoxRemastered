            ...
            if (VGA.vga.draw.cursor.enabled && (VGA.vga.draw.cursor.count & 0x8) != 0) {
                ...
                if (font_addr >= 0 && font_addr < VGA.vga.draw.blocks) {
                    if (line >= VGA.vga.draw.cursor.sline && line <= VGA.vga.draw.cursor.eline) {
                        ...
                        /*Bit32u*/
                        int att = VGA.TXT_FG_Table[RAM.readb(VGA.vga.tandy.draw_base + VGA.vga.draw.cursor.address + 1) & 0xf];
                        RAM.writed(draw, att);
                        draw += 4;
                        RAM.writed(draw, att);
                        draw += 4;
        ...
        public void call(/*Bitu*/int val) {
            ...
            switch (VGA.vga.draw.mode) {
                case VGA.Drawmode.PART:
                    if (VGA.vga.draw.parts_left != 0) {
                        if (Log.level <= LogSeverities.LOG_NORMAL)
                            Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL, "Parts left: " + VGA.vga.draw.parts_left);
                ...
                case VGA.Drawmode.EGALINE:
                    if (VGA.vga.draw.lines_done < VGA.vga.draw.lines_total) {
                        if (Log.level <= LogSeverities.LOG_NORMAL)
                            Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL, "Lines left: %d" + (VGA.vga.draw.lines_total - VGA.vga.draw.lines_done));
    ...
    public static Pic.PIC_EventHandler VGA_SetupDrawing = new Pic.PIC_EventHandler() {
        ...
        public void call(/*Bitu*/int val) {
            ...
            if (Dosbox.IS_EGAVGA_ARCH()) {
                ...
                if ((VGA.vga.seq.clocking_mode & 1) != 0) clock /= 8;
                else clock /= 9;
            ...
            if (Config.C_DEBUG) {
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, "h total " + htotal + " end " + hdend + " blank (" + hbstart + "/" + hbend + ") retrace (" + hrstart + "/" + hrend + ")");
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, "v total " + vtotal + " end " + vdend + " blank (" + vbstart + "/" + vbend + ") retrace (" + vrstart + "/" + vrend + ")");
            }
            if (htotal == 0) return;
            if (vtotal == 0) return;
            ...
            */
            ...
            if (Config.C_DEBUG) {
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, StringHelper.sprintf("h total %2.5f (%3.2fkHz) blank(%02.5f/%02.5f) retrace(%02.5f/%02.5f)",
                ...
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, StringHelper.sprintf("v total %2.5f (%3.2fHz) blank(%02.5f/%02.5f) retrace(%02.5f/%02.5f)",
            ...
            if ((width != VGA.vga.draw.width) ||
                ...
                if (Config.C_DEBUG) {
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, "Width " + width + ", Height " + height + ", fps " + fps);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, (doublewidth ? "double" : "normal") + " width, " + (doubleheight ? "double" : "normal") + " height aspect " + aspect_ratio);
                }
