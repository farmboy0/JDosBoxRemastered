...
public class VGA_draw {
    public static final int TEMPLINE_SIZE = 1920 * 4;
    private static final int VGA_PARTS = 4;
    public static int TempLine;
    private static VGA_Line_Handler VGA_DrawLine;
    private static final VGA_Line_Handler VGA_Draw_1BPP_Line = (vidstart, line) -> {
        ...
        for (/*Bitu*/int x = VGA.vga.draw.blocks; x > 0; x--, vidstart++) {
            /*Bitu*/
            int val = RAM.readb(base + (vidstart & 8 * 1024 - 1));
    ...
    private static final VGA_Line_Handler VGA_Draw_2BPP_Line = (vidstart, line) -> {
        int base = VGA.vga.tandy.draw_base + ((line & VGA.vga.tandy.line_mask) << VGA.vga.tandy.line_shift);
        int draw = TempLine;
        for (/*Bitu*/int x = 0; x < VGA.vga.draw.blocks; x++) {
            /*Bitu*/
            int val = RAM.readb(base + (vidstart & VGA.vga.tandy.addr_mask));
            vidstart++;
    ...
    private static final VGA_Line_Handler VGA_Draw_2BPPHiRes_Line = (vidstart, line) -> {
        int base = VGA.vga.tandy.draw_base + ((line & VGA.vga.tandy.line_mask) << VGA.vga.tandy.line_shift);
        int draw = TempLine;
        for (/*Bitu*/int x = 0; x < VGA.vga.draw.blocks; x++) {
            /*Bitu*/
            int val1 = RAM.readb(base + (vidstart & VGA.vga.tandy.addr_mask));
            ++vidstart;
            /*Bitu*/
            int val2 = RAM.readb(base + (vidstart & VGA.vga.tandy.addr_mask));
            ++vidstart;
    ...
    private static final VGA_Line_Handler VGA_Draw_CGA16_Line = (vidstart, line) -> {
        int base = VGA.vga.tandy.draw_base + ((line & VGA.vga.tandy.line_mask) << VGA.vga.tandy.line_shift);
        int draw = TempLine;
        ...
        for (/*Bitu*/int x = 0; x < VGA.vga.draw.blocks; x++) {
            /*Bitu*/
            int val1 = CGA16_READER(base, vidstart, x);
            /*Bitu*/
            int val2 = val1 & 0xf;
            val1 >>= 4;
            ...
            RAM.writed(draw,
                0x80808080 | temp1 | val1 | (temp2 | val1) << 8 | (temp3 | val1) << 16 | (temp4 | val1) << 24);
            draw += 4;
            ...
            i++;
            RAM.writed(draw,
                0x80808080 | temp1 | val2 | (temp2 | val2) << 8 | (temp3 | val2) << 16 | (temp4 | val2) << 24);
            draw += 4;
        }
        return TempLine;
    };
    private static final VGA_Line_Handler VGA_Draw_4BPP_Line = (vidstart, line) -> {
        int base = VGA.vga.tandy.draw_base + ((line & VGA.vga.tandy.line_mask) << VGA.vga.tandy.line_shift);
        int draw = TempLine;
    ...
    private static final VGA_Line_Handler VGA_Draw_4BPP_Line_Double = (vidstart, line) -> {
        int base = VGA.vga.tandy.draw_base + ((line & VGA.vga.tandy.line_mask) << VGA.vga.tandy.line_shift);
        int draw = TempLine;
    ...
    private static final VGA_Line_Handler VGA_Draw_Linear_Line = (vidstart, line) -> {
        /*Bitu*/
        ...
        if (VGA.vga.draw.linear_mask - offset < VGA.vga.draw.line_length)
            RAM.memcpy(VGA.vga.draw.linear_base + VGA.vga.draw.linear_mask + 1, VGA.vga.draw.linear_base,
                VGA.vga.draw.line_length);
        return VGA.vga.draw.linear_base + offset;
    };
    private static final VGA_Line_Handler VGA_Draw_Xlat16_Linear_Line = (vidstart, line) -> {
    ...
    private static final VGA_Line_Handler VGA_Draw_VGA_Line_HWMouse = (vidstart, line) -> {
        ...
        if (VGA.vga.s3.hgc.posx >= VGA.vga.draw.width || lineat < VGA.vga.s3.hgc.originy
    ...
    private static final VGA_Line_Handler VGA_Draw_LIN16_Line_HWMouse = (vidstart, line) -> {
        if (VGA.svga.hardware_cursor_active == null || !VGA.svga.hardware_cursor_active.call())
            return VGA.vga.mem.linear + vidstart;
        ...
        if (VGA.vga.s3.hgc.posx >= VGA.vga.draw.width || lineat < VGA.vga.s3.hgc.originy
            ...
            for (/*Bitu*/int m = cursorMemStart; m < cursorMemEnd;) {
                // for each byte of cursor data
                /*Bit8u*/
                int bitsA = RAM.readb(VGA.vga.mem.linear + m);
                /*Bit8u*/
                int bitsB = RAM.readb(VGA.vga.mem.linear + m + 2);
                for (/*Bit8u*/int bit = 0x80 >> cursorStartBit; bit != 0; bit >>= 1) {
                    // for each bit
                    cursorStartBit = 0;
                    if ((bitsA & bit) != 0) {
    ...
    private static final VGA_Line_Handler VGA_Draw_LIN32_Line_HWMouse = (vidstart, line) -> {
        if (VGA.svga.hardware_cursor_active == null || !VGA.svga.hardware_cursor_active.call())
            return VGA.vga.mem.linear + vidstart;
    ...
    private static final VGA_Line_Handler VGA_TEXT_Draw_Line = (vidstart, line) -> {
        /*Bits*/
        int font_addr;
        int draw = TempLine;
        ...
        for (/*Bitu*/int cx = 0; cx < VGA.vga.draw.blocks; cx++) {
            /*Bitu*/
            int chr = RAM.readb(vidmem + cx * 2);
            /*Bitu*/
            int col = RAM.readb(vidmem + cx * 2 + 1);
            /*Bitu*/
        ...
        if (VGA.vga.draw.cursor.enabled && (VGA.vga.draw.cursor.count & 0x8) != 0) {
            ...
            if (font_addr >= 0 && font_addr < VGA.vga.draw.blocks) {
                if (line >= VGA.vga.draw.cursor.sline && line <= VGA.vga.draw.cursor.eline) {
                    ...
                    /*Bit32u*/
                    int att = VGA.TXT_FG_Table[RAM.readb(VGA.vga.tandy.draw_base + VGA.vga.draw.cursor.address + 1)
                        & 0xf];
                    RAM.writed(draw, att);
                    draw += 4;
                    RAM.writed(draw, att);
    ...
    private static final VGA_Line_Handler VGA_TEXT_Herc_Draw_Line = (vidstart, line) -> {
        /*Bits*/
        int font_addr;
        int draw = TempLine;
        int vidmem = VGA_Text_Memwrap(vidstart);
        ...
        for (/*Bitu*/int cx = 0; cx < VGA.vga.draw.blocks; cx++) {
            ...
            int chr = RAM.readb(vidmem + cx * 2);
            /*Bitu*/
            int attrib = RAM.readb(vidmem + cx * 2 + 1);
            if ((attrib & 0x77) == 0) {
                ...
                if ((attrib & 0x77) == 0x70) {
                    ...
                    if ((attrib & 0x8) != 0)
                        fg = VGA.TXT_FG_Table[0xf];
                    else
                        fg = VGA.TXT_FG_Table[0x0];
                } else {
                    /*Bitu*/
                    if ((VGA.vga.crtc.underline_location & 0x1f) == line && (attrib & 0x77) == 0x1)
                        underline = true;
                    bg = VGA.TXT_BG_Table[0x0];
                    if ((attrib & 0x8) != 0)
                        fg = VGA.TXT_FG_Table[0xf];
                    else
                        fg = VGA.TXT_FG_Table[0x7];
                }
                /*Bit32u*/
                int mask1, mask2;
                if (underline)
        ...
        if (VGA.vga.draw.cursor.enabled && (VGA.vga.draw.cursor.count & 0x8) == 0) {
            ...
            if (font_addr >= 0 && font_addr < VGA.vga.draw.blocks) {
                if (line >= VGA.vga.draw.cursor.sline && line <= VGA.vga.draw.cursor.eline) {
                    draw = TempLine + font_addr * 8;
                    /*Bit8u*/
                    int attr = RAM.readb(VGA.vga.tandy.draw_base + VGA.vga.draw.cursor.address + 1);
                    /*Bit32u*/
                    int cg;
                    if ((attr & 0x8) != 0) {
    ...
    private static final VGA_Line_Handler VGA_TEXT_Xlat16_Draw_Line = (vidstart, line) -> {
        // keep it aligned:
        ...
        for (/*Bitu*/int cx = 0; cx < blocks; cx++) {
            ...
            int chr = RAM.readb(vidmem + cx * 2);
            /*Bitu*/
            int attr = RAM.readb(vidmem + cx * 2 + 1);
            /*Bitu*/
            ...
            /*Bitu*/
            int foreground = VGA.vga.draw.blink || (attr & 0x80) == 0 ? attr & 0xf : background;
        ...
        if ((VGA.vga.draw.cursor.count & 0x8) != 0 && line >= VGA.vga.draw.cursor.sline
            ...
            /*Bits*/
            int attr_addr = VGA.vga.draw.cursor.address - vidstart >> 1;
            if (attr_addr >= 0 && attr_addr < VGA.vga.draw.blocks) {
                ...
                /*Bitu*/
                int foreground = RAM.readb(VGA.vga.tandy.draw_base + VGA.vga.draw.cursor.address + 1) & 0xf;
        ...
        public void call(/*Bitu*/int val) {
            ...
            if (--VGA.vga.draw.parts_left != 0) {
                Pic.PIC_AddEvent(VGA_DrawPart, (float) VGA.vga.draw.delay.parts,
                    VGA.vga.draw.parts_left != 1 ? VGA.vga.draw.parts_lines : VGA.vga.draw.lines_total
                        - VGA.vga.draw.lines_done);
            } else {
                if (VGA_memory.VGA_KEEP_CHANGES)
                    VGA_ChangesEnd();
    ...
    private static final Pic.PIC_EventHandler VGA_VerticalTimer = new Pic.PIC_EventHandler() {
        @Override
        public String toString() {
        ...
        public void call(/*Bitu*/int val) {
            ...
            if (Dosbox.IS_EGAVGA_ARCH()) {
                ...
                if (Dosbox.svgaCard == SVGACards.SVGA_S3Trio && VGA.vga.config.line_compare == 0)
            ...
            switch (VGA.vga.mode) {
                case VGA.M_EGA:
                    if ((VGA.vga.crtc.mode_control & 0x1) == 0)
                        VGA.vga.draw.linear_mask &= ~0x10000;
                    else
                        VGA.vga.draw.linear_mask |= 0x10000;
                case VGA.M_LIN4:
                ...
                case VGA.M_HERC_TEXT:
                    if (Dosbox.machine == MachineType.MCH_HERC)
                        VGA.vga.draw.linear_mask = 0xfff; // 1 page
                    else if (Dosbox.IS_EGAVGA_ARCH())
                        VGA.vga.draw.linear_mask = 0x7fff; // 8 pages
                    else
                        VGA.vga.draw.linear_mask = 0x3fff; // CGA, Tandy 4 pages
                    ...
                    VGA.vga.draw.blink = VGA.vga.draw.blinking & VGA.vga.draw.cursor.count >> 4 != 0
                        || !VGA.vga.draw.blinking;
                    break;
                case VGA.M_HERC_GFX:
                    break;
                case VGA.M_CGA4:
            ...
            if (VGA.vga.draw.vblank_skip != 0) {
                ...
                VGA.vga.draw.address += VGA.vga.draw.address_add
                    * (VGA.vga.draw.vblank_skip / VGA.vga.draw.address_line_total);
            }
            ...
            switch (VGA.vga.draw.mode) {
                case VGA.Drawmode.PART:
                    if (VGA.vga.draw.parts_left != 0) {
                        if (Log.level <= LogSeverities.LOG_NORMAL)
                            Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                                "Parts left: " + VGA.vga.draw.parts_left);
                    ...
                    Pic.PIC_AddEvent(VGA_DrawPart, (float) VGA.vga.draw.delay.parts + draw_skip,
                        VGA.vga.draw.parts_lines);
                    break;
                ...
                case VGA.Drawmode.EGALINE:
                    if (VGA.vga.draw.lines_done < VGA.vga.draw.lines_total) {
                        if (Log.level <= LogSeverities.LOG_NORMAL)
                            Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                                "Lines left: %d" + (VGA.vga.draw.lines_total - VGA.vga.draw.lines_done));
        ...
        public void call(/*Bitu*/int val) {
            ...
            if (Dosbox.IS_EGAVGA_ARCH()) {
                ...
                if (Dosbox.IS_VGA_ARCH() && Dosbox.svgaCard == SVGACards.SVGA_None
                    && (VGA.vga.mode == VGA.M_EGA || VGA.vga.mode == VGA.M_VGA)) {
                    ...
                    if ((VGA.vga.crtc.maximum_scan_line & 0x80) != 0)
                        VGA.vga.draw.address_line_total *= 2;
                    VGA.vga.draw.double_scan = false;
                } else if (Dosbox.IS_VGA_ARCH())
                    VGA.vga.draw.double_scan = (VGA.vga.crtc.maximum_scan_line & 0x80) > 0;
                else
                    VGA.vga.draw.double_scan = vtotal == 262;
            } else {
            ...
            if (Config.C_DEBUG) {
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, "h total " + htotal + " end " + hdend
                        + " blank (" + hbstart + "/" + hbend + ") retrace (" + hrstart + "/" + hrend + ")");
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, "v total " + vtotal + " end " + vdend
                        + " blank (" + vbstart + "/" + vbend + ") retrace (" + vrstart + "/" + vrend + ")");
            }
            ...
            if (Dosbox.IS_VGA_ARCH()) { // others need more investigation
                if (vbstart < vtotal) { // There will be no blanking at all otherwise
                    if (vbend > vtotal) {
                        ...
                        vblank_skip = vbend;
                        if (Log.level <= LogSeverities.LOG_WARN)
                            Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_WARN,
                        ...
                        if (vbend < vdend) {
                            ...
                            if (Log.level <= LogSeverities.LOG_WARN)
                                Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_WARN,
                                    "Unsupported blanking: line " + vbstart + "-" + vbend);
                        } else {
            ...
            */
            ...
            /*Bitu*/
            int sync = VGA.vga.misc_output >> 6;
            switch (sync) {
            ...
            switch (VGA.vga.mode) {
                ...
                case VGA.M_LIN16:
                    ...
                    width <<= 3;
                    if ((VGA.vga.crtc.mode_control & 0x8) != 0
                        || Dosbox.svgaCard == SVGACards.SVGA_S3Trio && (VGA.vga.s3.pll.cmd & 0x10) != 0)
                        doublewidth = true;
                    /* Use HW mouse cursor drawer if enabled */
                    VGA_ActivateHardwareCursor();
                    break;
                ...
                case VGA.M_TANDY2:
                    ...
                    if (Dosbox.machine == MachineType.MCH_PCJR)
                        doublewidth = (VGA.vga.tandy.gfx_control & 0x8) == 0x00;
                    else
                        doublewidth = (VGA.vga.tandy.mode_control & 0x10) == 0;
                ...
                case VGA.M_TANDY4:
                    ...
                    if (Dosbox.machine == MachineType.MCH_TANDY)
                        ...
                        doublewidth = (VGA.vga.tandy.mode_control & 0x01) == 0x00;
                    VGA.vga.draw.blocks = width * 2;
                ...
                default:
                    if (Log.level <= LogSeverities.LOG_ERROR)
                        Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_ERROR,
                            "Unhandled VGA mode " + VGA.vga.mode + " while checking for resolution");
                    break;
            }
            VGA_CheckScanLength();
            ...
            */
            ...
            if (Config.C_DEBUG) {
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL,
                        StringHelper.sprintf("h total %2.5f (%3.2fkHz) blank(%02.5f/%02.5f) retrace(%02.5f/%02.5f)",
                            new Object[] { Float.valueOf((float) VGA.vga.draw.delay.htotal),
                                Float.valueOf((float) (1.0 / VGA.vga.draw.delay.htotal)),
                                Float.valueOf((float) VGA.vga.draw.delay.hblkstart),
                                Float.valueOf((float) VGA.vga.draw.delay.hblkend),
                                Float.valueOf((float) VGA.vga.draw.delay.hrstart),
                                Float.valueOf((float) VGA.vga.draw.delay.hrend) }));
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL,
                        StringHelper.sprintf("v total %2.5f (%3.2fHz) blank(%02.5f/%02.5f) retrace(%02.5f/%02.5f)",
                            new Object[] { Float.valueOf((float) VGA.vga.draw.delay.vtotal),
                                Float.valueOf((float) (1000.0 / VGA.vga.draw.delay.vtotal)),
                                Float.valueOf((float) VGA.vga.draw.delay.vblkstart),
                                Float.valueOf((float) VGA.vga.draw.delay.vblkend),
                                Float.valueOf((float) VGA.vga.draw.delay.vrstart),
                                Float.valueOf((float) VGA.vga.draw.delay.vrend) }));
            }

            // need to resize the output window?
            if (width != VGA.vga.draw.width || height != VGA.vga.draw.height || VGA.vga.draw.doublewidth != doublewidth
                || VGA.vga.draw.doubleheight != doubleheight
                || Math.abs(aspect_ratio - VGA.vga.draw.aspect_ratio) > 0.0001 || VGA.vga.draw.bpp != bpp
                || fps_changed) {
                ...
                if (doubleheight)
                    VGA.vga.draw.lines_scaled = 2;
                else
                    VGA.vga.draw.lines_scaled = 1;
                if (Config.C_DEBUG) {
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL,
                            "Width " + width + ", Height " + height + ", fps " + fps);
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, (doublewidth ? "double" : "normal")
                            + " width, " + (doubleheight ? "double" : "normal") + " height aspect " + aspect_ratio);
                }
    ...
    private static int CGA16_READER(int base, int vidstart, int OFF) {
        return RAM.readb(base + (vidstart + OFF & 8 * 1024 - 1));
    }
    ...
    private static void VGA_ChangesEnd() {
        if (VGA.vga.changes.active) {
            //		VGA.vga.changes.active = false;
            /*Bitu*/
            long end = VGA.vga.draw.address >> VGA.VGA_CHANGE_SHIFT;
            /*Bitu*/
            long total = 4 + end - VGA.vga.changes.start;
            /*Bit32u*/
    ...
    private static void VGA_ProcessSplit() {
        if ((VGA.vga.attr.mode_control & 0x20) != 0) {
            ...
            if (VGA.vga.mode != VGA.M_TEXT && Dosbox.machine != MachineType.MCH_EGA)
                VGA.vga.draw.address += VGA.vga.draw.panning;
        }
        VGA.vga.draw.address_line = 0;
    }
    ...
    public static void VGA_SetBlinking(/*Bitu*/int enabled) {
        ...
        int b;
        if (Log.level <= LogSeverities.LOG_NORMAL)
            Log.log(LogTypes.LOG_VGA, LogSeverities.LOG_NORMAL, "Blinking " + enabled);
        if (enabled != 0) {
            b = 0;
            VGA.vga.draw.blinking = true; //used to -1 but blinking is unsigned
    ...
    public static void VGA_ActivateHardwareCursor() {
        ...
        if (VGA.svga.hardware_cursor_active != null) {
            if (VGA.svga.hardware_cursor_active.call())
    ...
    public static void VGA_SetOverride(boolean vga_override) {
