...
public class VGA_xga {
    ...
    public static IoHandler.IO_WriteHandler XGA_Write = (port, val, len) -> {
        ...
        switch (port) {
            ...
            default:
                if (port <= 0x4000) {
                    ...
                    Log.log_msg("XGA: Wrote to port " + Integer.toString(port, 16) + " with "
                        + Integer.toString(val, 16) + ", len " + len);
                break;
        }
    };
    public static IoHandler.IO_ReadHandler XGA_Read = (port, len) -> {
        switch (port) {
            case 0x8118:
            case 0x9ae8:
    ...
    private static /*Bitu*/int XGA_Read_Multifunc() {
        switch (xga.read_sel++) {
            case 0:
                return xga.MIPcount;
            case 1:
                return xga.scissors.y1;
            case 2:
                return xga.scissors.x1;
            case 3:
                return xga.scissors.y2;
            case 4:
                return xga.scissors.x2;
            case 5:
                return xga.pix_cntl;
            case 6:
                return xga.control1;
    ...
    private static void XGA_DrawPoint(/*Bitu*/int x, /*Bitu*/int y, /*Bitu*/int c) {
        if (((xga.curcommand & 0x1) == 0) || ((xga.curcommand & 0x10) == 0) || (x < xga.scissors.x1) || (x > xga.scissors.x2))
            return;
        if (y < xga.scissors.y1)
            return;
        if (y > xga.scissors.y2)
            return;

        /*Bit32u*/
        int memaddr = y * XGA_SCREEN_WIDTH() + x;
        ...
        switch (XGA_COLOR_MODE()) {
            case VGA.M_LIN8:
                if (memaddr >= VGA.vga.vmemsize)
                    break;
                RAM.writeb(VGA.vga.mem.linear + memaddr, (short) c);
                break;
            case VGA.M_LIN15:
                if (memaddr * 2 >= VGA.vga.vmemsize)
                    break;
    ...
    private static /*Bitu*/int XGA_GetMixResult(/*Bitu*/int mixmode, /*Bitu*/int srcval, /*Bitu*/int dstdata) {
        /*Bitu*/
        int destval = 0;
        switch (mixmode & 0xf) {
            case 0x00: /* not DST */
    ...
    private static void XGA_DrawLineVector(/*Bitu*/int val) {
        ...
        for (i = 0; i <= dx; i++) {
            /*Bitu*/
            int mixmode = xga.pix_cntl >> 6 & 0x3;
            switch (mixmode) {
    ...
    private static void XGA_DrawLineBresenham(/*Bitu*/int val) {
        ...
        if ((xga.desty & 0x2000) != 0)
            dminor |= 0xffffe000;
        dminor >>= 1;

        /*Bits*/
        destxtmp = (/*Bit16s*/short) xga.destx;
        if ((xga.destx & 0x2000) != 0)
            destxtmp |= 0xffffe000;
    ...
    private static boolean XGA_CheckX() {
        ...
        if (!xga.waitcmd.newline) {
            ...
            if (xga.waitcmd.curx < 2048 && xga.waitcmd.curx > xga.waitcmd.x2) {
                ...
                /*Bit16u*/
                int realx = 4096 - xga.waitcmd.curx;
                if (xga.waitcmd.x2 > 2047) { // x end is negative too
                    /*Bit16u*/
                    int realxend = 4096 - xga.waitcmd.x2;
                    if (realx == realxend) {
                        xga.waitcmd.curx = xga.waitcmd.x1;
                        xga.waitcmd.cury++;
                        xga.waitcmd.cury &= 0x0fff;
                    ...
                    case 0x02: // Data from PIX_TRANS selects the mix
                        ...
                        for (/*Bitu*/int k = 0; k < chunks; k++) { // chunks counter
                            ...
                            for (/*Bitu*/int n = 0; n < chunksize; n++) { // pixels
                                ...
                                if ((val & mask) != 0)
                                    mixmode1 = xga.foremix;
                                else
                                    mixmode1 = xga.backmix;
    ...
    private static void XGA_BlitRect(/*Bitu*/int val) {
        ...
        /*Bitu*/
        int mixselect = xga.pix_cntl >> 6 & 0x3;
        /*Bitu*/
    ...
    private static void XGA_DrawPattern(/*Bitu*/int val) {
        /*Bitu*/
        int srcdata;
        /*Bitu*/
        int dstdata;
        ...
        switch (cmd) {
            ...
            case 2: /* Rectangle fill */
                if ((val & 0x100) == 0) {
                    xga.waitcmd.wait = false;
                    if (XGA_SHOW_COMMAND_TRACE)
                        Log.log_msg(StringHelper.sprintf("XGA: Draw immediate rect: xy(%3d/%3d), len(%3d/%3d)",
                            new Object[] { Integer.valueOf(xga.curx), Integer.valueOf(xga.cury),
                                Integer.valueOf(xga.MAPcount), Integer.valueOf(xga.MIPcount) }));
                    ...
                    if (XGA_SHOW_COMMAND_TRACE)
                        Log.log_msg(
                            StringHelper.sprintf("XGA: Draw wait rect, w/h(%3d/%3d), x/y1(%3d/%3d), x/y2(%3d/%3d), %4x",
                                new Object[] { Integer.valueOf(xga.MAPcount + 1), Integer.valueOf(xga.MIPcount + 1),
                                    Integer.valueOf(xga.curx), Integer.valueOf(xga.cury),
                                    Integer.valueOf(xga.curx + xga.MAPcount & 0x0fff),
                                    Integer.valueOf(xga.cury + xga.MIPcount + 1 & 0x0fff),
                                    Integer.valueOf(val & 0xffff) }));
                }
                break;
            case 6: /* BitBLT */
                if (XGA_SHOW_COMMAND_TRACE)
            ...
            case 7: /* Pattern fill */
                if (XGA_SHOW_COMMAND_TRACE)
                    Log.log_msg(StringHelper.sprintf("XGA: Pattern fill: src(%3d/%3d), dest(%3d/%3d), fill(%3d/%3d)",
                        new Object[] { Integer.valueOf(xga.curx), Integer.valueOf(xga.cury), Integer.valueOf(xga.destx),
                            Integer.valueOf(xga.desty), Integer.valueOf(xga.MAPcount),
                            Integer.valueOf(xga.MIPcount) }));
                XGA_DrawPattern(val);
                break;
            default:
        ...
        switch (XGA_COLOR_MODE()) {
            ...
            case VGA.M_LIN32:
                if ((xga.control1 & 0x200) != 0)
                    return (int) reg;
                xga.control1 ^= 0x10;
                if ((xga.control1 & 0x10) != 0)
                    return (int) reg & 0x0000ffff;
                else
                    return (int) (reg >>> 16);
        }
        return 0;
    ...
    private static class XGAStatus {
        ...
        public static class XGA_WaitCmd {
            ...
            /*Bit32u*/ long data;
            /* transient data passed by multiple calls */
            /*Bitu*/ int datasize;
            /*Bitu*/ int buswidth;
        }
