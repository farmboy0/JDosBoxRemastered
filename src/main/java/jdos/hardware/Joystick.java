...
public class Joystick extends Module_base {
    ...
    private static final int OHMS = 120000 / 2;
    private static final float JOY_S_CONSTANT = 0.0000242f;
    private static final float S_PER_OHM = 0.000000011f;
    static Stick[] stick = new Stick[2];
    static Joystick test;
    public static Section.SectionFunction JOYSTICK_Destroy = section -> {
        test = null;
        for (int i = 0; i < stick.length; i++)
            stick[i] = null;
    };
    public static Section.SectionFunction JOYSTICK_Init = section -> {
        for (int i = 0; i < stick.length; i++)
            stick[i] = new Stick();
        test = new Joystick(section);
        section.AddDestroyFunction(JOYSTICK_Destroy, true);
    };
    private static /*Bit32u*/ long last_write = 0;
    ...
    private static final IoHandler.IO_ReadHandler read_p201 = (port, iolen) -> {
        ...
        /**
         * Format of the byte to be returned: | 7 | 6 | 5 | 4 | 3 | 2 | 1 | 0 |
         ** +-------------------------------+ | | | | | | | | Joystick B, Button 2 ---+ |
         * | | | | | +--- Joystick A, X Axis Joystick B, Button 1 -------+ | | | |
         * +------- Joystick A, Y Axis Joystick A, Button 2 -----------+ | |
         * +----------- Joystick B, X Axis Joystick A, Button 1 ---------------+
         * +--------------- Joystick B, Y Axis
         **/
        /*Bit8u*/
        short ret = 0xff;
    ...
    private static final IoHandler.IO_ReadHandler read_p201_timed = (port, iolen) -> {
        /*Bit8u*/
        short ret = 0xff;
        double currentTick = Pic.PIC_FullIndex();
        if (stick[0].enabled) {
            if (stick[0].xtick < currentTick)
                ret &= ~1;
            if (stick[0].ytick < currentTick)
                ret &= ~2;
        }
        if (stick[1].enabled) {
            if (stick[1].xtick < currentTick)
                ret &= ~4;
            if (stick[1].ytick < currentTick)
                ret &= ~8;
        }
    ...
    private static final IoHandler.IO_WriteHandler write_p201 = (port, val, iolen) -> {
    ...
    private static final IoHandler.IO_WriteHandler write_p201_timed = (port, val, iolen) -> {
        // Store writetime index
        ...
        double currentTick = Pic.PIC_FullIndex();
        if (stick[0].enabled) {
            stick[0].xtick = currentTick + 1000.0 * (JOY_S_CONSTANT + S_PER_OHM * ((stick[0].xpos + 1.0) * OHMS));
            stick[0].ytick = currentTick + 1000.0 * (JOY_S_CONSTANT + S_PER_OHM * ((stick[0].ypos + 1.0) * OHMS));
        }
        if (stick[1].enabled) {
            stick[1].xtick = currentTick
                + 1000.0 * (JOY_S_CONSTANT + S_PER_OHM * ((swap34 ? stick[1].ypos : stick[1].xpos) + 1.0) * OHMS);
            stick[1].ytick = currentTick
                + 1000.0 * (JOY_S_CONSTANT + S_PER_OHM * ((swap34 ? stick[1].xpos : stick[1].ypos) + 1.0) * OHMS);
        }
    };
    int joytype;
    ...
    public Joystick(Section configuration) {
        ...
        if (type.equalsIgnoreCase("none"))
            joytype = JoystickType.JOY_NONE;
        else if (type.equalsIgnoreCase("false"))
            joytype = JoystickType.JOY_NONE;
        else if (type.equalsIgnoreCase("auto"))
            joytype = JoystickType.JOY_AUTO;
        else if (type.equalsIgnoreCase("2axis"))
            joytype = JoystickType.JOY_2AXIS;
        else if (type.equalsIgnoreCase("4axis"))
            joytype = JoystickType.JOY_4AXIS;
        else if (type.equalsIgnoreCase("4axis_2"))
            joytype = JoystickType.JOY_4AXIS_2;
        else if (type.equalsIgnoreCase("fcs"))
            joytype = JoystickType.JOY_FCS;
        else if (type.equalsIgnoreCase("ch"))
            joytype = JoystickType.JOY_CH;
        else
            joytype = JoystickType.JOY_AUTO;
        ...
        stick[0].xtick = stick[0].ytick = stick[1].xtick = stick[1].ytick = Pic.PIC_FullIndex();
    }
