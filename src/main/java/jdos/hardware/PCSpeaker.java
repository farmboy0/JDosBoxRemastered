...
public class PCSpeaker extends Module_base {
    ...
    private static final Mixer.MIXER_Handler PCSPEAKER_CallBack = len -> {
        /*Bit16s*/
        ...
        if (spkr.last_ticks + 10000 < test_ticks)
            turnoff = true;
        if (spkr.mode == SPKR_MODES.SPKR_OFF && spkr.last_ticks + 1000 < test_ticks)
            turnoff = true;
    ...
    private static final Section.SectionFunction PCSPEAKER_ShutDown = section -> {
        test = null;
        spkr = null;
    };
    public static Section.SectionFunction PCSPEAKER_Init = section -> {
        spkr = new Spkr();
