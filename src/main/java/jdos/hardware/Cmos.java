...
public class Cmos extends Module_base {
    static Cmos test;
    private static final CMOS cmos = new CMOS();

    private static final Pic.PIC_EventHandler cmos_timerevent = new Pic.PIC_EventHandler() {
        @Override
    ...
    private static final IoHandler.IO_WriteHandler cmos_selreg = (port, val, iolen) -> {
        cmos.reg = (byte) (val & 0x3f);
        cmos.nmi = (val & 0x80) > 0;
    };
    private static final IoHandler.IO_WriteHandler cmos_writereg = (port, val, iolen) -> {
        switch (cmos.reg) {
            case 0x00: /* Seconds */
            case 0x02: /* Minutes */
            case 0x04: /* Hours */
            ...
            case 0x0a: /* Status reg A */
                ...
                if ((val & 0x70) != 0x20)
                    Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR, "CMOS Illegal 22 stage divider value");
            ...
            default:
                ...
                if (Log.level <= LogSeverities.LOG_ERROR)
                    Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR,
                        "CMOS:WRite to unhandled register " + Integer.toString(cmos.reg, 16));
        }
    };
    private static final IoHandler.IO_ReadHandler cmos_readreg = (port, iolen) -> {
        if (cmos.reg > 0x3f) {
            if (Log.level <= LogSeverities.LOG_ERROR)
                Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_ERROR,
                    "CMOS:Read from illegal register " + Integer.toString(cmos.reg, 16));
            return 0xff;
        }
        /*Bitu*/
        int drive_a, drive_b;
        ...
        switch (cmos.reg) {
            ...
            case 0x1b:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
                if (Bios_disk.imageDiskList[2] != null)
                    return (int) (Bios_disk.imageDiskList[2].cylinders & 0xff);
                return 0;
            case 0x1c:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
                if (Bios_disk.imageDiskList[2] != null)
                    return (int) ((Bios_disk.imageDiskList[2].cylinders & 0xff00) >> 8);
                return 0;
            case 0x1d:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
            ...
            case 0x20:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
                if (Bios_disk.imageDiskList[2] != null)
                    return 0xc0 | (Bios_disk.imageDiskList[2].heads > 8 ? 1 : 0) << 3;
                return 0;
            case 0x21:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
            ...
            case 0x24:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
                if (Bios_disk.imageDiskList[3] != null)
                    return (int) (Bios_disk.imageDiskList[3].cylinders & 0xff);
                return 0;
            case 0x25:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
                if (Bios_disk.imageDiskList[3] != null)
                    return (int) ((Bios_disk.imageDiskList[3].cylinders & 0xff00) >> 8);
                return 0;
            case 0x26:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
            ...
            case 0x29:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
                if (Bios_disk.imageDiskList[3] != null)
                    return 0xc0 | (Bios_disk.imageDiskList[3].heads > 8 ? 1 : 0 << 3);
                return 0;
            case 0x2a:
                if (Bios.boot)
                    return cmos.regs[cmos.reg] & 0xFF;
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_BIOS, LogSeverities.LOG_NORMAL,
                        "CMOS:Read from reg " + Integer.toString(cmos.reg, 16));
                return cmos.regs[cmos.reg];
        }
    };
    ...
    private static final Section.SectionFunction CMOS_Destroy = section -> {
        test = null;
    ...
    public static Section.SectionFunction CMOS_Init = section -> {
        for (int i = 0; i < ReadHandler.length; i++)
    ...
    private static void cmos_checktimer() {
        ...
        if (cmos.timer.div == 0 || !cmos.timer.enabled)
            return;
        if (Log.level <= LogSeverities.LOG_NORMAL)
            Log.log(LogTypes.LOG_PIT, LogSeverities.LOG_NORMAL,
                "RTC Timer at " + StringHelper.format(1000.0 / cmos.timer.delay, 2) + " hz");
        //	PIC_AddEvent(cmos_timerevent,cmos.timer.delay);
        /* A rtc is always running */
        double remd = Pic.PIC_FullIndex() % cmos.timer.delay;
        Pic.PIC_AddEvent(cmos_timerevent, (float) (cmos.timer.delay - remd)); //Should be more like a real pc. Check
    ...
    public static void CMOS_SetRegister(/*Bitu*/int regNr, /*Bit8u*/int val) {
