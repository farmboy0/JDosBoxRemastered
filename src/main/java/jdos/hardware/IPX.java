...
public class IPX extends Module_base {
    ...
    private static/*Bit32u*/ int udpPort;
    private static boolean isIpxServer;
    private static boolean isIpxConnected;
    private static InetAddress ipxServConnIp; // IPAddress for client connection to server
    ...
    private static final Dos_system.MultiplexHandler IPX_Multiplex = () -> {
        if (CPU_Regs.reg_eax.word() != 0x7a00)
            return false;
    ...
    //#ifdef IPX_DEBUGMSG
    private static /*Bitu*/ int ECBSerialNumber = 0;
    private static /*Bitu*/ int ECBAmount = 0;
    private static ECBClass ECBList; // Linked list of ECB's
    private static ECBClass ESRList; // ECBs waiting to be ESR notified
    private static final Pic.PIC_EventHandler IPX_AES_EventHandler = param -> {
        ...
        public /*Bitu*/int call() {
            ...
            while (ESRList != null) {
                ...
                if (ESRList.getESRAddr() != 0) {
                    ...
                    CPU_Regs.reg_eax.low(0xff);
                    Callback.CALLBACK_RunRealFar(Memory.RealSeg(ESRList.getESRAddr()),
                        Memory.RealOff(ESRList.getESRAddr()));
                }
                ESRList.close();
            } // while
    ...
    private static final Program.PROGRAMS_Main IPXNET_ProgramStart = () -> new IPXNET();
    private static /*Bit16u*/ int dospage;
    private static IPX test;
    public static Section.SectionFunction IPX_ShutDown = section -> test.close();
    public static Section.SectionFunction IPX_Init = section -> {
    ...
    private IPX(Section configuration) {
        ...
        if (dospage == 0)
            dospage = Dos_tables.DOS_GetMemory(2); // can not be freed yet
    ...
    private static void handleIpxRequest() {
        ...
        switch (CPU_Regs.reg_ebx.word()) {
            case 0x0000: // Open socket
                OpenSocket();
                Log.log_msg(StringHelper.sprintf("IPX: Open socket %4x",
                    new Object[] { Integer.valueOf(swapByte(CPU_Regs.reg_edx.word())) }));
                break;
            case 0x0001: // Close socket
                Log.log_msg(StringHelper.sprintf("IPX: Close socket %4x",
                    new Object[] { Integer.valueOf(swapByte(CPU_Regs.reg_edx.word())) }));
                CloseSocket();
                break;
            case 0x0002: // get local target
                ...
                for (/*Bitu*/int i = 0; i < 6; i++)
                    Memory.real_writeb(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_edi.word() + i,
                        Memory.real_readb(CPU_Regs.reg_esVal.dword, CPU_Regs.reg_esi.word() + i + 4));
            ...
            case 0x0007: // SCHEDULE SPECIAL IPX EVENT
            {
                ...
                Pic.PIC_AddEvent(IPX_AES_EventHandler, 1000.0f / (1193182.0f / 65536.0f) * CPU_Regs.reg_eax.word(),
                    tmpECB.ECBAddr);
            ...
            case 0x0009: // Get internetwork address
            {
                Log.log_msg(StringHelper.sprintf("IPX: Get internetwork address %2x:%2x:%2x:%2x:%2x:%2x", new Object[] {
                    Integer.valueOf(localIpxAddr.netnode[5] & 0xFF), Integer.valueOf(localIpxAddr.netnode[4] & 0xFF),
                    Integer.valueOf(localIpxAddr.netnode[3] & 0xFF), Integer.valueOf(localIpxAddr.netnode[2] & 0xFF),
                    Integer.valueOf(localIpxAddr.netnode[1] & 0xFF),
                    Integer.valueOf(localIpxAddr.netnode[0] & 0xFF) }));
            ...
            default:
                Log.log_msg(StringHelper.sprintf("Unhandled IPX function: %4x",
                    new Object[] { Integer.valueOf(CPU_Regs.reg_ebx.word()) }));
                break;
        }
    }
    ...
    private static void pingAck(IPaddress retAddr) {
        ...
        System.arraycopy(localIpxAddr.netnode, 0, regHeader.src.addr.byNode.node, 0,
            regHeader.src.addr.byNode.node.length);
    ...
    private static void sendPacket(ECBClass sendecb) {
        ...
        for (/*Bitu*/int m = 0; m < 6; m++) {
            if (localIpxAddr.netnode[m] != outbuffer[m + 0xa])
                isloopback = false;
            if (immedAddr[m] != (byte) 0xff)
    ...
    private static boolean ConnectToServer(String strAddr) {
        try {
            ...
            /*Bit32u*/
            long ticks, elapsed;
            ticks = MainBase.GetTicks();

            while (true) {
                elapsed = MainBase.GetTicks() - ticks;
                if (elapsed > 5000) {
                ...
                if (length > 0) {
                    ...
                    System.arraycopy(regHeader.dest.addr.byNode.node, 0, localIpxAddr.netnode, 0,
                        localIpxAddr.netnode.length);
                    localIpxAddr.netnum(regHeader.dest.network);
                    break;
                }
            }

            Log.log_msg(StringHelper.sprintf("IPX: Connected to server.  IPX address is %d:%d:%d:%d:%d:%d",
                new Object[] { Integer.valueOf(localIpxAddr.netnode[0] & 0xFF),
                    Integer.valueOf(localIpxAddr.netnode[1] & 0xFF), Integer.valueOf(localIpxAddr.netnode[2] & 0xFF),
                    Integer.valueOf(localIpxAddr.netnode[3] & 0xFF), Integer.valueOf(localIpxAddr.netnode[4] & 0xFF),
                    Integer.valueOf(localIpxAddr.netnode[5] & 0xFF) }));
    ...
    public static final class packetBuffer {
        ...
        /*Bit16s*/ short packetSize;
        // Packet size remaining in read
    ...
    static final class IPXHeader {
        ...
        public static class transport {
            ...
            public static class addrtype {
                ...
                public int host() {
                    return byNode.node[0] & 0xFF | (byNode.node[1] & 0xFF) << 8 | (byNode.node[2] & 0xFF) << 16
                        | (byNode.node[3] & 0xFF) << 24;
                }

                public String hostAsString() {
                    return (byNode.node[0] & 0xFF) + "." + (byNode.node[1] & 0xFF) + "." + (byNode.node[2] & 0xFF) + "."
                        + (byNode.node[3] & 0xFF);
                }
    ...
    private static class ECBClass {
        ...
        public ECBClass(/*Bit16u*/int segment, /*Bit16u*/int offset) {
            ...
            if (Config.IPX_DEBUGMSG) {
                ...
                Log.log_msg(StringHelper.sprintf("ECB: SN%7d created.   Number of ECBs: %3d, ESR %4x:%4x, ECB %4x:%4x",
                    new Object[] { Integer.valueOf(SerialNumber), Integer.valueOf(ECBAmount),
                        Integer.valueOf(Memory.real_readw(Memory.RealSeg(ECBAddr), Memory.RealOff(ECBAddr) + 6)),
                        Integer.valueOf(Memory.real_readw(Memory.RealSeg(ECBAddr), Memory.RealOff(ECBAddr) + 4)),
                        Integer.valueOf(segment), Integer.valueOf(offset) }));
            }
        ...
        public /*RealPt*/int getESRAddr() {
            return Memory.RealMake(Memory.real_readw(Memory.RealSeg(ECBAddr), Memory.RealOff(ECBAddr) + 6),
        ...
        public void close() {
            if (Config.IPX_DEBUGMSG) {
                ECBAmount--;
                Log.log_msg(StringHelper.sprintf("ECB: SN%7d destroyed. Remaining ECBs: %3d",
                    new Object[] { Integer.valueOf(SerialNumber), Integer.valueOf(ECBAmount) }));
            }
    ...
    private static class IPXNET extends Program {
        ...
        public void Run() {
            ...
            if (cmd.GetCount() == 0) {
                ...
                WriteOut(
                    "IPXNET [ CONNECT | DISCONNECT | STARTSERVER | STOPSERVER | PING | HELP |\n         STATUS ]\n\n");
                return;
            }
            ...
            if ((temp_line = cmd.FindCommand(1)) != null) {
                ...
                if ("startserver".equals(temp_line)) {
                    if (!isIpxServer) {
                        ...
                        if (startsuccess) {
                            ...
                            if (udpPort < 1024)
                                WriteOut(
                                    "Try a port number above 1024. See IPXNET HELP CONNECT on how to specify a port.\n");
                        }
                    } else {
                ...
                if ("status".equals(temp_line)) {
                    ...
                    if (incomingPacket.connected) {
                        WriteOut(
                            "CONNECTED -- Server at " + ipxServConnIp.getHostAddress() + " port " + udpPort + "\n");
                    } else {
                ...
                if ("ping".equals(temp_line)) {
                    ...
                    pingSend();
                    ticks = MainBase.GetTicks();
                    while (MainBase.GetTicks() - ticks < 1500) {
                        Callback.CALLBACK_Idle();
                        if (pingCheck(pingHead)) {
                            WriteOut("Response from " + pingHead.src.addr.hostAsString() + ", port "
                                + pingHead.src.addr.port() + " time=" + (MainBase.GetTicks() - ticks) + "ms\n");
                        }
                    }
                    Timer.TIMER_AddTickHandler(IPX_ClientLoop);
                }
