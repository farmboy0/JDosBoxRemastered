...
public class IoHandler {
    ...
    private static final IO_ReadHandler IO_ReadBlocked = (port, iolen) -> ~0;
    private static final IO_WriteHandler IO_WriteBlocked = (port, val, iolen) -> {
    };
    private static final IO_ReadHandler IO_ReadDefault = (port, iolen) -> {
        switch (iolen) {
            case 1:
                if (Log.level <= LogSeverities.LOG_WARN)
                    Log.log(LogTypes.LOG_IO, LogSeverities.LOG_WARN,
                        "Read from port " + Integer.toString(port, 16));
                io_readhandlers[0][port] = IO_ReadBlocked;
                return 0xff;
            case 2:
                return io_readhandlers[0][port].call(port, 1) | io_readhandlers[0][port + 1].call(port + 1, 1) << 8;
            case 4:
                return io_readhandlers[1][port].call(port, 2)
                    | io_readhandlers[1][port + 2].call(port + 2, 2) << 16;
        }
        return 0;
    };
    private static final IO_WriteHandler IO_WriteDefault = (port, val, iolen) -> {
        switch (iolen) {
            case 1:
                if (Log.level <= LogSeverities.LOG_WARN)
                    Log.log(LogTypes.LOG_IO, LogSeverities.LOG_WARN,
                        "Writing " + Integer.toString(val, 16) + " to port " + Integer.toString(port, 16));
    ...
    public static void IO_RegisterReadHandler(/*Bitu*/int port, IO_ReadHandler handler, /*Bitu*/int mask,
        ...
        while (range-- != 0) {
            ...
            if ((mask & IO_MD) != 0)
                io_readhandlers[2][port] = handler;
            port++;
        }
    }
    ...
    public static void IO_RegisterWriteHandler(/*Bitu*/int port, IO_WriteHandler handler, /*Bitu*/int mask,
        ...
        while (range-- != 0) {
            ...
            if ((mask & IO_MD) != 0)
                io_writehandlers[2][port] = handler;
            port++;
        }
    }
    ...
    public static void IO_FreeReadHandler(/*Bitu*/int port, /*Bitu*/int mask, /*Bitu*/int range) {
        while (range-- != 0) {
            ...
            if ((mask & IO_MD) != 0)
                io_readhandlers[2][port] = IO_ReadDefault;
            port++;
        }
    }
    ...
    public static void IO_FreeWriteHandler(/*Bitu*/int port, /*Bitu*/int mask, /*Bitu*/int range) {
        while (range-- != 0) {
            ...
            if ((mask & IO_MD) != 0)
                io_writehandlers[2][port] = IO_WriteDefault;
            port++;
        }
    }
    ...
    public static class IO_ReadHandleObject extends IO_Base {
        ...
        public void Install(/*Bitu*/int port, IO_ReadHandler handler, /*Bitu*/int mask, /*Bitu*/int range/*=1*/) {
            if (!installed) {
                ...
                Log.exit("IO_readHandler already installed port " + Integer.toString(port, 16));
        }
    ...
    public static class IO_WriteHandleObject extends IO_Base {
        ...
        public void Install(/*Bitu*/int port, IO_WriteHandler handler, /*Bitu*/int mask, /*Bitu*/int range/*=1*/) {
            if (!installed) {
                ...
                Log.exit("IO_writeHandler already installed port " + Integer.toString(port, 16));
        }
