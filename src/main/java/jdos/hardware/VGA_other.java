...
public class VGA_other {
        ...
        switch (VGA.vga.other.index) {
            ...
            case 0x03: //Horizontal sync width
                if (Dosbox.machine == MachineType.MCH_TANDY)
                    VGA.vga.other.vsyncw = (/*Bit8u*/short) (val >> 4);
                else
                    VGA.vga.other.vsyncw = 16; // The MC6845 has a fixed v-sync width of 16 lines
            ...
            case 0x09: //Max scanline
                ...
                if ((VGA.vga.other.max_scanline ^ val) != 0)
                    VGA.VGA_StartResize();
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "MC6845:Write " + Integer.toString(val, 16) + " to illegal index "
                            + Integer.toString(VGA.vga.other.index, 16));
        }
    };
    ...
    private static final IoHandler.IO_ReadHandler read_crtc_data_other = (port, iolen) -> {
        switch (VGA.vga.other.index) {
            ...
            default:
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                        "MC6845:Read from illegal index " + Integer.toString(VGA.vga.other.index, 16));
        }
        /*Bitu*/
        return ~0;
    };
        ...
        switch (port) {
            ...
            case 0x3dc: // Preset lightpen latch
                if (!VGA.vga.other.lightpen_triggered) {
                    ...
                    VGA.vga.other.lightpen += (int) (timeInLine / VGA.vga.draw.delay.hdend
                        * (VGA.vga.draw.address_add / 2));
            ...
            case 0x3d8:
                ...
                if ((VGA.vga.tandy.mode_control & 0x2) != 0) { // graphics mode
                    if ((VGA.vga.tandy.mode_control & 0x10) != 0) {// highres mode
                        ...
                        VGA.VGA_SetMode(VGA.M_TANDY4); // lowres mode
        ...
        switch (port) {
            ...
            case 0x3d9:
                ...
                if (VGA.vga.mode == VGA.M_TANDY2)
                    ...
                    VGA.vga.attr.palette[0] = (short) (VGA.vga.tandy.color_select & 0xf); // Pirates!
    ...
    private static final IoHandler.IO_ReadHandler read_herc_status = (port, iolen) -> {
        ...
        double timeInLine = timeInFrame % VGA.vga.draw.delay.htotal;
        if (timeInLine >= VGA.vga.draw.delay.hrstart && timeInLine <= VGA.vga.draw.delay.hrend)
            retval |= 0x1;
...
// to match an entry that is generated here.
        ...
        for (/*Bitu*/int i = 0; i < 16; i++) {
            for (/*Bitu*/int j = 0; j < 5; j++) {
                ...
                I -= colorBit3;
                Q -= colorBit4;
                Y = j / 4.0; //calculated avarage is over 4 bits
        ...
        if (Dosbox.machine == MachineType.MCH_TANDY) {
                ...
                case VGA.M_TANDY4:
                    if ((VGA.vga.tandy.gfx_control & 0x8) != 0) {
                        ...
                        VGA.VGA_SetCGA4Table( // function sets both medium and highres 4color tables
                            VGA.vga.attr.palette[0], VGA.vga.attr.palette[1], VGA.vga.attr.palette[2],
                            VGA.vga.attr.palette[3]);
                    } else {
                        int color_set = 0;
                        int r_mask = 0xf;
                        if ((VGA.vga.tandy.color_select & 0x10) != 0)
                            color_set |= 8; // intensity
                        if ((VGA.vga.tandy.color_select & 0x20) != 0)
                            color_set |= 1; // Cyan Mag. White
                        if ((VGA.vga.tandy.mode_control & 0x04) != 0) { // Cyan Red White
                            color_set |= 1;
                            r_mask &= ~1;
                        }
                        VGA.VGA_SetCGA4Table(VGA.vga.attr.palette[0],
            ...
            switch (VGA.vga.mode) {
                case VGA.M_TANDY2:
                    ...
                    break;
                case VGA.M_TANDY4:
                    VGA.VGA_SetCGA4Table(VGA.vga.attr.palette[0], VGA.vga.attr.palette[1], VGA.vga.attr.palette[2],
                        VGA.vga.attr.palette[3]);
                    break;
                default:
                    break;
    ...
    private static void PCJr_FindMode() {
        if ((VGA.vga.tandy.mode_control & 0x2) != 0) {
            if ((VGA.vga.tandy.mode_control & 0x10) != 0) {
                ...
                if (VGA.vga.mode == VGA.M_TANDY4)
                    VGA.VGA_SetModeNow(VGA.M_TANDY16); // TODO lowres mode only
                else
                    VGA.VGA_SetMode(VGA.M_TANDY16);
            } else if ((VGA.vga.tandy.gfx_control & 0x08) != 0) {
        ...
        switch (VGA.vga.tandy.reg_index) {
            case 0x0:
                if (Dosbox.machine == MachineType.MCH_PCJR) {
                    ...
                    if (Log.level <= LogSeverities.LOG_NORMAL)
                        Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL,
                            "Unhandled Write " + Integer.toString(val, 16) + " to tandy reg "
                                + Integer.toString(VGA.vga.tandy.reg_index, 16));
                }
                break;
            ...
            default:
                if ((VGA.vga.tandy.reg_index & 0xf0) == 0x10) { // color palette
                    ...
                    Log.log(LogTypes.LOG_VGAMISC, LogSeverities.LOG_NORMAL, "Unhandled Write "
                        + Integer.toString(val, 16) + " to tandy reg " + Integer.toString(VGA.vga.tandy.reg_index, 16));
        }
    }
