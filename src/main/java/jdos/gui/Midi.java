        ...
        if (midi.status == 0xf0) {
            if ((data & 0x80) == 0) {
                ...
                if ((midi.sysex.start != 0) && (midi.sysex.used >= 4) && (midi.sysex.used <= 9) && (midi.sysex.buf[1] == 0x41) && (midi.sysex.buf[3] == 0x16)) {
                    ...
                    try {
                        sysex_msg.setMessage(midi.sysex.buf, midi.sysex.used);
                    } catch (Exception e) {
                    }
                ...
                if (Log.level <= LogSeverities.LOG_NORMAL)
                    Log.log(LogTypes.LOG_ALL, LogSeverities.LOG_NORMAL, "Sysex message size " + midi.sysex.used);
...
//                }
                try {
                    msg.setMessage(midi.cmd_buf[0], midi.cmd_buf[1], midi.cmd_buf[2]);
                } catch (Exception e) {
                }
                midi.handler.send(msg, -1);
