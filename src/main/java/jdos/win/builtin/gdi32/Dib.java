...
public class Dib extends WinAPI {
    ...
    public static int GetDIBColorTable(int hdc, int uStartIndex, int cEntries, int pColors) {
    ...
    public static int SetDIBitsToDevice(int hdc, int XDest, int YDest, int dwWidth, int dwHeight, int XSrc, int YSrc,
        int uStartScan, int cScanLines, int lpvBits, int lpbmi, int fuColorUse) {
        return StretchDIBits(hdc, XDest, YDest, dwWidth, dwHeight, XSrc, YSrc, dwWidth, dwHeight, lpvBits, lpbmi,
            fuColorUse, SRCCOPY);
    }
    ...
    public static int StretchDIBits(int hdc, int XDest, int YDest, int nDestWidth, int nDestHeight, int XSrc, int YSrc,
        int nSrcWidth, int nSrcHeight, int lpBits, int lpBitsInfo, int iUsage, int dwRop) {
        WinDC dc = WinDC.get(hdc);
        if (dc == null)
            return 0;
        ...
        BitBlt.StretchBlt2D(g, dc.x + XDest, dc.x + YDest, nDestWidth, nDestHeight,
            bitmap.createJavaBitmap(true).getImage(), XSrc, bitmap.height - YSrc - 1 - nSrcHeight, nSrcWidth,
            nSrcHeight, dwRop);
        g.dispose();
        return nDestHeight;
    }
