...
public class Mmio extends WinAPI {
    public static final int MMIOM_READ = MMIO_READ; /* read */
    public static final int MMIOM_WRITE = MMIO_WRITE; /* write */
    public static final int MMIOM_SEEK = 2; /* seek to a new position in file */
    public static final int MMIOM_OPEN = 3; /* open file */
    public static final int MMIOM_CLOSE = 4; /* close file */
    public static final int MMIOM_WRITEFLUSH = 5; /* write and flush */
    public static final int MMIOM_RENAME = 6; /* rename specified file */
    private static final Hashtable<Integer, IOProc> defaultProcs = new Hashtable<>();
    private static final IOProc mmioDosIOProc = (lpmmioinfo, uMessage, lParam1, lParam2) -> {
    ...
    public static int mmioAdvance(int hmmio, int p, int uFlags) {
    ...
    public static int mmioAscend(int hmmio, int p, int wFlags) {
    ...
    public static int mmioClose(int hmmio, int uFlags) {
        WinMMIO wm = WinMMIO.get(hmmio);
        if (wm == null)
            return MMSYSERR_INVALHANDLE;
    ...
    public static int mmioDescend(int hmmio, int p, int lpckParent, int uFlags) {
        int dwOldPos;
        int srchCkId;
        int srchType;
        ...
        if (lpckParent != NULL) {
            ...
            if (dwOldPos < parent.dwDataOffset || dwOldPos >= parent.dwDataOffset + parent.cksize) {
    ...
    public static int mmioGetInfo(int hmmio, int lpmmioinfo, int wFlags) {
        WinMMIO mmio = WinMMIO.get(hmmio);
        if (mmio == null)
            return MMSYSERR_INVALHANDLE;
        if (lpmmioinfo == 0)
    ...
    public static int mmioOpenA(int szFilename, int lpmmioinfo, int dwOpenFlags) {
        ...
        if ((dwOpenFlags & MMIO_ALLOCBUF) != 0) {
            refmminfo.wErrorRet = MMIO_SetBuffer(wm, refmminfo.pchBuffer,
                refmminfo.cchBuffer != 0 ? refmminfo.cchBuffer : MMIO_DEFAULTBUFFER, 0);
    ...
    public static int mmioRead(int hmmio, int pch, int cch) {
        WinMMIO wm = WinMMIO.get(hmmio);
        if (wm == null)
            return -1;
    ...
    public static int mmioSeek(int hmmio, int lOffset, int iOrigin) {
        WinMMIO wm = WinMMIO.get(hmmio);
        if (wm == null)
            return MMSYSERR_INVALHANDLE;
        ...
        if (offset != 0 && offset >= wm.dwFileSize && wm.info.fccIOProc != FOURCC_MEM) {
            ...
            && (offset < wm.info.lBufOffset || offset >= wm.info.lBufOffset + wm.info.cchBuffer || !wm.bBufferLoaded)) {
            /* stay in same buffer ? */
            /* some memory mapped buffers are defined with -1 as a size */

            /* condition to change buffer */
            if (wm.info.fccIOProc == FOURCC_MEM || MMIO_Flush(wm, 0) != MMSYSERR_NOERROR ||
    ...
    public static int mmioSetInfo(int hmmio, int p, int uFlags) {
        WinMMIO wm = WinMMIO.get(hmmio);
        if (wm == null)
            return MMSYSERR_INVALHANDLE;
    ...
    public static int mmioWrite(int hmmio, int pch, int cch) {
        WinMMIO wm = WinMMIO.get(hmmio);
        if (wm == null)
            return -1;
     ...
     * <p/>
     * RETURNS The FOURCC code for the extension if found, else 0.
     */
    ...
    private static int mmioStringToFOURCCA(String s) {
        while (s.length() < 4) {
            s = s + " ";
        }
    ...
    private static int MMIO_Flush(WinMMIO wm, int uFlags) {
    ...
    private static class CustomIOProc implements IOProc {
        int pIOProc;
