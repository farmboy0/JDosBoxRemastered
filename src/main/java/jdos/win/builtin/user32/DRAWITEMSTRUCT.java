...
public class DRAWITEMSTRUCT extends WinAPI {
    ...
    public void write(int address) {
        writed(address, CtlType);
        address += 4;
        writed(address, CtlID);
        address += 4;
        writed(address, itemID);
        address += 4;
        writed(address, itemAction);
        address += 4;
        writed(address, itemState);
        address += 4;
        writed(address, hwndItem);
        address += 4;
        writed(address, hDC);
        address += 4;
        rcItem.write(address);
        address += WinRect.SIZE;
