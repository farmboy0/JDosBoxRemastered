...
public class ButtonWindow extends WinAPI {
    ...
    private static final int STATE_GWL_OFFSET = 0;
    private static final int HFONT_GWL_OFFSET = 4;
    private static final int HIMAGE_GWL_OFFSET = 8;
    private static final int NB_EXTRA_BYTES = 12;
    /* undocumented flags */
    private static final int BUTTON_NSTATES = 0x0F;
    private static final int BUTTON_BTNPRESSED = 0x40;
    private static final int BUTTON_UNKNOWN2 = 0x20;
    private static final int BUTTON_UNKNOWN3 = 0x10;
    private static final int MAX_BTN_TYPE = 16;
    private static final int[] maxCheckState = { BST_UNCHECKED, /* BS_PUSHBUTTON */
    ...
    private static int checkBoxWidth = 0, checkBoxHeight = 0;
    private static final Callback.Handler button_proc = new HandlerBase() {
        @Override
     ...
     * Button Functions
     */
    private static final ButtonPaint PB_Paint = (hWnd, hdc, action) -> {
     ...
     */
    private static final ButtonPaint CB_Paint = (hWnd, hdc, action) -> {
        int state = get_button_state(hWnd);
        int style = WinWindow.GetWindowLongA(hWnd, GWL_STYLE);
    ...
    private static final ButtonPaint GB_Paint = (hWnd, hdc, action) -> {
        int style = WinWindow.GetWindowLongA(hWnd, GWL_STYLE);
    ...
    private static final ButtonPaint UB_Paint = (hWnd, hdc, action) -> {
        int state = get_button_state(hWnd);
     ...
     */
    private static final ButtonPaint OB_Paint = (hWnd, hdc, action) -> {
        int state = get_button_state(hWnd);
        ...
        dis.itemState = ((state & BST_FOCUS) != 0 ? ODS_FOCUS : 0) | ((state & BST_PUSHED) != 0 ? ODS_SELECTED : 0)
            | (WinWindow.IsWindowEnabled(hWnd) != 0 ? 0 : ODS_DISABLED);
        dis.hwndItem = hWnd;
        dis.hDC = hdc;
        dis.itemData = 0;
    ...
    private static final ButtonPaint[] btnPaintFunc = { PB_Paint, /* BS_PUSHBUTTON */
    ...
    private static void BUTTON_NOTIFY_PARENT(int hWnd, int code) {
        /* Notify parent which has created this button control */
        Message.SendMessageA(WinWindow.GetParent(hWnd), WM_COMMAND,
            MAKEWPARAM(WinWindow.GetWindowLongA(hWnd, GWLP_ID), code), hWnd);
    }

    private static int get_button_state(int hwnd) {
        return WinWindow.GetWindowLongA(hwnd, STATE_GWL_OFFSET);
    }

    private static void set_button_state(int hwnd, int state) {
        WinWindow.SetWindowLongA(hwnd, STATE_GWL_OFFSET, state);
    }

    private static int get_button_font(int hwnd) {
        return WinWindow.GetWindowLongA(hwnd, HFONT_GWL_OFFSET);
    }

    private static void set_button_font(int hwnd, int font) {
        WinWindow.SetWindowLongA(hwnd, HFONT_GWL_OFFSET, font);
    }

    private static int get_button_type(int window_style) {
        return window_style & BS_TYPEMASK;
    }

    /* paint a button of any type */
    private static void paint_button(int hwnd, int style, int action) {
     ...
     */
    private static int ButtonWndProc_common(int hWnd, int uMsg, int wParam, int lParam) {
        int style = WinWindow.GetWindowLongA(hWnd, GWL_STYLE);
        ...
        switch (uMsg) {
            ...
            case WM_LBUTTONDBLCLK:
                if ((style & BS_NOTIFY) != 0 || btn_type == BS_RADIOBUTTON || btn_type == BS_USERBUTTON
                ...
                if (uMsg == WM_KEYUP || rect.contains(pt)) {
                    ...
                    switch (btn_type) {
                        ...
                        case BS_AUTORADIOBUTTON:
                            ...
                            break;
                        case BS_AUTO3STATE:
                            Message.SendMessageA(hWnd, BM_SETCHECK,
                                (state & BST_INDETERMINATE) != 0 ? 0 : (state & 3) + 1, 0);
                            break;
                    }
    ...
    /**
     * ******************************************************************* Convert
     * button styles to flags used by DrawText.
     */
    private static int BUTTON_BStoDT(int style, int ex_style) {
     ...
     * <p/>
     * Returns flags to be passed to DrawText. Calculated rectangle doesn't take
     * into account button state (pushed, etc.). If there is nothing to draw (no
     * text/image) output rectangle is empty, and return value is (UINT)-1.
     */
    private static int BUTTON_CalcLabelRect(int hwnd, int hdc, WinRect rc) {
        int style = WinWindow.GetWindowLongA(hwnd, GWL_STYLE);
     ...
     */
    public static int BUTTON_DrawTextCallback(int hdc, int lp, int wp, int cx, int cy) {
     ...
     */
    private static void BUTTON_CheckAutoRadioButton(int hwnd) {
        int parent = WinWindow.GetParent(hwnd);
