...
public class VideoMemory {
    public static int SIZE = 0; // MB
    private static int size;

    public static int mapVideoRAM(int amount) {
        if (size != 0) {
            Win.panic("Video memory doesn't support more than one mapping");
        }
        int frame = WinSystem.memory.frames.length * 32;
        long address = WinSystem.getCurrentProcess().addressSpace.getNextAddress(WinProcess.ADDRESS_VIDEO_START, amount,
            true);
    ...
    public static void unmapVideoRAM(int address) {
        int directory = WinSystem.getCurrentProcess().page_directory;
