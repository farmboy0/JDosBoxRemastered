...
import jdos.win.builtin.gdi32.WinEnhancedMetaFile;
import jdos.win.builtin.kernel32.WinProcess;
import jdos.win.builtin.user32.CREATESTRUCT;
import jdos.win.builtin.user32.DRAWITEMSTRUCT;
import jdos.win.builtin.user32.DefWnd;
import jdos.win.builtin.user32.Message;
import jdos.win.builtin.user32.PAINTSTRUCT;
import jdos.win.builtin.user32.Painting;
import jdos.win.builtin.user32.Resource;
import jdos.win.builtin.user32.SysParams;
import jdos.win.builtin.user32.UiTools;
...
import jdos.win.builtin.user32.WinText;
...
public class StaticWindow extends WinAPI {
    private static final int HFONT_GWL_OFFSET = 0;
    private static final int HICON_GWL_OFFSET = 4;
    // LRESULT WINAPI SendMessage(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
    private static final Callback.Handler static_proc = new HandlerBase() {
        @Override
    ...
    private static final StaticPaint STATIC_PaintOwnerDrawfn = (hWnd, hdc, style) -> {
    ...
    private static final StaticPaint STATIC_PaintTextfn = (hWnd, hdc, style) -> {
        int format;
        ...
        if (window.text.length() > 0) {
            if ((style & SS_TYPEMASK) == SS_SIMPLE && (style & SS_NOPREFIX) != 0) {
                ...
                WinDC.ExtTextOutA(hdc, rect.left, rect.top, ETO_CLIPPED | ETO_OPAQUE, rc,
                    StringUtil.allocateTempA(window.text), window.text.length(), NULL);
            } else {
    ...
    private static final StaticPaint STATIC_PaintRectfn = (hWnd, hdc, style) -> {
        int rc = getTempBuffer(WinRect.SIZE);
        int hBrush;
    ...
    private static final StaticPaint STATIC_PaintIconfn = (hWnd, hdc, style) -> {
        int rc = getTempBuffer(WinRect.SIZE);
        WinPos.GetClientRect(hWnd, rc);
        int hbrush = STATIC_SendWmCtlColorStatic(hWnd, hdc);
        ...
        if (icon != null) {
            ...
            WinIcon.DrawIconEx(hdc, iconRect.left, iconRect.top, hIcon, iconRect.right - iconRect.left,
                iconRect.bottom - iconRect.top, 0, NULL, DI_NORMAL);
        }
    };
    private static final StaticPaint STATIC_PaintBitmapfn = (hWnd, hdc, style) -> {
        ...
        if (hMemDC != 0) {
            ...
            if ((style & SS_CENTERIMAGE) != 0) {
                ...
                BitBlt.StretchBlt(hdc, 0, 0, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top, hMemDC,
                    0, 0, bitmap.getWidth(), bitmap.getHeight(), SRCCOPY);
            }
            WinDC.SelectObject(hMemDC, oldbitmap);
            WinDC.DeleteDC(hMemDC);
        }
    };
    private static final StaticPaint STATIC_PaintEnhMetafn = (hWnd, hdc, style) -> {
        int rc = getTempBuffer(WinRect.SIZE);
        WinPos.GetClientRect(hWnd, rc);
        int hbrush = STATIC_SendWmCtlColorStatic(hWnd, hdc);
        WinDC.FillRect(hdc, rc, hbrush);
    ...
    private static final StaticPaint STATIC_PaintEtchedfn = (hWnd, hdc, style) -> {
        int rc = getTempBuffer(WinRect.SIZE);
...
//        return hicon;
//    }
    private static final StaticPaint[] staticPaintFunc = { STATIC_PaintTextfn, /* SS_LEFT */
    ...
    public static void registerClass(WinProcess process) {
    ...
    /***********************************************************************
     * STATIC_SetIcon Set the icon for an SS_ICON control.
     */
    private static int STATIC_SetIcon(int hwnd, int hicon, int style) {
        int prevIcon;
    ...
    /***********************************************************************
     * STATIC_SetBitmap Set the bitmap for an SS_BITMAP control.
     */
    private static int STATIC_SetBitmap(int hwnd, int hBitmap, int style) {
        int hOldBitmap;
        ...
        if (hBitmap != 0 && (style & SS_CENTERIMAGE) == 0 && (style & SS_REALSIZECONTROL) == 0) {
            ...
            {
                WinPos.SetWindowPos(hwnd, 0, 0, 0, bitmap.getWidth(), bitmap.getHeight(),
    ...
    /***********************************************************************
     * STATIC_SetEnhMetaFile Set the enhanced metafile for an SS_ENHMETAFILE
     * control.
     */
    private static int STATIC_SetEnhMetaFile(int hwnd, int hEnhMetaFile, int style) {
    ...
    /***********************************************************************
     * STATIC_GetImage Gets the bitmap for an SS_BITMAP control, the icon/cursor for
     * an SS_ICON control or the enhanced metafile for an SS_ENHMETAFILE control.
     */
    static int STATIC_GetImage(int hwnd, int wParam, int style) {
        switch (style & SS_TYPEMASK) {
            case SS_ICON:
                if (wParam != IMAGE_ICON && wParam != IMAGE_CURSOR)
                    return NULL;
                break;
            case SS_BITMAP:
    ...
    /***********************************************************************
     * STATIC_LoadIconA Load the icon for an SS_ICON control.
     */
    static int STATIC_LoadIconA(int hInstance, int name, int style) {
        ...
        if (hInstance != 0) {
            if ((style & SS_REALSIZEIMAGE) != 0)
                ...
                if (hicon == 0)
                    hicon = WinCursor.LoadCursorA(hInstance, name);
            }
        }
        if (hicon == 0)
            hicon = WinIcon.LoadIconA(0, name);
        /* Windows doesn't try to load a standard cursor,
           probably because most IDs for standard cursors conflict
           with the IDs for standard icons anyway */
        return hicon;
    ...
    /***********************************************************************
     * STATIC_TryPaintFcn Try to immediately paint the control.
     */
    private static void STATIC_TryPaintFcn(int hwnd, int full_style) {
    ...
    private static int STATIC_SendWmCtlColorStatic(int hwnd, int hdc) {
    ...
    /***********************************************************************
     * hasTextStyle Tests if the control displays text.
     */
    private static boolean hasTextStyle(int style) {
        switch (style & SS_TYPEMASK) {
            case SS_SIMPLE:
            case SS_LEFT:
            case SS_LEFTNOWORDWRAP:
     ...
     */
    private static int StaticWndProc_common(int hwnd, int uMsg, int wParam, int lParam) {
        ...
        switch (uMsg) {
            ...
            case WM_ENABLE:
                ...
                if ((full_style & SS_NOTIFY) != 0) {
                    if (wParam != 0) {
                        Message.SendMessageA(WinWindow.GetParent(hwnd), WM_COMMAND,
                            MAKEWPARAM(WinWindow.GetWindowLongA(hwnd, GWLP_ID), STN_ENABLE), hwnd);
                    } else {
                        Message.SendMessageA(WinWindow.GetParent(hwnd), WM_COMMAND,
                            MAKEWPARAM(WinWindow.GetWindowLongA(hwnd, GWLP_ID), STN_DISABLE), hwnd);
                    }
                }
                break;
            ...
            case WM_NCCREATE: {
                ...
                if ((full_style & SS_SUNKEN) != 0)
                    WinWindow.SetWindowLongA(hwnd, GWL_EXSTYLE,
                        WinWindow.GetWindowLongA(hwnd, GWL_EXSTYLE) | WS_EX_STATICEDGE);
            ...
            case WM_SETFONT:
                if (hasTextStyle(full_style)) {
                    ...
                    if (LOWORD(lParam) != 0)
                        Painting.RedrawWindow(hwnd, NULL, 0,
            ...
            case WM_NCLBUTTONDOWN:
                if ((full_style & SS_NOTIFY) != 0)
                    Message.SendMessageA(WinWindow.GetParent(hwnd), WM_COMMAND,
                        MAKEWPARAM(WinWindow.GetWindowLongA(hwnd, GWLP_ID), STN_CLICKED), hwnd);
                return 0;
            ...
            case WM_NCLBUTTONDBLCLK:
                if ((full_style & SS_NOTIFY) != 0)
                    Message.SendMessageA(WinWindow.GetParent(hwnd), WM_COMMAND,
                        MAKEWPARAM(WinWindow.GetWindowLongA(hwnd, GWLP_ID), STN_DBLCLK), hwnd);
                return 0;
