...
public class ARP extends EtherUtil {
    ...
    private void parse(byte[] buffer, int offset) {
        hardware = readWord(buffer, offset);
        offset += 2;
        protocol = readWord(buffer, offset);
        offset += 2;
        hlen = buffer[offset++] & 0xFF;
        plen = buffer[offset++] & 0xFF;
        op = readWord(buffer, offset);
        offset += 2;
        System.arraycopy(buffer, offset, senderMac, 0, 6);
        offset += 6;
        senderAddress = readDWord(buffer, offset);
        offset += 4;
        System.arraycopy(buffer, offset, targetMac, 0, 6);
        offset += 6;
