...
public class EtherUtil {
    ...
    protected static final byte[] SERVER_MAC_ADDRESS = { (byte) 0xAC, (byte) 0xDE, 0x48, (byte) 0x88, (byte) 0x99,
        (byte) 0xAB };

    protected static LinkedList frames;

    public static int readWord(byte[] bytes, int off) {
        return bytes[off + 1] & 0xFF | (bytes[off] & 0xFF) << 8;
    }

    public static int readDWord(byte[] bytes, int off) {
        return bytes[off + 3] & 0xFF | (bytes[off + 2] & 0xFF) << 8 | (bytes[off + 1] & 0xFF) << 16
            | (bytes[off] & 0xFF) << 24;
    }

    public static void writeDWord(byte[] bytes, int off, int value) {
    ...
    public static void writeWord(byte[] bytes, int off, int value) {
        bytes[off + 1] = (byte) value;
        bytes[off] = (byte) (value >> 8);
    }

    public static short csum(byte[] buffer, int offset, int length) {
        int i = 0;
