...
public class Shell {
    ...
    public static Section.SectionFunction AUTOEXEC_Destroy = section -> {
        test = null;
        AutoexecObject.Shutdown();
    };

    public static Section.SectionFunction AUTOEXEC_Init = section -> {
        test = new AUTOEXEC(section);
        section.AddDestroyFunction(AUTOEXEC_Destroy, false);
    };
    private static /*Bitu*/ int call_shellstop;
    private static final Callback.Handler shellstop_handler = new Callback.Handler() {
        @Override
    ...
    private static final Program.PROGRAMS_Main SHELL_ProgramStart = () -> new Dos_shell();
    public static Config.StartFunction SHELL_Init = () -> {
        Msg.add("SHELL_ILLEGAL_PATH", "Illegal Path.\n");
        Msg.add("SHELL_CMD_HELP",
            "If you want a list of all supported commands type \033[33;1mhelp /all\033[0m .\nA short list of the most often used commands:\n");
        ...
        Msg.add("SHELL_CMD_CHDIR_HINT_2",
            "directoryname is longer than 8 characters and/or contains spaces.\nTry \033[31mcd %s\033[0m\n");
        Msg.add("SHELL_CMD_CHDIR_HINT_3",
            "You are still on drive Z:, change to a mounted drive with \033[31mC:\033[0m.\n");
        ...
        Msg.add("SHELL_EXECUTE_DRIVE_NOT_FOUND",
        ...
        Msg.add("SHELL_CMD_SUBST_FAILURE",
        ...
        /*Bit16u*/
        int env_seg = Dos.DOS_FIRST_SHELL + 19; //DOS_GetMemory(1+(4096/16))+1;
        /*Bit16u*/
    ...
    private static class AUTOEXEC extends Module_base {
        ...
        public AUTOEXEC(Section configuration) {
            ...
            if (extra != null && !secure && !Dosbox.control.cmdline.FindExist("-noautoexec", true)) {
                ...
                if (!echo_off)
                    echo_off = extra.startsWith("@echo off", 9);
