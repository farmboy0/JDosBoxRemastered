...
public class Dos_shell extends Program {
    ...
    public void Run() {
        ...
        if (cmd.FindStringRemainBegin("/C", line)) {
            ...
            if (pos >= 0)
                line.value = line.value.substring(0, pos);
        ...
        if (Dosbox.machine == MachineType.MCH_CGA)
            WriteOut(Msg.get("SHELL_STARTUP_CGA"));
        if (Dosbox.machine == MachineType.MCH_HERC)
            WriteOut(Msg.get("SHELL_STARTUP_HERC"));
    ...
    void ParseLine(String line) {
        ...
        int num = GetRedirection(s, in, out, append);
        line = s.value;
        if (num > 1)
        ...
        if (out.value != null) {
            ...
            if (append.value) {
                ...
                status = Dos_files.DOS_OpenFileExtended(out.value, Dos_files.OPEN_READWRITE,
                    Dos_system.DOS_ATTR_ARCHIVE, 0x12, dummy, dummy2);
            }

            if (!status && normalstdout)
                Dos_files.DOS_OpenFile("con", Dos_files.OPEN_READWRITE, dummy); //Read only file, open con again
            if (!normalstdin && in.value == null)
    ...
    /*Bitu*/int GetRedirection(StringRef s, StringRef ifn, StringRef ofn, BooleanRef append) {
        ...
        while (lr.length() > 0) {
            ...
            lr = lr.substring(1);
            if (quote
                && ch != '"') { /* don't parse redirection within quotes. Not perfect yet. Escaped quotes will mess the count up */
                lw.append(ch);
                continue;
            }
    ...
    String InputCommand() {
        /*Bitu*/int size = Shell.CMD_MAXLINE - 2;
        //lastcharacter+0
        /*Bit8u*/byte[] c = new byte[1];
        /*Bit16u*/IntRef n = new IntRef(1);
        /*Bitu*/int str_len = 0;
        /*Bitu*/int str_index = 0;
            ...
            switch (c[0]) {
                case 0x00: /* Extended Keys */
                {
                    ...
                    switch (c[0]) {
                        ...
                        case 0x48: /* UP */
                            if (l_history.size() == 0 || it_history == l_history.size())
                                break;
                        ...
                        case 0x50: /* DOWN */
                            if (l_history.size() == 0 || it_history == 0)
                                break;
                        ...
                        case 15: /* Shift-Tab */
                            if (l_completion.size() != 0) {
                                if (it_completion == 0)
                                    it_completion = l_completion.size();
                                it_completion--;
                ...
                case 0x08: /* BackSpace */
                    if (str_index != 0) {
                        ...
                        size++;
                        if (str_remain != 0) {
                            for (int i = 0; i < str_remain; i++)
                                line[str_index - 1 + i] = line[str_index + i];
                            line[--str_len] = 0;
                            str_index--;
                            /* Go back to redraw */
                ...
                case '\t': {
                    if (l_completion.size() != 0) {
                        it_completion++;
                        if (it_completion == l_completion.size())
                            it_completion = 0;
                    } else {
                        ...
                        int path;
                        if ((path = sLine.substring(completion_index).lastIndexOf('\\')) >= 0)
                            completion_index += path + 1;
                        if ((path = sLine.substring(completion_index).lastIndexOf('/')) >= 0)
                            completion_index += path + 1;
                        ...
                        while (res) {
                            ...
                            if (!name.value.equals(".") && !name.value.equals("..")) {
                                if (dir_only) { //Handle the dir only case different (line starts with cd)
                                    if ((att.value & Dos_system.DOS_ATTR_DIRECTORY) != 0)
                                    ...
                                    if (ext != null && (ext.equalsIgnoreCase("BAT") || ext.equalsIgnoreCase("COM")
                    ...
                    if (l_completion.size() != 0 && ((String) l_completion.elementAt(it_completion)).length() != 0) {
                        ...
                        str_len = str_index = completion_index + len.value;
                        size = Shell.CMD_MAXLINE - str_index - 2;
                        Dos_files.DOS_WriteFile(Dos_files.STDOUT,
                            ((String) l_completion.elementAt(it_completion)).getBytes(), len);
                    }
                }
                    break;
                case 0x1b: /* ESC */
    ...
    void ShowPrompt() {
        ...
        WriteOut(String.valueOf(drive) + ":\\" + dir.value + ">");
    }
    ...
    void DoCommand(String line) {
        ...
        if (Execute(cmd_buffer.toString(), line) || CheckConfig(cmd_buffer.toString(), line))
            return;
    ...
    public boolean Execute(String name, String args) {
        ...
        if ((name.substring(1).equals(":") || name.substring(1).equals(":\\"))
            && StringHelper.isalpha(name.charAt(0))) {
            if (!Dos_files.DOS_SetDrive((short) (name.toUpperCase().charAt(0) - 'A'))) {
                WriteOut(Msg.get("SHELL_EXECUTE_DRIVE_NOT_FOUND"),
                    new Object[] { Character.valueOf(name.toUpperCase().charAt(0)) });
            }
            return true;
        }
        ...
        if (sExtension.equalsIgnoreCase(".bat")) { /* Run the .bat file */
...
//    #endif
    ...
    String Which(String name) {
        ...
        if (!GetEnvStr("PATH", temp) || (temp.value.length() == 0))
            return null;
        ...
        while (pathenv.length() > 0) {
            ...
            if (path.length() > 0) {
                ...
                if (Dos_files.DOS_FileExists(p + ".EXE"))
                    return p + ".EXE";
                if (Dos_files.DOS_FileExists(p + ".BAT"))
                    return p + ".BAT";
            }
        }
        return null;
    ...
    static String FormatNumber(/*Bitu*/long num) {
        ...
        numg = num;
        if (numg != 0) {
            return StringHelper.sprintf("%d,%03d,%03d,%03d",
                new Object[] { Long.valueOf(numg), Long.valueOf(numm), Long.valueOf(numk), Long.valueOf(numb) });
        }
        if (numm != 0) {
            return StringHelper.sprintf("%d,%03d,%03d",
                new Object[] { Long.valueOf(numm), Long.valueOf(numk), Long.valueOf(numb) });
        }
        if (numk != 0) {
            return StringHelper.sprintf("%d,%03d", new Object[] { Long.valueOf(numk), Long.valueOf(numb) });
        }
    ...
    private static boolean ScanCMDBool(StringRef cmd, String check) {
        ...
        while ((pos = cmd.value.toUpperCase().indexOf(check.toUpperCase(), pos)) >= 0) {
            int start = pos;
            pos += check.length();
            if (cmd.value.length() == pos || cmd.value.charAt(pos) == ' ' || cmd.value.charAt(pos) == '\t'
                || cmd.value.charAt(pos) == '/') {
    ...
    handler CMD_HELP = a -> {
        ...
        /* Print the help */
        if (!optall)
            WriteOut(Msg.get("SHELL_CMD_HELP"));
        /*Bit32u*/int cmd_index = 0, write_count = 0;
        while (this.cmd_list[cmd_index].name != null) {
            if (optall || this.cmd_list[cmd_index].flags == 0) {
                WriteOut("<\033[34;1m" + StringHelper.leftJustify(this.cmd_list[cmd_index].name, 8) + "\033[0m> "
                    + Msg.get(this.cmd_list[cmd_index].help));
                if (++write_count % 22 == 0)
                    this.CMD_PAUSE.call("");
            }
            cmd_index++;
    ...
    handler CMD_COPY = a -> {
        ...
        /*Bit16u*/IntRef date = new IntRef(0);
        /*Bit16u*/IntRef time = new IntRef(0);
        ...
        while ((source_p = StringHelper.StripWord(args)) != null && source_p.length() > 0) {
            do {
                ...
                if (source_x_len > 0) {
                    if (source_x.charAt(source_x_len - 1) == ':')
                        has_drive_spec = true;
                }
                ...
                sources.add(new copysource(source_x, (plus >= 0) == true));
                if (plus >= 0) {
        ...
        if (target.filename.length() == 0)
            target = new copysource(defaulttarget, true);
        ...
        while (sources.size() > 0) {
            ...
            if (temp >= 0)
                pathTarget.value = pathTarget.value.substring(0, temp);//strip off *.* from target
                ...
                if ((attr.value & Dos_system.DOS_ATTR_DIRECTORY) == 0) {
                    ...
                    if (Dos_files.DOS_OpenFile(nameSource, 0, sourceHandle)) {
                        ...
                        if (oldsource.concat || Dos_files.DOS_CreateFile(nameTarget, 0, targetHandle)) {
                            ...
                            if (!oldsource.concat
                                || Dos_files.DOS_OpenFile(nameTarget, Dos_files.OPEN_READWRITE, targetHandle)
                        ...
                        WriteOut(Msg.get("SHELL_CMD_COPY_FAILURE"), new Object[] { source.filename });
                }
                //On the next file
        ...
        WriteOut(Msg.get("SHELL_CMD_COPY_SUCCESS"), new Object[] { Integer.valueOf(count) });
        Dos.dos.dta(save_dta);
    };
    ...
    handler CMD_DATE = a -> {
        ...
        if (parts.length == 3) {
            try {
                ...
                CPU_Regs.reg_eax.high(0x2b); // set system date
                Callback.CALLBACK_RunRealInt(0x21);
                if (CPU_Regs.reg_eax.low() == 0xff)
                    WriteOut(Msg.get("SHELL_CMD_DATE_ERROR"));
                return;
            } catch (Exception e) {
        ...
        if (!dateonly)
            WriteOut(Msg.get("SHELL_CMD_DATE_NOW"));

        String formatstring = Msg.get("SHELL_CMD_DATE_FORMAT");
        if (formatstring.length() != 5)
            return;
        ...
        if (!dateonly)
            WriteOut(Msg.get("SHELL_CMD_DATE_SETHLP"));
    };
    ...
    handler CMD_DIR = a -> {
        ...
        if (args.value.indexOf("*") < 0 && args.value.indexOf("?") < 0) {
            ...
            if (Dos_files.DOS_GetFileAttr(args.value, attribute)
                && (attribute.value & Dos_system.DOS_ATTR_DIRECTORY) != 0) {
        ...
        if (!optB)
            WriteOut(Msg.get("SHELL_CMD_DIR_INTRO"), new Object[] { path.value });
        ...
        if (!ret) {
            if (!optB)
                WriteOut(Msg.get("SHELL_CMD_FILE_NOT_FOUND"), new Object[] { args.value });
            Dos.dos.dta(save_dta);
            return;
        }
        ...
        do { /* File name and extension */
            ...
            if (optAD && (attr.value & Dos_system.DOS_ATTR_DIRECTORY) == 0)
                continue;
            ...
            if (optB) {
                ...
                /*Bit16u*/int year = (date.value >> 9) + 1980;
                ...
                if ((attr.value & Dos_system.DOS_ATTR_DIRECTORY) != 0) {
                    if (optW) {
                        ...
                        WriteOut("%-8s %-3s   %-16s %02d-%02d-%04d %2d:%02d\n",
                            new Object[] { name.value, ext, "<DIR>", Integer.valueOf(day), Integer.valueOf(month),
                                Integer.valueOf(year), Integer.valueOf(hour), Integer.valueOf(minute) });
                    }
                    dir_count++;
                } else {
                    if (optW) {
                        ...
                        WriteOut("%-8s %-3s   %16s %02d-%02d-%04d %2d:%02d\n",
                            new Object[] { name.value, ext, numformat, Integer.valueOf(day), Integer.valueOf(month),
                                Integer.valueOf(year), Integer.valueOf(hour), Integer.valueOf(minute) });
                    }
                    file_count++;
        ...
        if (!optB) {
            ...
            WriteOut(Msg.get("SHELL_CMD_DIR_BYTES_USED"), new Object[] { Long.valueOf(file_count), numformat });
            /*Bit8u*/short drive = dta.GetSearchDrive();
            //TODO Free Space
            /*Bitu*/int free_space = 1024 * 1024 * 100;
            if (Dos_files.Drives[drive] != null) {
                /*Bit16u*/IntRef bytes_sector = new IntRef(0);
            ...
            WriteOut(Msg.get("SHELL_CMD_DIR_BYTES_FREE"), new Object[] { Long.valueOf(dir_count), numformat });
        }
        Dos.dos.dta(save_dta);
    };

    handler CMD_DELETE = a -> {
        StringRef args = new StringRef(a);
        if (HELP(args, "DELETE"))
            return;
        /* Command uses dta so set it to our internal dta */
        ...
        StringRef name = new StringRef();
        /*Bit32u*/LongRef size = new LongRef(0);
        /*Bit16u*/IntRef time = new IntRef(0), date = new IntRef(0);
        /*Bit8u*/ShortRef attr = new ShortRef(0);
        Dos_DTA dta = new Dos_DTA(Dos.dos.dta());
        while (res) {
            dta.GetResult(name, size, date, time, attr);
            if ((attr.value & (Dos_system.DOS_ATTR_DIRECTORY | Dos_system.DOS_ATTR_READ_ONLY)) == 0) {
                if (!Dos_files.DOS_UnlinkFile(path + name.value))
                    WriteOut(Msg.get("SHELL_CMD_DEL_ERROR"), new Object[] { full.value });
            }
            res = Dos_files.DOS_FindNext();
        }
        Dos.dos.dta(save_dta);
    ...
    handler CMD_SET = ar -> {
        ...
        if (p < 0) {
            if (!GetEnvStr(args.value, line))
                WriteOut(Msg.get("SHELL_CMD_SET_NOT_SET"), new Object[] { args.value });
            WriteOut(line.value + "\n");
        } else {
    ...
    handler CMD_IF = a -> {
        ...
        while (args.value.toUpperCase().startsWith("NOT") && args.value.length() > 3) {
            if (!StringHelper.isspace(args.value.charAt(3)) && args.value.charAt(3) != '=')
                break;
    ...
    handler CMD_TIME = a -> {
        ...
        if (ScanCMDBool(args, "h")) {
            ...
            long ticks = (long) ((calendar.get(Calendar.HOUR_OF_DAY) * 3600 + calendar.get(Calendar.MINUTE) * 60
        ...
        */
        if (timeonly) {
            ...
            WriteOut(StringHelper.sprintf("%2d:%02d:%02d,%02d\n",
                new Object[] { Integer.valueOf(CPU_Regs.reg_ecx.high()), Integer.valueOf(CPU_Regs.reg_ecx.low()),
                    Integer.valueOf(CPU_Regs.reg_edx.high()), Integer.valueOf(CPU_Regs.reg_edx.low()) }));
        }
    };
    ...
    handler CMD_SUBST = a -> {
        ...
        if (!Dos_files.DOS_MakeName(arg, fulldir, drive) || !(Dos_files.Drives[drive.value] instanceof Drive_local)) {
            WriteOut(Msg.get("SHELL_CMD_SUBST_FAILURE"));
            return;
        }
    ...
    handler CMD_CHOICE = a -> {
        ...
        if (args.value.length() > 0) {
            ...
            if (rem != null && rem.charAt(0) == ':')
                rem = rem.substring(1); /* optional : after /c */
        }
        if (rem == null || rem.length() == 0)
            rem = defchoice; /* No choices specified use YN */
        if (!optS)
            rem = rem.toUpperCase(); /* When in no case-sensitive mode. make everything upcase */
        if (args.value.length() > 0) {
            args.value = StripSpaces(args.value);
    ...
    handler CMD_VER = a -> {
        ...
        if (args.value.length() > 0) {
            ...
            WriteOut(Msg.get("SHELL_CMD_VER_VER"), new Object[] { Config.VERSION,
                Integer.valueOf(Dos.dos.version.major), Integer.valueOf(Dos.dos.version.minor) });
    };

    SHELL_Cmd[] cmd_list = { new SHELL_Cmd("DIR", 0, CMD_DIR, "SHELL_CMD_DIR_HELP"),
        ...
        new SHELL_Cmd("HELP", 1, CMD_HELP, "SHELL_CMD_HELP_HELP"), new SHELL_Cmd("IF", 1, CMD_IF, "SHELL_CMD_IF_HELP"),
        ...
        new SHELL_Cmd("VER", 0, CMD_VER, "SHELL_CMD_VER_HELP"), new SHELL_Cmd(null, 0, null, null) };
